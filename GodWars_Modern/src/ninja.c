...
ch->pcdata->powers[NPOWER_CHIKYU],
	...
	if (!str_cmp(arg1,"sora"))
	{
		...
		if (ch->pcdata->powers[NPOWER_SORA] > 7)
                 send_to_char("Kakusite -Mask- able to mask as players (mask target)\n\r", ch);
                return;
	}
	...
	int max;
	     ...
	     if (!str_cmp(arg1,"sora"   )) {improve = NPOWER_SORA;   max=8;}
...
void do_michi(  CHAR_DATA *ch, char *argument  )
{
	...
	if (!IS_NINJA(ch) )
	{
        ...
        if (!str_cmp(ch->clan, "Nightstalkers"))
        {
            send_to_char("Nightstalkers are unable to reach a state of michi.\n\r",ch);
            return;
        }
...
void do_noutcast( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char   buf[MAX_STRING_LENGTH];
    ...
    if (!IS_NINJA(victim))
    {
	send_to_char("They aren't a ninja.\n\r", ch );
	return;
    }
    ...
    if (IS_NINJA(victim) && str_cmp(victim->clan, ch->clan))
    {
    ...
    victim->lord = str_dup("");
    do_save(victim, "");
	sprintf(buf, "You have outcast %s from the %s clan!\n\r",victim->name,ch->clan);
	send_to_char(buf, ch);
	sprintf(buf, "%s has outcast you from the %s clan!\n\r",ch->name,ch->clan);
	send_to_char(buf, victim);
}

void do_humble( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char      arg [MAX_INPUT_LENGTH];
    ...
    if (IS_NPC(ch)) return;
        ...
        if ( !IS_NINJA(ch) || (!IS_EXTRA(ch, EXTRA_PRINCE) && ch->vampgen > 2))
	{
		send_to_char("Huh?\n\r", ch);
		return;
	}
...
/*    if (!str_cmp(ch->clan, "Nightstalkers"))
    {
        send_to_char( "Nightstalkers are not able to class others.\n\r", ch );
        return;
    }*/
    ...
    if (IS_NINJA(victim) && strlen(victim->clan) <3)
    {
    ...
    {
        send_to_char( "You cannot discipline someone that was a vampire.\n\r", ch );
        return;
    }

	if (!IS_NINJA(victim) && victim->class !=0){
...
void do_promote(CHAR_DATA *ch, char *argument )
{
    ...
    do_save(victim, "");
        sprintf(buf, "You have promoted %s to Chunin!\n\r",victim->name);
        send_to_char(buf, ch);
        sprintf(buf, "You have been promoted to Chunin of %s!\n\r",ch->clan);
        send_to_char(buf, victim);
}
...
void do_demote( CHAR_DATA *ch, char *argument )
{
    ...
    REMOVE_BIT(victim->extra, EXTRA_PRINCE);
    do_save(victim, "");
        sprintf(buf, "You have demoted %s to Genin.\n\r",victim->name);
        send_to_char(buf, ch);
        sprintf(buf, "You have been demoted to Genin of %s.\n\r",ch->clan);
        send_to_char(buf, victim);
}
...
void do_kakusu( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_NINJA(ch) && ch->pcdata->powers[NPOWER_SORA] < 3 )
	{
...
/*if (ch->fight_timer >0) {
	...
	if ( ch->pcdata->power[NINJAKI] < 30 )
	{
		send_to_char("You don't have 30 ki to activate your power.\n\r", ch);
		return;
	} 

    if ( IS_AFFECTED(ch, AFF_NINJAHIDE) )
    {
...
{
	if(IS_NPC(ch)) return;
	if(!IS_NINJA(ch) || ch->pcdata->powers[NPOWER_CHIKYU] < 7)
	{
		send_to_char("You have not learned the Chikyu principle to 7.\n\r", ch);
		return;
	}
	else if( ch->pcdata->power[NINJAKI] < 15 )
	{
...
void do_kanzuite( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NINJA(ch) && ch->pcdata->powers[NPOWER_SORA] < 5 )
	{
	  send_to_char("You have not learned the Sora principle to 5.\n\r", ch);
	  return;
	}
	
	else if ( ch->pcdata->power[NINJAKI] < 30 )  
	{
...
void do_mienaku( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_NINJA(ch) && ch->pcdata->powers[NPOWER_NINGENNO] < 3 )
	{
...
/* Use escape instead of flee so people know it's the ninja power */
	...
	WAIT_STATE(victim, 10);
    	act("You start to move at super speeds and escape $N.",ch,NULL,victim,TO_CHAR);
    	act("$n starts to move at super speeds and escapes $N.",ch,NULL,victim,TO_ROOM);
    	act("$n starts to move at super speeds and escapes you.",ch,NULL,victim,TO_VICT);
    	
    	return;
    }
  
	  ch->pcdata->power[NINJAKI] -= 20; 
...
void do_ningentra( CHAR_DATA *ch, char *argument )
{
	...
	if (!str_cmp(ch->clan, "Jurojin")) rvnum = 31920;
	if (!str_cmp(ch->clan, "Kotaro")) rvnum = 16700;
	if (!str_cmp(ch->clan, "Bishamon")) rvnum = 16852; 
	if (rvnum == -1) 
	{
		send_to_char("Your clan HQ has not been recognised...\n\r", ch);
		send_to_char("Please request Aristoi to add the HQ to the list.\n\r", ch);
		return;
	}
...
void do_tsume(CHAR_DATA *ch, char *argument) {
	...
	if (!IS_NINJA(ch) || ch->pcdata->powers[NPOWER_NINGENNO] < 1) {
...
void do_hara_kiri(CHAR_DATA *ch, char *argument) {
	...
	send_to_char("You have not learned the Chikyu principle to 6.\n\r", ch );
	return;}
	...
	{
	    ch->hit = ch->hit/hara_dam;
	    ch->mana = ch->mana/hara_dam;
	    ch->move = ch->move/hara_dam;
	}

	send_to_char("You cut your finger and bleed profusely.\n\r", ch );
	act("$n cuts $s finger and obtains the power of HaraKiri.",ch,NULL,NULL,TO_ROOM);
	return;
}
...
void do_trip(CHAR_DATA *ch, char *argument) {
	...
	if(!IS_NINJA(ch) || ch->pcdata->powers[NPOWER_NINGENNO] < 7 ) {
		send_to_char("You have not learned the Ninjenno principle to 7.\n\r", ch);
		return;
	}
...
void do_mizotamouyo( CHAR_DATA *ch, char *argument )
{
	...
	if( IS_NPC(ch) ) return;
	if( !IS_NINJA(ch) || ch->pcdata->powers[NPOWER_SORA] < 7 )
	{
		send_to_char("You have not learned the Sora principle to 7.", ch);
		return;
	}
    if (ch->fight_timer > 0) 
	{
	...
	{
	    guardian_message( victim );
	    act("$n just failed to use Step on you.",ch,NULL,victim,TO_VICT);
	}
	return;
    }
