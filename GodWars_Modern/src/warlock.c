...
void do_spheres( CHAR_DATA *ch, char *argument )
{
  ...
  if (IS_MAGE(ch) && IS_SET(ch->more, MORE_MARAUDER))
  {
    stc("Real Magi do not need spheres!\n\r",ch);
    return;
  }
  if ( arg1[0] == '\0' )
  {
  ...
  if ( arg2[0] == '\0' )
  {
	...
	centre_text(buf,ch);
	sprintf(buf,"This increases the level of your spells by %f.",ch->spheres[MENT]*.03);
	centre_text(buf,ch);
	divide_to_char(ch);
	return;
      }
