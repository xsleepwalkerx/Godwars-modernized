...
*/
	...
	if ( IS_ANGEL(ch) )
	{
	    ...
	    send_to_char("Aura (2500), Claws (2500), Feet (1500), Nightsight (3000),\n\r",ch);
	    ...
	    send_to_char("Angelform (25000), Cherub (10000), Inferno (10000), Eyespy (1000), Windwall (1000).\n\r",ch);
	    send_to_char("Vision (10000 per increase), Rejuvenation (10000 per increase).\n\r",ch);
	}
	else
	{
	    send_to_char( "Syntax: Inpart <person> <power>\n\r", ch );
	...
	if (!IS_SET(ch->act, PLR_DEMON))
        {
            if ( IS_ANGEL(ch) )
                send_to_char("Only the Archangel has the power to inpart pact.\n\r",ch);
           else
                send_to_char("Only the Demon Lord has the power to inpart pact.\n\r",ch);
            return;
        }
    ...
    else if (!str_cmp(arg2,"firewall") && !IS_ANGEL(ch)) 
	{inpart = DEM_FIREWALL; inpart2 = C_POWERS; cost = 1000;}
    else if (!str_cmp(arg2,"windwall") && IS_ANGEL(ch)) 
	{inpart = DEM_FIREWALL; inpart2 = C_POWERS; cost = 1000;}
    ...
    else if (!str_cmp(arg2,"rejuvenation") && IS_ANGEL(ch)) 
    {
	cost = 10000;
	if ( victim->pcdata->powers[DISC_OBEAH] >= 5 )
	{
	    send_to_char("They already have Supreme Rejuvenation.\n\r",ch);
	    return;
	}
	if (ch->pcdata->power[POWER_TOTAL] < cost || 
	    ch->pcdata->power[POWER_CURRENT] < cost)
...
*/
	...
	send_to_char("Colour can be one of: Violet Purple Red Black Midnight.\n\r",ch);
	send_to_char("Armour can be one of: Ring Collar Plate Helmet Leggings Boots Gauntlets\n\r                      Sleeves Cape Belt Bracer Visor.\n\r",ch);
	return;
    }
    if (str_cmp(arg1,"violet") && str_cmp(arg1,"purple") && str_cmp(arg1,"red") 
    && str_cmp(arg1,"black") && str_cmp(arg1,"midnight"))
    {
	send_to_char("Syntax: demonarmour <colour> <armour>\n\r",ch);
	send_to_char("Colour can be one of: Violet Purple Red Black Midnight.\n\r",ch);
	send_to_char("Armour can be one of: Ring Collar Plate Helmet Leggings Boots Gauntlets\n\r                      Sleeves Cape Belt Bracer Visor.\n\r",ch);
    ...
    if (!str_cmp(arg1,"violet") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000))
    {
	send_to_char("It costs 5000 points of power to create a piece of Violet demon armour.\n\r",ch);
	return;
    }
    if (!str_cmp(arg1,"purple") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000 || ch->pcdata->quest < 100))
    {
	send_to_char("It costs 5000 power and 100 qp to create a piece of Purple demon armour.\n\r",ch);
	return;
    }
    if (!str_cmp(arg1,"red") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000 || ch->pcdata->quest < 250))
    {
	send_to_char("It costs 5000 power and 250 qp to create a piece of Red demon armour.\n\r",ch);
	return;
    }
    if (!str_cmp(arg1,"black") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000 || ch->pcdata->quest < 750))
    {
	send_to_char("It costs 5000 power and 750 qp to create a piece of Black demon armour.\n\r",ch);
	return;
    }
    if (!str_cmp(arg1,"midnight") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000 || ch->pcdata->quest < 1500))
    {
	send_to_char("It costs 5000 power and 1500 qp to create a piece of Midnight demon armour.\n\r",ch);
	return;
    }
    ...
    if (!str_cmp(arg1,"red"))
    {obj->specpower = ARM_RED;ch->pcdata->quest -= 250;obj->points += 250;}
    if (!str_cmp(arg1,"black"))
    {obj->specpower = ARM_BLACK;ch->pcdata->quest -= 750;obj->points += 750;}
    if (!str_cmp(arg1,"midnight"))
    {obj->specpower = ARM_MIDNIGHT;ch->pcdata->quest -= 1500;obj->points += 1500;}
...
*/
	...
	send_to_char("Colour can be one of: Green Blue Ivory White Silver.\n\r",ch);
	send_to_char("Armour can be one of: Ring Collar Plate Helmet Leggings Boots Gauntlets\n\r                      Sleeves Cape Belt Bracer Visor.\n\r",ch);
	return;
    }
    if (str_cmp(arg1,"green") && str_cmp(arg1,"blue") && str_cmp(arg1,"ivory") 
    && str_cmp(arg1,"white") && str_cmp(arg1,"silver"))
    {
	send_to_char("Syntax: angelarmour <colour> <armour>\n\r",ch);
	send_to_char("Colour can be one of: Green Blue Ivory White Silver.\n\r",ch);
	send_to_char("Armour can be one of: Ring Collar Plate Helmet Leggings Boots Gauntlets\n\r                      Sleeves Cape Belt Bracer Visor.\n\r",ch);
    ...
    if (!str_cmp(arg1,"green") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000))
    {
	send_to_char("It costs 5000 points of power to create a piece of Green angel armour.\n\r",ch);
	return;
    }
    if (!str_cmp(arg1,"blue") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000 || ch->pcdata->quest < 100))
    {
	send_to_char("It costs 5000 power and 100 qp to create a piece of Blue angel armour.\n\r",ch);
	return;
    }
    if (!str_cmp(arg1,"ivory") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000 || ch->pcdata->quest < 250))
    {
	send_to_char("It costs 5000 power and 250 qp to create a piece of Ivory angel armour.\n\r",ch);
	return;
    }
    if (!str_cmp(arg1,"white") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000 || ch->pcdata->quest < 750))
    {
	send_to_char("It costs 5000 power and 750 qp to create a piece of White angel armour.\n\r",ch);
	return;
    }
    if (!str_cmp(arg1,"silver") && (ch->pcdata->power[0] < 5000 || 
	ch->pcdata->power[1] < 5000 || ch->pcdata->quest < 1500))
    {
	send_to_char("It costs 5000 power and 1500 qp to create a piece of Silver angel armour.\n\r",ch);
	return;
    }
    if ( vnum == 0 || (pObjIndex = get_obj_index( vnum )) == NULL)
    {
    ...
    {obj->specpower = ARM_IVORY;ch->pcdata->quest -= 250;obj->points += 250;}
    if (!str_cmp(arg1,"white"))
    {obj->specpower = ARM_WHITE;ch->pcdata->quest -= 750;obj->points += 750;}
    if (!str_cmp(arg1,"silver"))
    {obj->specpower = ARM_SILVER;ch->pcdata->quest -= 1500;obj->points += 1500;}
...
void do_horns( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_ANGEL(ch))
	{
	send_to_char("Your halo fades into nothingness.\n\r",ch);
	act("$n's halo fades into nothingness.", ch, NULL, NULL, TO_ROOM);
	}
	else
	{
	...
	if (IS_ANGEL(ch))
	{
    send_to_char("A halo of blue fire forms around your head.\n\r",ch);
    act("A halo of blue fire forms around $n's head.", ch, NULL, NULL, TO_ROOM);
	}
	else
	{
...
void do_wings( CHAR_DATA *ch, char *argument )
{
	...
	if (ch->pcdata->obeah > 0)
	   {
	     send_to_char("You cannot close your wings until they stop glowing.\n\r",ch);
	     return;
           }
...
void do_pact( CHAR_DATA *ch, char *argument )
{
    ...
    if (!can_sire || IS_ACT(victim, PLR_CLASSELDER))
    {
    ...
    if ( IS_NINJA(victim) )
    {
        send_to_char( "You cannot make a pact with the killers.\n\r", ch );
        return;
    }      

    if ( IS_MONK(victim) )
    {
        send_to_char( "You cannot make a pact with the monks.\n\r", ch );
        return;
    } 

    if ( IS_SET(victim->act,WAS_STAKED))
    {
        send_to_char( "You cannot make a pact with someone that was a vampire.\n\r", ch );
        return;
    }
    ...
    if ( IS_BAALI(ch) || IS_NEPHANDI(ch) || IS_BLACK_SPIRAL_DANCER(ch))
    {
	send_to_char("Hybrids can't have followers.\n\r",ch);
	return;
    }
...
//	victim->max_hit += 8000;
//	victim->pcdata->power[POWER_CURRENT] += 20000;
//	victim->pcdata->power[POWER_TOTAL] += 20000;
    }
    else
    {
	ch->exp = ch->exp - 666;
    ...
    REMOVE_BIT(victim->act, PLR_WIZINCOG);
    REMOVE_BIT(victim->extra, EXTRA_SIRE);
    REMOVE_BIT(victim->extra, EXTRA_PRINCE);
    victim->vampgen = 0;
    victim->vampaff = 0;
...
void do_offersoul( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You will allow demons to buy your soul.\n\r",ch);
...
void do_humanform( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch)) return;
    if (!IS_DEMON(ch) && !IS_ANGEL(ch) && !IS_NEPHANDI(ch) && !IS_BAALI(ch) && !IS_BLACK_SPIRAL_DANCER(ch))
    {
	send_to_char("Huh?\n\r", ch);
	return;
    }
...
void do_wallflames( CHAR_DATA *ch, char *argument )
{
    ...
    int        duration = number_range(30,90);
    int        direction, rev;
    ...
    {
	send_to_char("You haven't been granted the gift of Wind Wall.\n\r",ch);
	return;
    }
    ...
    obj->timer = duration;
    free_string(obj->name);
    obj->name = str_dup("wall swirling wind");
    free_string(obj->short_descr);
    obj->short_descr = str_dup("a wall of wind");
    sprintf(buf,"A swirling wall of buffering winds resists passage %s.",arg);
    free_string(obj->description);
    obj->description = str_dup(buf);
    obj_to_room( obj, ch->in_room );

    SET_BIT(pexit->exit_info, EX_WALL_WIND);

    if (direction == DIR_UP)
	sprintf(buf,"$n's wings buffer and create a wall of wind above you.");
    else if (direction == DIR_DOWN)
	sprintf(buf,"$n's wings buffer and create a wall of wind below you.");
    else
	sprintf(buf,"$n's wings buffer and create a wall of wind to the %s.",arg);
...
void do_hooves( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_ANGEL(ch))
	{
	send_to_char("The angelic nimbus over your feet fades away.\n\r",ch);
	act("The angelic nimbus over $n's feet fade away.", ch, NULL,NULL,TO_ROOM);
	}
	else
	{
	...
	if (IS_ANGEL(ch))
	{
    send_to_char("Your feet are engulfed in a blue nimbus.\n\r",ch);
    act("$n's feet are engulfed in a blue nimbus.", ch, NULL, NULL,TO_ROOM);
	}
	else
	{
...
void do_demonform( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_ABOMINATION(ch) && IS_SET(ch->act, PLR_WOLFMAN))
        ...
        {
        sprintf(buf, "A great demonic beast [%s]", ch->name);
        free_string( ch->morph );
        ch->morph = str_dup( buf );
        }
    return;
...
void do_angelform( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->pcdata->power[POWER_TOTAL] < cost ||
            ch->pcdata->power[POWER_CURRENT] < cost)
        {
            send_to_char("You have insufficient power.\n\r",ch);
            return;
        }
        ch->pcdata->power[POWER_TOTAL] -= cost;
        ch->pcdata->power[POWER_CURRENT] -= cost;
    ...
    {
	send_to_char("Your body grows and distorts into a radiant Angelic form.\n\r",ch);
	act("$n's body grows and distorts into a radiant Angelic form.",ch,NULL,NULL,TO_ROOM);
    }
    SET_BIT(ch->vampaff, VAM_CLAWS);
    SET_BIT(ch->vampaff, VAM_NIGHTSIGHT);
    SET_BIT(ch->affected_by, AFF_POLYMORPH);
...
//	ch->morph = str_dup( "an enormous golden werewolf" );
        {
        sprintf(buf, "An enormous golden werewolf [%s]", ch->name);
        free_string( ch->morph );
        ch->morph = str_dup( buf );
        }
    else
//	ch->morph = str_dup( "a beautiful golden beast" );
        {
        sprintf(buf, "A radiant Angelic form [%s]", ch->name);
        free_string( ch->morph );
        ch->morph = str_dup( buf );
        }
    return;
...
void do_orb( CHAR_DATA *ch, char *argument )
{
  CHAR_DATA *victim;
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if ( arg1[0] == '\0' )
  {
    stc("Whom do you wish to fly to?\n\r",ch);
    return;
  }
  ...
  if ( ch == victim)
  {
     stc("Why you wanna fly to your self?,\n\r", ch);
     return;
  }
    if ( victim->level > 700 )
    {
...
/*
  if ( !IS_NPC(victim))
  {
    send_to_char("You can only fly to mobs.\n\r", ch );
    return;
  }
*/
...
act("You raise your wings and fly away.",ch,NULL,victim,TO_CHAR);
    char_from_room(ch);
    char_to_room( ch, victim->in_room );
    do_look(ch,"");
act("You land gracefully.",ch,NULL,victim,TO_CHAR);
act("$n lands from the sky, and gently tucks $s wings away.",ch,NULL,victim,TO_ROOM);
 

  return;
}

void do_wattack( CHAR_DATA *ch, char *argument )
{
    char arg [MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    int dam;
    ...
    {
        send_to_char("Who do you wish to use Wing Attack on?\n\r",ch);
        return;
    }
    ...
    if (is_safe(ch,victim))
    {
        ...
        {
            guardian_message( victim );
            act("$n just failed to use Wing Attack on you.",ch,NULL,victim,TO_VICT );
        }
        return;
    }
    WAIT_STATE( ch, 12 );
    dam = number_range(45,55);dt += TYPE_ADD_AGG;
    dam += char_damroll(ch);
    if (dam == 0) dam = 1;
    if ( !IS_AWAKE(victim) )
        dam *= 2;
...
*/
    ...
    {
        dam *= 1.8;
        send_to_char("They are aligned evil!\n\r", ch);
    }

    if ( dam <= 0 )
        dam = 1;

    act("Your wings unfold, and a bolt of white energy hits $N.",ch,NULL,victim,TO_CHAR);
    act("$n strikes $N with a bolt of white energy.",ch,NULL,victim,TO_NOTVICT);
    act("$n hits you with a bolt of white energy.",ch,NULL,victim,TO_VICT);

    damage( ch, victim, dam, gsn_wattack + TYPE_ADD_AGG );
    return;
}

void do_spiritshare( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char arg[MAX_INPUT_LENGTH];
    char buf[MAX_STRING_LENGTH];
    ...
    {
      send_to_char("You require level 1 Rejuvenation to use Spiritshare.\n\r",ch);
      return;
    }

    if (!IS_SET(ch->pcdata->disc[C_POWERS], DEM_WINGS))
    {
	send_to_char("Rejuvenation requires your wings.\n\r",ch);
	return;
    }
    ...
    if (ch->pcdata->obeah > 0)
    {
      if (ch->pcdata->obeah > 1)
          sprintf(buf,"You cannot call upon your Rejuvenation powers for another %d seconds\n\r",ch->pcdata->obeah);
          else
          sprintf(buf,"You cannot call upon your Rejuvenation powers for another 1 second\n\r");
          send_to_char(buf,ch);
	  return;
    }
    ...
    if (arg[0] == '\0' )
    {
       send_to_char("Who you want to share your spirit with?\n\r",ch);
       return;
    }

    if (victim == ch)
    {
       send_to_char("You can't share your spirit with yourself.\n\r",ch);
       return;
    }
    ...
    if (IS_EVIL(victim))
    {
       send_to_char("They are to evil to benefit from the powers of Rejuvenation.\n\r",ch);
       return;
    }
    ...
    ch->hit -= spirit;
    ch->pcdata->obeah = 10;
    act("You imbue $N with some portion of your strength.",ch,NULL,victim,TO_CHAR);
    act("$n imbues you with some of $s strength",ch,NULL,victim,TO_VICT);
    act("$n shares the strength and health of $s spirit with $N",ch,NULL,victim,TO_NOTVICT);

    if (!IS_NPC(victim) && ch != victim && IS_MORE(victim, MORE_GUARDIAN))
    {
        guardian_message( victim );
        act("$n just used SpiritShare on you.",ch,NULL,victim,TO_VICT);
    }
    return;
}
...
void do_strengthen( CHAR_DATA *ch, char *argument )
{
    ...
    {
      send_to_char("You require level 2 Rejuvenation to use Strengthen.\n\r",ch);
      return;
    }
    if (!IS_SET(ch->pcdata->disc[C_POWERS], DEM_WINGS))
    {
    ...
    if (arg[0] == '\0' )
    {
       send_to_char("Who you want to strengthen?\n\r",ch);
       return;
    }
    ...
    {
      if (IS_SET(victim->more, MORE_NOPAIN))
      {
        send_to_char("They have already been strengthened.\n\r",ch);
        return;
      }
    }
    ...
    {
       send_to_char("They do not require any strengthening.\n\r",ch);
       return;
    }
    ...
    ch->pcdata->obeah = 30;
    victim->pcdata->pain = 300;
    if (victim == ch)
    {
      send_to_char("You strengthen yourself and gain resistance to pain.\n\r",ch);
      act("$n glows with a blue light and strengthens $mself.\n\r",ch,NULL,victim,TO_ROOM);
    }
    else
    {
    act("You gift $N with some strength and render him resistant to pain.",ch,NULL,victim,TO_CHAR);
    act("$n gifts you with some strength, healing your wounds and cleansing you from pain.",ch,NULL,victim,TO_VICT);
    act("$n glows with a blue light as they strengthen $N",ch,NULL,victim,TO_NOTVICT);
    }

    if (!IS_NPC(victim) && ch != victim && IS_MORE(victim, MORE_GUARDIAN))
    {
        guardian_message( victim );
        act("$n just used Strengthen on you.",ch,NULL,victim,TO_VICT);
    }
    return;
}

void do_guardianangel( CHAR_DATA *ch, char *argument )
{
    char buf[MAX_STRING_LENGTH];
    ...
    {
      send_to_char("You require level 3 Rejuvenation to use GuardianAngel.\n\r",ch);
      return;
    }
    if (!IS_SET(ch->pcdata->disc[C_POWERS], DEM_WINGS))
    {
    ...
    ch->pcdata->obeah = 180;
    send_to_char("You summon a spirit for your defence from the void.\n\r",ch);
    act("$n raises $m arms and calls a spirit from the void to defend $s",ch,NULL,NULL,TO_ROOM);

    return;
}

void do_purifyspirit( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char arg[MAX_INPUT_LENGTH];
    char buf[MAX_STRING_LENGTH];
    ...
    {
      send_to_char("You require level 4 Rejuvenation to use PurifySpirit.\n\r",ch);
      return;
    }
    if (!IS_SET(ch->pcdata->disc[C_POWERS], DEM_WINGS))
    {
    ...
    if (arg[0] == '\0' )
    {
       send_to_char("Who's Spirit do you wish to Purify?\n\r",ch);
       return;
    }
    ...
    {
        send_to_char( "There defences are still up.\n\r",ch);
        return;
    }
    if (victim->beast >= 90)
    {
        send_to_char( "You cannot raise their beast anymore.\n\r", ch);
        return;
    }
    if (victim->beast <= 5)
    {
        send_to_char( "You cannot lower their beast anymore.\n\r", ch);
        return;
    }
    ...
    ch->pcdata->obeah = 15;
    act("You purify $N's spirit with your aura.",ch,NULL,victim,TO_CHAR);
    act("$n's aura blazes through you.",ch,NULL,victim,TO_VICT);
    act("$n's aura flows through $N",ch,NULL,victim,TO_NOTVICT);

    if (!IS_NPC(victim) && ch != victim && IS_MORE(victim, MORE_GUARDIAN))
    {
        guardian_message( victim );
        act("$n just used Purify Spirit on you.",ch,NULL,victim,TO_VICT);
    }
    return;
}
...
void do_empower( CHAR_DATA *ch, char *argument )
{
    ...
    {
      send_to_char("You require level 5 Rejuvenation to use Empower.\n\r",ch);
      return;
    }
    if (!IS_SET(ch->pcdata->disc[C_POWERS], DEM_WINGS))
    {
    ...
    if (arg[0] == '\0' )
    {
       send_to_char("Who do you wish to Empower?\n\r",ch);
       return;
    }
    ...
    if (IS_EVIL(victim))
    {
       send_to_char("You can't empower the wicked.\n\r",ch);
       return;
    }
    ...
    update_pos(victim);
    ch->pcdata->obeah = 90;
    act("Your aura empowers $N to their strongest, cleansing weakness.",ch,NULL,victim,TO_CHAR);
    act("$n empowers you, restoring you to your strongest.",ch,NULL,victim,TO_VICT);
    act("$n's aura flashes and empowers $N, leaving them whole and healthy",ch,NULL,victim,TO_NOTVICT);

    if (!IS_NPC(victim) && ch != victim && IS_MORE(victim, MORE_GUARDIAN))
    {
        guardian_message( victim );
        act("$n just used Empower on you.",ch,NULL,victim,TO_VICT);
    }
    return;
}
