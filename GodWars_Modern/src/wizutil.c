...
sprintf (buffer, "#c%5d#n #g%-26.26s#n %s#c%5d#n #g%-26.26s#n(%-8.15s)\n\r",
			  left->vnum, left->name, 
			  sExit,
...
/* make a list of unallocated vnums. Thanos */
void do_vopen( CHAR_DATA *ch, char *argument )
{
    AREA_DATA *pArea;
    char buf[MAX_INPUT_LENGTH];
/* change top_vnum to reflect your muds highest known vnum 
...
/* fill in the used vnums */
    for ( pArea = area_first; pArea; pArea = pArea->next )
	for ( i=pArea->lvnum;i<pArea->uvnum;i++ )
	    vlist[i] = 1;

/* some visual aids */
    send_to_char(" (lvnum - uvnum) " " (lvnum - uvnum) "
		 " (lvnum - uvnum) " " (lvnum - uvnum)\n\r"
		 "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-"
		 "=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n\r", ch);


    for ( i = 1; i < top_vnum; i++ )
    {
	/* if its used skip it */
	if ( vlist[i] > 0 )
	    continue;

	/* set the lower open vnum */
	lvnum = i+1;

	/* run through the list until the next used vnum */
	while ( vlist[i] < 0 ) i++;
	/* set the upper open vnum */
	uvnum = i-1;

	/* the if statement is so we don't list the 0 and other single
	   open vnums */
	if ( lvnum < uvnum )
	{
	    /* format it up */
	    sprintf(buf," (%5d - %5d) ",lvnum,uvnum);
	    send_to_char(buf,ch);
	    if ( col++ % 4 == 0 )
	    	send_to_char("\n\r",ch);
	}
