...
bool    rchance		args( ( int num ) );

bool	rchance( int num)
{
...
/*
	if ( IS_SET(ch->monkstuff, MONK_DEATH) )
	  ...
	  if ( ch->hit > (ch->max_hit /2 ) )
	  {
	      ...
	      send_to_char("Your writhe in agony as magical energies tear you asunder.\n\r",ch);
	  ...
	  else
	  {
	      ...
	      {
	         send_to_char("You feel the magical forces leave your body.\n\r",ch);
	  ...
	  if ( ch->hit < (ch->max_hit /2 ) )
	  {
	    ...
	    if ( ch->move < ch->max_move )
	      ...
	      send_to_char("Your body emits glowing sparks.\n\r",ch);
	  ...
	  else
	  {
	      if ( number_range( 1,2 ) == 1 )
	      {
	         send_to_char("The sparks fizzle and die.\n\r",ch);
	...
	if ( IS_SET(ch->newbits, NEW_POWER) )
 	{
      ...
      if ( ch->chi[CURRENT] > 0 )
      {
        if ( IS_MONK(ch) && ch->move > ( 500 - ch->chi[CURRENT] * 20 ))
            ch->move -= ch->chi[CURRENT] * 20;
        else
	{
	  if ( ch->chi[CURRENT] > 0 )
          {
	    send_to_char("You feel exhausted, and lose control of your ch'i.\n\r",ch);
	    ch->chi[CURRENT] -= 1;
	  }
	}
      }
...
void multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    /* MobProg: fight trigger — fire once per combat round when mob attacks */
    ...
    if (!IS_NPC(ch) && IS_KOE(ch) && ch->pcdata->powers[KOE_BLACKWIND] >= 1 && number_range(1,3 ) == 1)
     {                                                                       
      one_hit(ch, victim, gsn_fury +TYPE_ADD_AGG,0 );                      
     }                                                                       
                                                                         
    if (!IS_NPC(ch) && IS_KOE(ch) && ch->pcdata->powers[KOE_BLACKWIND] >= 1 && number_range(1,3 ) == 1)
     {                                                                        
      one_hit(ch, victim, gsn_backfist +TYPE_ADD_AGG,0 );                  
     }
 
    if (!IS_NPC(ch) && IS_NINJA(ch) 
       ...
       && number_range(1,2 ) == 1)
     {
         ...
         if ( ch->pcdata->powers[NPOWER_NINGENNO] > 3
            && number_range(1,3) == 1)
          {
           one_hit(ch, victim, gsn_shiroken +TYPE_ADD_AGG,0 );
          }
     }
     if ( !IS_NPC(ch) && IS_NINJA(ch) &&
        ch->pcdata->powers[NPOWER_NINGENNO] >= 4)
        {
         spell_poison( gsn_poison,
                     (ch->level*number_range(5,10)),ch,victim );
        }
...
*/
    ...
    if ( !IS_NPC(ch) && get_celerity(ch) > 0 && IS_LICH(ch))
	count += get_celerity(ch);
    if (!IS_NPC(ch) && IS_SKINDANCER(ch) && get_tribe(ch,TRIBE_SILENT_STRIDERS) > 0 )
    {
	if (get_tribe(ch, TRIBE_SILENT_STRIDERS) > 2 ) count += 4;
	else count++;
    }
    
    if (!IS_NPC(ch) && IS_WEREWOLF(ch) && get_tribe(ch, TRIBE_SILENT_STRIDERS) > 0 )
    {
	if (get_tribe(ch, TRIBE_SILENT_STRIDERS) > 2 ) count += 7;
	else count++;
    }
    if ( !IS_NPC(ch) && IS_DEMON(ch) && IS_SET(ch->pcdata->disc[C_POWERS], DEM_SPEED) )
	count ++;

    if ( !IS_NPC(ch) && IS_NEPHANDI(ch) && IS_SET(ch->pcdata->disc[C_POWERS], DEM_SPEED) )
	count ++;

    if ( !IS_NPC(ch) && IS_ANGEL(ch) && IS_SET(ch->pcdata->disc[C_POWERS], DEM_SPEED) )
	{
	 if (ch->pcdata->speed > 0)
         {
	  count += (ch->pcdata->speed / 60) + 5;
         }
        else
         {
          count+=5;
...
/*    if ( !IS_NPC(ch)  && !IS_SET(ch->affected_by2, AF2_GARGOYLE) && 
get_celerity(ch) > 0)*/
    ...
    if (!IS_NPC(ch) && IS_WEREWOLF(ch) && get_tribe(ch, TRIBE_SILENT_STRIDERS) > 0 )
    {
	if (get_tribe(ch, TRIBE_SILENT_STRIDERS) > 2 ) count -= 5;
	else count--;
    }
    if (!IS_NPC(ch) && IS_SKINDANCER(ch) && get_tribe(ch, TRIBE_SILENT_STRIDERS) > 0 )
    {
	if (get_tribe(ch, TRIBE_SILENT_STRIDERS) > 2 ) count -= 3;
	else count--;
    }
    if ( !IS_NPC(ch) && IS_KOE(ch))                                  
    {                                                            
        if (ch->pcdata->powers[KOE_BLACKWIND] <= 0) count -= 4;      
        else if (ch->pcdata->powers[KOE_BLACKWIND] == 1) count -= 7; 
        else if (ch->pcdata->powers[KOE_BLACKWIND] == 2) count -= 9; 
        else if (ch->pcdata->powers[KOE_BLACKWIND] == 3) count -= 11;
        else if (ch->pcdata->powers[KOE_BLACKWIND] == 4) count -= 13;
        else if (ch->pcdata->powers[KOE_BLACKWIND] >= 5) count -= 15;
    }

    if (!IS_NPC(ch) && IS_FAE(ch)) count -=  (ch->pcdata->ELEMENTAL_ATTACK);
    if ( !IS_NPC(ch) && IS_DEMON(ch) && IS_SET(ch->pcdata->disc[C_POWERS], DEM_SPEED) )
	count-=3;
    if ( !IS_NPC(ch) && IS_NEPHANDI(ch) && IS_SET(ch->pcdata->disc[C_POWERS], DEM_SPEED) )
	count-=3;
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, int handtype)
{
	...
	else if ( wield == NULL )
	{
		...
		{dam = number_range( 30, 75 );dt += TYPE_ADD_AGG;}
    ...
    if ( IS_MAGE(ch) )
    {
         ...
         if ( ch->spheres[MENT] >= 1 )
            dam *= ( ch->spheres[MENT] *.2 );
     }

    /* Sphere Thing, not sure of -Xentoth */
/*
    if ( !IS_NPC( victim ) )
    {
       if ( IS_MAGE(victim) )
...
/* heavensgate 
    if (IS_ANGEL(ch) && (number_range(1, 2) == 2)
    {
...
void update_damcap( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	default: break;
	case CLASS_FAE:
	    max_dam += ch->pcdata->ELEMENTAL_ATTACK * 100;
	    break;
        case CLASS_MAGE:
              max_dam = 1250;
             if ( ch->spheres[MENT] > 0 )
              max_dam += ( 50 * ch->spheres[MENT] );
        break;
        case CLASS_KOE:                                                   
             if (ch->vampgen < 2) max_dam = 10000;
        else if (ch->vampgen < 3) max_dam = 2000;                         
        else if (ch->vampgen < 4) max_dam = 1900;                         
        else if (ch->vampgen < 5) max_dam = 1800;                         
        else                      max_dam = 1600;                         
             if (ch->pcdata->powers[KOE_BLACKWIND] >= 3 ) max_dam += 500; 
        break;                                                            

        case CLASS_NINJA:
             if (ch->vampgen < 3) max_dam = 1500;
        else if (ch->vampgen < 4) max_dam = 1400;
        else if (ch->vampgen < 5) max_dam = 1300;
        else                              max_dam = 1200;
        max_dam += (ch->pcdata->wolf * 5);
	...
	case CLASS_BLACK_SPIRAL_DANCER:
	    if (get_tribe(ch,TRIBE_SHADOW_LORDS) > 0) max_dam += 150;
	    if (get_tribe(ch,TRIBE_GET_OF_FENRIS) > 1) max_dam += 150;
	    if (IS_SET(ch->act, PLR_WOLFMAN))
		max_dam += ch->pcdata->wolf;
	    if ( get_disc(ch,DISC_POTENCE) > 0)
		max_dam += (get_disc(ch,DISC_POTENCE) * 20);

	    if (IS_SET(ch->act, PLR_DEMON)) max_dam += 350;
	    else if (IS_EXTRA(ch, EXTRA_PRINCE)) max_dam += 250;
	    else if (IS_EXTRA(ch, EXTRA_SIRE)) max_dam += 150;
	    else max_dam += 200;
	...
	case CLASS_WEREWOLF: case CLASS_SKINDANCER:
	    ...
	    {
		if (IS_ITEMAFF(ch, ITEMA_RIGHT_SILVER) && get_tribe(victim,TRIBE_SILVER_FANGS) < 5)
		    max_dam += 250;
		if (IS_ITEMAFF(ch, ITEMA_LEFT_SILVER) && get_tribe(victim,TRIBE_SILVER_FANGS) < 5)
		    max_dam += 250;
	    }
...
*/
	...
	if ( IS_AFFECTED(ch, AFF_HIDE) )
	{
        ...
        if (!IS_NPC(ch) && IS_NINJA(ch) && IS_AFFECTED(ch, AFF_NINJAHIDE))
        {
	...
	if (IS_VAMPIRE(victim) && IS_SET(victim->affected_by2, AF2_GARGOYLE))
        {
 
           if (get_disc(victim, DISC_VISCERATIKA) <= 0) dam *= 1;
	...
	if ( dam < 0 )
        ...
        {
            float faeMod = 1;
            if (IS_SET(victim->newbits, NEW_MONKSKIN)) faeMod -=0.10;
            faeMod -= 2 * victim->pcdata->ELEMENTAL_DEFENCE / 100;
            dam *= faeMod;
        }
...
void hurt_person( CHAR_DATA *ch, CHAR_DATA *victim, int dam, bool agg_dam )
{
    ...
    /* MobProg: HP percent threshold trigger */
    if ( IS_NPC(victim) && dam > 0 && victim->max_hit > 0 &&
         victim->hit > 0 &&
         IS_SET(victim->pIndexData->progtypes, TRIG_HPCNT) )
    {
        PROG_LIST *prg;
        for ( prg = victim->pIndexData->progs; prg; prg = prg->next )
        {
            if ( prg->trig_type != TRIG_HPCNT ) continue;
            int pct = prg->trig_phrase ? atoi(prg->trig_phrase) : 25;
            int cur_pct = victim->hit * 100 / victim->max_hit;
            if ( cur_pct <= pct )
                mprog_hpfall_trigger( victim, ch );
        }
    }
...
{       demon_gain( ch, victim ); }
    ...
    &&   victim->wait == 0 )
    {
	send_to_char("You start to bawl for your mommy.\n\r",victim);
	act("$N starts to bawl for mommy.",ch,NULL,victim,TO_NOTVICT);
    }
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    if (!IS_NPC(victim) && victim->desc==NULL && victim->position != POS_FIGHTING)
    {   
	send_to_char("They are currently LINK-DEAD!\n\r", ch);
	return TRUE;
    }
    if ( ch->in_room == NULL || ((IS_SET(ch->in_room->room_flags,ROOM_SAFE) && victim->death_timer > 0) 
&& victim->race <= 5) )
    {
...
*/
    ...
    if ((IS_SET(victim->in_room->room_flags,ROOM_SAFE) && victim->death_timer > 0) && victim->race <= 5)
    {
	send_to_char( "Something in the air prevents you from attacking.\n\r", ch );
	return TRUE;
    }
...
*/
    ...
    if (( IS_NPC(ch) && ch->level < 700 ) && IS_MORE(victim, MORE_ANIMAL_MASTER) )
    {
...
/* Unclassed check ...needs to be tested   -Xentoth    
    if (!IS_NPC(ch) && !IS_NPC(victim))
    {
        if (ch->class != NULL && victim->class == NULL)
        {
            send_to_char( "You cannot fight the unclassed.\n\r", ch );
            return TRUE;
        }
        if (ch->class == NULL && victim->class != NULL)
        {
            send_to_char( "You are not able to fight the classed.\n\r", ch );
            return TRUE;
        }
    }
*/
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
	    ...
	    || obj->item_type != ITEM_WEAPON || number_percent() <= 50 )
	{
		    ...
		    || obj->item_type != ITEM_WEAPON )
		{
			...
			if ((IS_WEREWOLF(victim) && get_tribe(victim, TRIBE_RED_TALONS) > 2)
			    ...
			    || (IS_ANGEL(victim) && IS_SET(victim->pcdata->disc[C_POWERS],DEM_CLAWS))
			    || (IS_KOE(victim) && KOE_BLACKWIND >= 2))
                        {
			    obj = NULL;
			    claws = TRUE;
			}
	...
	else if (IS_ANGEL(victim) && IS_SET(victim->pcdata->disc[C_POWERS],DEM_CLAWS))
            chance += (char_hitroll(victim) * 0.050);
	else
	...
	if (IS_WEREWOLF(ch) && get_auspice(ch, AUSPICE_PHILODOX) > 3 )
        ...
        if (IS_NINJA(ch))
        {
           ...
           if (ch->pcdata->powers[NPOWER_CHIKYU] > 2)
               chance -= 20;
           if ( ch->pcdata->power[NINJAKI] > 4 )
               chance -= (ch->pcdata->power[NINJAKI] /4);
       }
       if ( IS_MAGE(ch) )
       {
	...
	if (!IS_NPC(victim) && IS_WEREWOLF(victim) && get_tribe(victim, TRIBE_SILENT_STRIDERS) > 0 )
	{
        ...
        if (IS_NINJA(victim))
        {
           ...
           if (victim->pcdata->powers[NPOWER_CHIKYU] > 2)
               chance += 20;
           if (victim->pcdata->power[NINJAKI] > 4)
               chance += (victim->pcdata->power[NINJAKI] / 4);
       }
//	if (IS_FAE(victim)) chance += ch->pcdata->ELEMENTAL_DEFENCE * 3;
    ...
    if (IS_KOE(victim)) chance = 25;                        
    if (IS_KOE(victim) && KOE_BLACKWIND >= 2 ) chance += 40;
    if (IS_KOE(victim) && KOE_BLOOD >= 3 ) chance += 20;
    
    if (IS_ANGEL(ch)) chance = 25;
...
void raw_kill( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    if(!IS_NPC(ch))
    {
/* Stupid stat reduction workaround -Xentoth */
    if ( ( ch->class != 0) && (victim->class == 0)
        && victim->race < 1 && !IS_NPC(victim))
    {
...
/*
    ...
    { 
        ch->exp = 0;
        ch->max_hit = ch->max_hit * .67;
	ch->max_mana = ch->max_mana * .67;
	ch->max_move = ch->max_move * .67;
        ch->paradox[0] += 3;
        ch->paradox[1] += 3;
...
*/
	...
	sprintf(buf, "#r%s#7 has collected the bounty on #r%s#7.", ch->name, victim->name);
	do_info(ch, buf);
	sprintf(buf, "You collect the bounty on %s's head.\n\r", victim->name);
	send_to_char(buf, ch);
        }
        if (rchance(10))
        {
          qpr = number_range(25,50);
          send_to_char("That was an Uber KILL\n\r",ch);
          ch->pcdata->quest += qpr;
        }
    }
    ...
    {
	/* MobProg death trigger — fire before corpse is made */
	if ( IS_NPC(victim) )
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	else if (dxp_info.dxp == 2)
        {
          xp = xp / 2;
                sprintf( buf, "You receive %d experience points due to half xp.\n\r", xp );
        send_to_char( buf, gch );
        }
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim )
{
    ...
    xp = xp * 2; /* Doubled experience - Kai */
    if(gch->played < 36000) { xp = xp * 2; } //Newbie Doubler- Ktulu
    return xp;
}
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	...
	(IS_NPC(victim) && IS_SET(victim->act, ACT_NOPARTS)) )
    {
	    ...
	    else if (IS_VAMPAFF(ch, VAM_CLAWS) && IS_MORE(ch, MORE_HAND_FLAME))
	    {
	    	...
	    	sprintf( buf2, "Your flaming claw hits $N%s%c [#g%d#n]", vs, punct, dam );
	    	sprintf( buf3, "$n's flaming claw hits you%s%c [#r%d#n]", vp, punct, dam );
		critical = TRUE;
	    }
	    else if (IS_VAMPAFF(ch, VAM_CLAWS))
	    {
	    	sprintf( buf1, "$n's claw hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "Your claw hits $N%s%c [#g%d#n]", vs, punct, dam );
	    	sprintf( buf3, "$n's claw hits you%s%c [#r%d#n]", vp, punct, dam );
		critical = TRUE;
	    }
	    else if (IS_MORE(ch, MORE_HAND_FLAME))
	    {
	    	sprintf( buf1, "$n's flaming hand hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "Your flaming hand hits $N%s%c [#g%d#n]", vs, punct, dam );
	    	sprintf( buf3, "$n's flaming hand hits you%s%c [#r%d#n]", vp, punct, dam );
		critical = TRUE;
	    }
	    else
	    {
	    	sprintf( buf1, "$n hits $N%s%c",  vp, punct );
	    	sprintf( buf2, "You hit $N%s%c [#g%d#n]", vs, punct, dam );
	    	sprintf( buf3, "$n hits you%s%c [#r%d#n]", vp, punct, dam );
		critical = TRUE;
	    }
    	}
    	else
		...
		if ( dt >= 0 && dt < MAX_SKILL )
		{
	    	    ...
	    	    sprintf( buf2, "Your %s strikes $N%s%c [#g%d#n]",  attack, vp, punct, dam );
	    	    sprintf( buf3, "$n's %s strikes you%s%c [#r%d#n]", attack2, vp, punct, dam );
		}
		else
		{
	    	    sprintf( buf1, "$n %s $N%s%c",  attack2, vp, punct );
	    	    sprintf( buf2, "You %s $N%s%c [#g%d#n]",  attack, vp, punct, dam );
	    	    sprintf( buf3, "$n %s you%s%c [#r%d#n]", attack2, vp, punct, dam );
		    critical = TRUE;
		}
	    }
		...
		victim->hit += (per_red * (dam-recover) * 0.01);
            ...
            else if ( !IS_NPC(victim) && IS_DEMON(victim) && !IS_NEPHANDI(victim) && (dam-recover) > 0
                && IS_SET(victim->pcdata->disc[C_POWERS], DEM_TOUGH) )
                victim->hit += number_range(1,(dam-recover));
 
            else if ( !IS_NPC(victim) && IS_DEMON(victim) && IS_NEPHANDI(victim) && (dam-recover) > 0
                && IS_SET(victim->pcdata->disc[C_POWERS], DEM_TOUGH) )
                victim->hit += number_range((dam-recover)/2,(dam-(recover/2)));
...
/* Autostancing - Kai */
    ...
    /* MobProg: kill trigger — victim mob sees player initiating attack */
    if ( IS_NPC(victim) )
        mprog_kill_trigger( victim, ch );
...
void do_circle( CHAR_DATA *ch, char *argument )
{
     ...
     if (IS_NINJA(ch) && ch->pcdata->powers[NPOWER_NINGENNO] < 6)
     {
        send_to_char("You have not learned this attack.\n\r",ch);
       return;
      }
    if ( ( victim = get_char_room( ch, arg) ) == NULL)
    {
...
void do_punch( CHAR_DATA *ch, char *argument )
{
	...
	(IS_WEREWOLF(victim) && get_tribe(victim, TRIBE_GET_OF_FENRIS) > 3) 
        || (IS_VAMPIRE(victim) && IS_SET(victim->affected_by2, AF2_GARGOYLE) && get_disc(victim, DISC_VISCERATIKA) > 3)
...
void do_decapitate( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->in_room == NULL || (IS_SET(ch->in_room->room_flags, ROOM_SAFE) && ch->race < 5))
    {
...
*/
    ...
    if (victim->race < 10 || ch->race < 10 )
    {
    if ( ( victim->race < 1 && ch->race > 0 ) || ((victim->race +5 <= ch->race)) )
    {
	...
	log_string( log_buf );
        sprintf( buf2, "<font color=\"#8B0000\"> %s <font color=\"#C0C0C0\"> decapitated by <font color=\"#8B0000\"> %s <font color=\"#C0C0C0\">at %s for no status.", victim->name, ch->name, victim->in_room->name );
        append_decap( ch, DECAP_FILE, buf2 );
	...
	switch (victim->pcdata->runes[0]) 
	{
	    ...
	    case YELLOW_MAGIC: mage_col = "yellow"; break;
            case BLACK_MAGIC:  mage_col = "black";  break;
	}
    ...
    victim->pcdata->wolf = 0;
        ...
        if (rchance(20))
        {
          ...
          send_to_char("That was an Uber Decapitation\n\r",ch);
          ch->pcdata->quest += qpr;
        }
	...
	victim->name, ch->name, victim->in_room->vnum );
    log_string( log_buf );
    sprintf( buf2, "<font color=\"#8B0000\"> %s <font color=\"#C0C0C0\"> decapitated by <font color=\"#8B0000\"> %s <font color=\"#C0C0C0\"> at %s.", victim->name, ch->name, victim->in_room->name );
    append_decap( ch, DECAP_FILE, buf2 );
    return;
}
...
/* End Aristoi */
    ...
    if (!IS_WEREWOLF(ch) && !IS_NPC(victim) && IS_IMMUNE(victim, IMM_VOODOO)) return;
...
void demon_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if (IS_DEMON(ch))
	{
	    ...
	    else if (IS_NPC(victim) && !IS_SET(victim->act, ACT_NOEXP) && dxp_info.dxp == 1)
	    {
		ch->pcdata->power[POWER_CURRENT] += victim->level * 2;
		ch->pcdata->power[POWER_TOTAL] += victim->level * 2;
	    }
