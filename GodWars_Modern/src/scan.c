...
/*
 * scan.c — Ported from Distorted Illusions / ROT 1.4
 * Adapted for GodWars Modernized by removing ROM-specific invis_level,
 * ghost_level, and AFF_FARSIGHT fields (can_see() handles visibility).
 * Exit array capped at 6 (GodWars has 6 exits, not 12).
 */
...
static void scan_list  args( ( ROOM_INDEX_DATA *scan_room, CHAR_DATA *ch, sh_int depth, sh_int door ) );
static void scan_char  args( ( CHAR_DATA *victim,          CHAR_DATA *ch, sh_int depth, sh_int door ) );
...
static void scan_char( CHAR_DATA *victim, CHAR_DATA *ch,
                       sh_int depth, sh_int door )
{
    char buf[MAX_INPUT_LENGTH];
