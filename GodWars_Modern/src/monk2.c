...
int find_dam( CHAR_DATA *ch, CHAR_DATA *victim, int dam, bool foot )
{
  ...
  if ( !IS_NPC(ch) && IS_MONK(ch) )
  {
    ...
    {
	if (IS_WEREWOLF(victim)) dam *= 0.3;
	if (victim->pcdata->tribes[TRIBE_SHADOW_LORDS] > 2 ) dam *= 0.3;
      if ( foot == TRUE )
      {
...
void do_deathtouch( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You need to obtain level 4 spirit to use Death touch.\n\r",ch);
	return;
    }

     if ( ch->focus[CURRENT] < 20 )
     {
	send_to_char("You need atleast 20 focus points to use th is power.\n\r", ch);
	return;
     }
...
void do_healingtouch( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You need to obtain level 3 spirit to use Healing touch.\n\r",ch);
	return;
    }
    ...
    if ( victim == ch )
    {
	send_to_char("You focus your energy, and magical sparks leap out of your body.\n\r",ch);
...
void do_spiritpower( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_MONK(ch) || ch->monkab[BODY] < 3 )
    {
	send_to_char("Huh?\n\r",ch);
	return;
    }
    ...
    {
	send_to_char("Your spiritual power fades.\n\r",ch);
	ch->damroll -= 50;
	ch->hitroll -= 50;
    ...
    ch->hitroll += 50;
    ch->move -= 25;
    send_to_char("Your body pulses with spiritual energy.\n\r",ch);
    return;
...
void do_relax( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->chi[CURRENT] < 1 )
    {
	send_to_char("You are already totally relaxed.\n\r",ch);
	return;
    }

    if ( ch->position == POS_FIGHTING )
    {
	send_to_char("You cannot relax your ch'i while fighting!\n\r",ch);
	return;
    }

    ch->chi[CURRENT]--;
    WAIT_STATE(ch, 12);
    send_to_char("You breathe deeply and relax your focus.\n\r",ch);
...
void do_chi( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->fighting == NULL )
    {
	send_to_char("You must be fighting to focus your ch'i.\n\r",ch);
	return;
    }
    ...
    {
	if (ch->chi[MAXIMUM] == 0)
	  send_to_char("You have not gained any control of your ch'i.\n\r",ch);
	else
	{
    ...
    if ( ch->chi[CURRENT] == 0 )
    {
      send_to_char("Your body flickers with energy.\n\r",ch);
      act("$n's body flickers with energy.",ch,NULL,NULL,TO_ROOM);
    }
    if ( ch->chi[CURRENT] == 1 )
    {
	send_to_char("Your body pulses with energy.\n\r",ch);
	act("$n's body pulses with energy.",ch,NULL,NULL,TO_ROOM);
    }
    if ( ch->chi[CURRENT] == 2 )
    {
	send_to_char("Your body glows with blue energy.\n\r",ch);
	act("$n's body glows with blue energy.",ch,NULL,NULL,TO_ROOM);
    }
    if ( ch->chi[CURRENT] == 3 )
    {
	send_to_char("Your body glows bright red.\n\r",ch);
	act("$n's body glows bright red.",ch,NULL,NULL,TO_ROOM);
    }
    if ( ch->chi[CURRENT] == 4 )
    {
	send_to_char("Your body flashes with power.\n\r",ch);
	act("$n's body flashes with power.",ch,NULL,NULL,TO_ROOM);
    }
    if ( ch->chi[CURRENT] == 5 )
    {
	send_to_char("Your body emits sparks of energy as you fully focus your ch'i.\n\r",ch);
...
void do_knee( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_SET(ch->monkstuff, TECH_KNEE) )
    {
...
void do_sweep( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_SET(ch->monkstuff, TECH_SWEEP) )
    {
        send_to_char("You haven't learned that technique.\n\r",ch);
        return;
    }  
...
void do_elbow( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_SET(ch->monkstuff, TECH_ELBOW) )
    {
	send_to_char("You haven't learned that technique.\n\r",ch);
	return;
    }
...
void do_thrustkick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_SET(ch->monkstuff, TECH_THRUST) )
    {
        send_to_char("You haven't learned that technique.\n\r",ch);
        return;
    } 
...
*/    
    ...
    {
	send_to_char("Your aura glows dark blue for a moment.\n\r",ch);
    ...
    {
	send_to_char("Your aura flashes bright blue.\n\r",ch);
...
void do_spinkick( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_SET(ch->monkstuff, TECH_SPIN) )
    {
        send_to_char("You haven't learned that technique.\n\r",ch);
        return;
    } 
...
*/    
    ...
    if ( IS_SET(ch->monkcrap, COMB_THRUST1) && IS_SET(ch->monkcrap,
COMB_THRUST2) )
    {
	...
	ch->monkcrap = 0;
	send_to_char("You spin around and around, going berserk.\n\r",ch);
...
void do_backfist( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_SET(ch->monkstuff, TECH_BACK) )
    {
        send_to_char("You haven't learned that technique.\n\r",ch);
        return;
    } 
...
sprintf(lin,"===============================================================================\n\r");
    ...
    if ( arg[0] == '\0' )
    {
      send_to_char(" Syntax: Learn <fight|techniques|abilities|chi> <power|chi>\n\r",ch);
      send_to_char("================================| Fight Styles |==============================\n\r",ch);
	sprintf(buf,"       Trip     [%s] Kick      [%s] Bash     [%s] Elbow    [%s] Knee     [%s]   \n\r",
            IS_SET(ch->monkstuff, FS_TRIP) ? "*" : " ",
       	    IS_SET(ch->monkstuff, FS_KICK) ? "*" : " ",
            IS_SET(ch->monkstuff, FS_BASH) ? "*" : " ",
            IS_SET(ch->monkstuff, FS_ELBOW) ? "*" : " ", 
            IS_SET(ch->monkstuff, FS_KNEE) ? "*" : " ");
	send_to_char(buf,ch);
	sprintf(buf,"       Disarm   [%s] Bite      [%s] Dirt     [%s] Grapple  [%s] Punch    [%s]   \n\r",
		IS_SET(ch->monkstuff, FS_DISARM) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_BITE) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_DIRT) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_GRAPPLE) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_PUNCH) ? "*" : " ");
	send_to_char(buf,ch);
	sprintf(buf,"       Rip      [%s] Stamp     [%s] Backfist [%s] Jumpkick [%s] Spinkick [%s]   \n\r",
		IS_SET(ch->monkstuff, FS_RIP) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_STAMP) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_BACKFIST) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_JUMPKICK) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_SPINKICK) ? "*" : " ");
	send_to_char(buf,ch);
	sprintf(buf,"       Sweep    [%s] Charge    [%s] Hurl     [%s] Gouge    [%s] Headbutt [%s]   \n\r",
		IS_SET(ch->monkstuff, FS_SWEEP) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_CHARGE) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_HURL) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_GOUGE) ? "*" : " ",
		IS_SET(ch->monkstuff, FS_HEADBUTT) ? "*" : " ");
	send_to_char(buf,ch);

send_to_char("===============================================================================\n\r",ch);
	send_to_char("                                   Techniques \n\r",ch);
	send_to_char(lin,ch);
	sprintf(buf,"    Thrust Kick [%s]   Spin Kick   [%s]   Backfist    [%s]   Palm Strike [%s]   \n\r",
	  IS_SET(ch->monkstuff, TECH_THRUST) ? "*" : " ",
	  IS_SET(ch->monkstuff, TECH_SPIN)   ? "*" : " ",
	  IS_SET(ch->monkstuff, TECH_BACK)   ? "*" : " ",
	  IS_SET(ch->monkstuff, TECH_PALM)   ? "*" : " ");
	send_to_char(buf,ch);
	sprintf(buf,"    Elbow       [%s]   Sweep       [%s]   Shin Kick   [%s]   Knee        [%s]   \n\r",
	  IS_SET(ch->monkstuff, TECH_ELBOW)  ? "*" : " ",
          IS_SET(ch->monkstuff, TECH_SWEEP)  ? "*" : " ",
          IS_SET(ch->monkstuff, TECH_SHIN)   ? "*" : " ",
          IS_SET(ch->monkstuff, TECH_KNEE)   ? "*" : " ");
	send_to_char(buf,ch);
	send_to_char(lin,ch);
	...
	send_to_char(" 	 	    You are currently not focusing your ch'i\n\r",ch);
	send_to_char(lin,ch);
	return;
    }
  ...
  else 
  {
	...
	{
	  send_to_char("You have already learned all you can in this ability.\n\r",ch);
	  return;
	}
	...
	if ( ch->chi[MAXIMUM] >= 6 )
	{
	  send_to_char("You have already mastered your ch'i.\n\r",ch);
	  return;
	}
	...
	ch->chi[MAXIMUM]++;
	save_char_obj(ch);
	send_to_char("You gain more control over your ch'i.\n\r",ch);
	return;
    }
    else
    {
    if (IS_SET(ch->monkstuff, inpart))
        {
