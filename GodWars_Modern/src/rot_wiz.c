...
void do_pset( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "  quest quest+ quest-\n\r",                      ch );
	send_to_char( "  weapon immune beast dps status\n\r",           ch );
	send_to_char( "  black blue red yellow green purple class\n\r", ch );
	...
	send_to_char( "Immune:  hurl backstab shielded kick disarm\n\r",  ch );
        ...
        send_to_char( "         Highlander Ghoul Angel Ninja Monk\n\r",   ch );
	send_to_char( "         Marauder Gargoyle Fae\n\r", ch );
	return;
    }
	...
	if ( !IS_ANGEL(victim) && (!IS_VAMPIRE(victim) && (!IS_MAGE(victim) 
	&& (!IS_DEMON(victim) && victim->class != CLASS_NONE )))) 
	{
	...
	if ( !str_cmp( arg3, "marauder" ) )
	{
	  if (IS_SET(victim->more, MORE_MARAUDER))
	   {
	   REMOVE_BIT(victim->more, MORE_MARAUDER);
	   send_to_char("Ok.\n\r",ch);
	   send_to_char("You are no longer a Marauder.\n\r", victim);
	   return;
           }
	    REMOVE_BIT(victim->more, MORE_OUTCAST);
	    SET_BIT(victim->more, MORE_MARAUDER);
	    victim->spheres[MCOR] = 0;
	    victim->spheres[MLIF] = 1;
	    victim->spheres[MFOR] = 0;
	    victim->spheres[MENT] = 0;
	    victim->spheres[MMIN] = 0;
	    victim->spheres[MSPI] = 0;
	    victim->spheres[MPRI] = 0;
	    victim->spheres[MTIM] = 5;
	    victim->spheres[MMAT] = 0;
	    send_to_char("Ok.\n\r",ch);
	    send_to_char("You are now a Marauder.\n\r", victim);
	    return;
	  }
	if ( !str_cmp( arg3, "vampire" ) )
	{
	...
	else if ( !str_cmp( arg3, "gargoyle" ) )
        {
            ...
            SET_BIT(victim->affected_by2, AF2_GARGOYLE);
            victim->vampgen = 2;
            update_gen(victim);
            victim->pcdata->cland[0] = -1;
            victim->pcdata->cland[1] = -1;
            victim->pcdata->cland[2] = -1;
            send_to_char("Ok.\n\r",ch);
            send_to_char("You are now a Gargoyle.\n\r",victim);
        }
	else if ( !str_cmp( arg3, "werewolf" ) )
	{
	    victim->class = CLASS_WEREWOLF;
	    victim->vampgen = 1;
	    victim->pcdata->tgift[0] = -1;
	    victim->pcdata->tgift[1] = -1;
	    victim->pcdata->tgift[2] = -1;
	    send_to_char("Ok.\n\r",ch);
