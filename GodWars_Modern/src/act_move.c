...
void move_char( CHAR_DATA *ch, int door )
{
    ...
    if ( IS_SET(pexit->exit_info, EX_WALL_WIND) && !IS_ANGEL(ch))
    {
	send_to_char("You struggle through the wall of buffering winds and fall to your knees.\n\r",ch);
	WAIT_STATE(ch, 9);
...
/* end freeze tag */
	...
	&& ( IS_NPC(ch) || !IS_SET(ch->act, PLR_WIZINVIS) || !IS_SET(ch->act, PLR_WIZINCOG) )
	&& can_see(victim,ch))
	{
	    if (((mount = ch->mount) != NULL && ch->mounted == IS_RIDING &&
	...
	if ( !IS_NPC(ch) && !IS_AFFECTED(ch, AFF_SNEAK) && IS_AFFECTED(ch,AFF_POLYMORPH)
	&& ( IS_NPC(ch) || !IS_SET(ch->act, PLR_WIZINVIS) || !IS_SET(ch->act, PLR_WIZINCOG) ) && can_see(victim,ch))
	{
	    if (((mount = ch->mount) != NULL && ch->mounted == IS_RIDING &&
	    IS_AFFECTED(mount,AFF_FLYING)) || IS_AFFECTED(ch,AFF_FLYING) ||
	    (!IS_NPC(ch) && IS_VAMPAFF(ch,VAM_FLYING)) )
    ...
    room_text(ch,">ENTER<");
    /* MobProg: greet trigger — mobs react to character entering */
    if ( !IS_NPC(ch) )
        mprog_greet_trigger( ch );
    /* RoomProg: enter trigger */
...
void do_write( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
		...
		SET_BIT(obj->spectype, ADV_STARTED);
                ...
                && IS_SET(ch->more, MORE_MARAUDER))
           {
                SET_BIT(obj->spectype, ADV_FINISHED);
                obj->toughness = number_range(0,5);
                obj->points += 1;
           }
	    else if (!str_cmp(arg1,"end.spell") 
		&& IS_SET(obj->spectype, ADV_STARTED) 
...
/* Movement spells re-instated by Maizoon */
...
/*
 * Couch code snippet by Mistif
 * Files affected are act_move, bit, db, merc.h and handler.
 */

  if (obj->item_type == ITEM_FURNITURE 
   || obj->value[0] == 1
   || obj->item_type == ITEM_SIT_SINGLE
   || obj->item_type == ITEM_SIT_DOUBLE
   || obj->item_type == ITEM_SIT_TRIPLE)
  {
  if ( ch->sat != NULL )
    {
...
/*    for(rch=ch->in_room->people; rch != NULL; rch = rch->next_in_room)
	...
	if (obj->sat != NULL)
	 {
	 send_to_char("There is already someone sitting there.\n\r", ch);
	 return;
	 }
	}

    if (obj->item_type == ITEM_SIT_SINGLE)
	{
	if (obj->sitnum == 1)
	 {
	 send_to_char("There is already someone sitting there.\n\r", ch);
    ...
    if (obj->item_type == ITEM_SIT_DOUBLE)
	{
	if (obj->sitnum == 2)
	 {
	 send_to_char("There are already two people sitting here.\n\r", ch);
	 return;
 	 }
	}
    if (obj->item_type == ITEM_SIT_TRIPLE)
	{
	if (obj->sitnum == 3)
 	 {
	 send_to_char("There are already three people sitting here.\n\r", ch);
	 return;
	 }
	}
    if (obj->sitnum == 2)
  ...
  {
  send_to_char("You cannot sit there\n\r", ch);
  return; 
  }
...
{
    if (( !IS_MAGE(ch)  && !IS_NINJA(ch) 
        && !IS_MONK(ch)) || IS_NPC(ch))
...
void do_train( CHAR_DATA *ch, char *argument )
{
   ...
   /* else if (!str_cmp(arg1,"focus") && IS_MONK(ch) )
	is_ok = TRUE;
*/
	...
	if ( cost > 28000 ) cost = 28000; /* Lets make this mud high powered */
	else if ( cost < 1 ) cost = 1;
	pAbility = (int32_t *) &ch->max_hit;
	pOutput          = "hp";
	...
	if ( cost > ch->exp || ch->exp < 1 )
	{
        ...
        if ( ch->max_hit >= 30000 )
        {
           if (last) send_to_char( "Your hit points are at max. \n\r", ch );
           return;
        }

	ch->exp -= (cost < 1 ? 1 : cost);
	ch->max_hit++;
	if (last) act( "Your hp increases!", ch, NULL, pOutput, TO_CHAR );
	return;
    }
	...
	if ( cost > 18000 ) cost = 18000; /* Lets make this mud high powered */
	else if ( cost < 1 ) cost = 1;
	pAbility = (int32_t *) &ch->max_mana;
	pOutput          = "mana";
	if ( cost > ch->exp || ch->exp < 1 )
	{
	    if (last) send_to_char( "You don't have enough exp.\n\r", ch );
        ...
        {
           if (last) send_to_char( "You have already maxed your Mana.\n\r", ch );
           return;
        }

	ch->exp -= (cost < 1 ? 1 : cost);
	ch->max_mana++;
	if (last) act( "Your mana increases!", ch, NULL, pOutput, TO_CHAR );
	return;
    }
	...
	if ( cost > ch->exp || ch->exp < 1 )
	{
        ...
        {
           if (last) send_to_char( "You have already maxed your Move", ch );
           return;
        }

	ch->exp -= (cost < 1 ? 1 : cost);
	ch->max_move++;
	if (last) act( "Your move increases!", ch, NULL, pOutput, TO_CHAR );
	return;
    }

    else if ( !str_cmp( arg1, "primal"))
    {
        if ( ch->practice >= 100 ) return;
        cost        = primal;
...
/*	
	sprintf( buf, "Focus            - %d exp per point.\n\r", focus);
	send_to_char( buf, ch );
*/
...
void do_hunt( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_WEREWOLF(ch) && !IS_NINJA(ch) && !IS_DEMON(ch))
    {
     send_to_char("Huh?\n\r", ch);
     return;
    }
...
void add_tracks( CHAR_DATA *ch, int direction )
{
    ...
    if (IS_NINJA(ch) && ch->pcdata->powers[NPOWER_SORA] > 3) return;
