...
void do_madness( CHAR_DATA *ch, char *argument )
{
   ...
   {
   	send_to_char("You require level 5 Metis to use Madness.\n\r",ch);
	return;
   }
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char( "Who do you want to use Madness on?\n\r", ch );
        return;
    }
    ...
    if (IS_SET(ch->in_room->room_flags, ROOM_SAFE))
        {
            send_to_char("Not while your in a safe room.\n\r",ch);
            return;
        }
    if ( ch == victim)
  {
     stc("Why do you want to make yourself go mad?\n\r", ch);
     return;
  }

    if (victim->madness == ch->name )
    {
       stc("The are already mad!\n\r",ch);
       return;
    }
    
     if (!IS_NPC(victim) && IS_WILLPOWER(victim, RES_DOMINATE))
    {
        act("You shake off $N's attempt to drive you mad.",victim,NULL,ch,TO_CHAR);
        act("$n shakes off $N's attempt to drive $m mad.",victim,NULL,ch,TO_NOTVICT);
        act("$n shakes off your attempt to drive $m mad.",victim,NULL,ch,TO_VICT);
        if (!IS_NPC(victim) && ch != victim && IS_MORE(victim, MORE_GUARDIAN))
        {
            guardian_message( victim );
            act("$n just failed to drive you mad.",ch,NULL,victim,TO_VICT);
        }
        return;
    }

    act("You have sent $N's inner demons raging in there soul.",ch,NULL,victim,TO_CHAR);
    act("$N grows wild and weary as his inner demons corrupt him.",ch,NULL,victim,TO_NOTVICT);
    stc("Your inner demons rebel against you.\n\r",victim);
    victim->madness = ch->name;
...
void do_parttheveil( CHAR_DATA *ch, char *argument )
{
    char arg [MAX_INPUT_LENGTH];  
  CHAR_DATA *victim;
  ...
  if ( !IS_WEREWOLF(ch) ) return;
    ...
    {
	send_to_char("You require the level 5 Homid power to Part the Veil.\n\r",ch);
	return;
    }
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char( "Who do you want to Part the Veil for?\n\r", ch );
        return;
    }
    ...
    if ( ch == victim)
  {
     stc("Why do you want to Part your own Veil?\n\r", ch);
     return;
  }
  if ( !IS_NPC(victim) && !IS_IMMUNE(victim, IMM_SUMMON) )
  {
	stc("You can't part the veil on them.\n\r",ch);
	multi_hit( victim, ch, TYPE_UNDEFINED );
	return;
  }
  if (IS_AFFECTED(victim, AFF_SHADOWPLANE))
  {
     stc("They have already been pulled through the veil.\n\r", ch);
     return;
  }
    act("You part the veil of the universe, pulling $N into it.",ch,NULL,victim,TO_CHAR);
    act("$n opens the veil of the umbra pulling $N within.",ch,NULL,victim,TO_NOTVICT);
    act("$n pulls you through the veil of the universe, and into the umbra.",ch,NULL,victim,TO_VICT);
...
void do_truefear( CHAR_DATA *ch, char *argument )
{
	...
	act("You broke a heart", ch,NULL,victim,TO_CHAR);
        act("Your former loves heart is broken", ch, NULL,victim,TO_VICT);
	}
     }
...
void do_porcupine( CHAR_DATA *ch, char *argument )
{
	...
	act("$n's long quill-like spines retract into their body.", ch, NULL, NULL, TO_ROOM);
...
void do_icy( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char("You start using the Icy Chill of Despair.\n\r",ch);
...
void do_camouflage( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_WEREWOLF(ch) && !IS_VAMPIRE(ch))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
       if (IS_VAMPIRE(ch) && !IS_SET(ch->affected_by2, AF2_GARGOYLE))
    {
        send_to_char("Huh?\n\r", ch);
        return;
    }

    if (IS_WEREWOLF(ch) && get_tribe(ch,TRIBE_WENDIGOS) < 1)
    {
	send_to_char("You require the level 1 power of the Wendigos to use Camouflage.\n\r",ch);
	return;
    }
        if (IS_VAMPIRE(ch) && get_disc(ch,DISC_VISCERATIKA) < 1)
    {
        send_to_char("You requre the level 1 power of Visceratika to use Skin of the Chameleon.\n\r", ch);
        return;
    }
    ...
    if (IS_WEREWOLF(ch) && !IS_AFFECTED(ch, AFF_HIDE)) 
    {
    ...
    if (IS_VAMPIRE(ch))
        {
        act("$n's skin changes its texture and colour.",ch,NULL,NULL,TO_ROOM);
        send_to_char("You activate Skin of the Chameleon.\n\r",ch);
        SET_BIT(ch->affected_by, AFF_HIDE);
    }
    WAIT_STATE( ch, 12 );
    return;
...
*/
    ...
    if ( victim->pcdata->tgift[2] == -1 )
    {
	send_to_char("Not until they choose a Tribe Gift.\n\r", ch);
	return;
    }
    ...
    if (victim->pcdata->tgift[2] == -1)
    {
        send_to_char("They must select their tribe gifts first.\n\r", ch);
        return;
    }
...
void do_bodywrack( CHAR_DATA *ch, char *argument )
{
    ...
    if (get_tribe(ch,TRIBE_BLACK_FURIES) < 3)
    {
	send_to_char("You require the level 3 Black Furies power to use Body Wrack.\n\r",ch);
	return;
    }
...
void do_wwpowers( CHAR_DATA *ch, char *argument )
{
	...
	if (get_breed(ch, BREED_HOMID) > 4) strcat(buf," PartTheVeil");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_breed(ch, BREED_METIS) > 1) strcat(buf," Burrow");
        if (get_breed(ch, BREED_METIS) > 2) strcat(buf," Madness");
    ...
    {
	strcpy(buf,"Ragabash: Blur");
	if (get_auspice(ch, AUSPICE_RAGABASH) > 1) strcat(buf," BlissfulIgnorance");
	if (get_auspice(ch, AUSPICE_RAGABASH) > 2) strcat(buf," Tail ");
        if (get_auspice(ch, AUSPICE_RAGABASH) > 3) strcat(buf," Readaura");
        if (get_auspice(ch, AUSPICE_RAGABASH) > 4) strcat(buf," WhelpBody");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_auspice(ch, AUSPICE_THEURGE) > 4) strcat(buf," Moongate");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_tribe(ch, TRIBE_SHADOW_LORDS) > 3) strcat(buf," Paralyz");
	...
	if (get_tribe(ch, TRIBE_SILVER_FANGS) > 4) strcat(buf," *SilverImmunity");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
...
void do_tail( CHAR_DATA *ch, char *argument )
{
    ...
    if (get_auspice(ch, AUSPICE_RAGABASH) < 3)
    {
	send_to_char("You require the level 3 Ragabash power to use Tail of the Monkey.\n\r",ch);
	return;
    }
...
void do_gifts( CHAR_DATA *ch, char *argument )
{
	...
	for ( sn = BREED_HOMID; sn <= BREED_METIS; sn++ )
	{
	    ...
	    if (ch->pcdata->tgift[0] == sn) strcpy(disc,upper_cap(disc));
	    if (ch->pcdata->breed[sn] != 0 )
	    {
		sprintf( buf, "%-16s: %-2d      ",
		    disc, get_breed(ch,sn) );
		if (ch->pcdata->tgift[0] == sn) ADD_COLOUR(ch, buf, WHITE);
		send_to_char( buf, ch );
		if ( ++col % 3 == 0 ) send_to_char( "\n\r", ch );
	    }
	}
	...
	for ( sn = AUSPICE_RAGABASH; sn <= AUSPICE_AHROUN; sn++ )
	{
	    ...
	    if (ch->pcdata->tgift[1] == sn) strcpy(disc,upper_cap(disc));
	    if (ch->pcdata->auspice[sn] != 0 )
	    {
		sprintf( buf, "%-16s: %-2d      ", disc, get_auspice(ch,sn) );
		if (ch->pcdata->tgift[1] == sn) ADD_COLOUR(ch, buf, WHITE);
		send_to_char( buf, ch );
		if ( ++col % 3 == 0 ) send_to_char( "\n\r", ch );
	    }
	}
	...
	for ( sn = TRIBE_BLACK_FURIES; sn <= TRIBE_WENDIGOS; sn++ )
	{
	    ...
	    if (ch->pcdata->tgift[2] == sn) strcpy(disc,upper_cap(disc));
	    if (ch->pcdata->tribes[sn] != 0)
	    {
		sprintf( buf, "%-16s: %-2d      ", disc, get_tribe(ch,sn) );
		if (ch->pcdata->tgift[2] == sn) ADD_COLOUR(ch, buf, WHITE);
		send_to_char( buf, ch );
		if ( ++col % 3 == 0 ) send_to_char( "\n\r", ch );
	    }
	}
    ...
    if ( ch->pcdata->tgift[gt] == improve ) ww_max = 5;
    ...
    {
	if ((gt == 0 || gt == 1) && ch->pcdata->tgift[gt] == improve) 
	    imp_cost = gift * 10;
	else
	    imp_cost = gift * 15;
    }
    else if (ch->pcdata->tgift[2] == -1 && gt == 2)
	imp_cost = gift * 9;
    else if (ch->pcdata->tgift[gt] == improve)
	imp_cost = gift * 5;
    else
	imp_cost = gift * 10;
...
void do_tribegifts( CHAR_DATA *ch, char *argument )
{
	...
	if ( ch->pcdata->tgift[2] == -1 && (strlen(ch->clan) > 1 || IS_ABOMINATION(ch) || IS_SKINDANCER(ch) || IS_BLACK_SPIRAL_DANCER(ch)) )
	{
