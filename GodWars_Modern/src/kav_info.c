...
void do_spell( CHAR_DATA *ch, char *argument )
{
    ...
    else if (!str_cmp(arg,"black"))  show_spell(ch,5);
...
void show_spell( CHAR_DATA *ch, int dtype )
{
	...
	sprintf(buf,"You are %s yellow magic.\n\r",bufskill);
        ...
        sprintf(buf,"You are %s black magic.\n\r",bufskill);
    else return;
    send_to_char(buf,ch);
    return;
}
...
void do_spydirection( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char("   The Mist is too thick to tell.\n\r",ch);
        return;
    }
...
void birth_date( CHAR_DATA *ch, bool is_self )
{
    ...
    if (is_self)
    {
	if (!IS_EXTRA2(ch, EXTRA_BORN)) yy += 17;
...
int years_old( CHAR_DATA *ch )
{
    ...
    if (!IS_EXTRA2(ch, EXTRA_BORN)) yy += 17; /* Starting age for all players */
    return yy;
}
...
void clear_eq( CHAR_DATA *ch )
{
	...
	if ( in_obj->carried_by != NULL )
	{
		...
		{
		    if      (obj->specpower == ARM_PURPLE  ) max_val = 100;
		    else if (obj->specpower == ARM_RED) max_val = 250;
		    else if (obj->specpower == ARM_BLACK   ) max_val = 750;
		    else if (obj->specpower == ARM_MIDNIGHT ) max_val = 1500;
		    else max_val = 0;
		}
		else max_val = 0;
...
void do_level( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char("#l--------------------------------------------#n\n\r", ch);
	send_to_char("Weapon Skills #l|#n Stances       #l|#n Magic\n\r", ch);
	send_to_char("#l--------------------------------------------#n\n\r", ch);
	sprintf(skill, "Unarmed: %4d #l|#n ", ch->wpn[0]);
	send_to_char( skill, ch );
	sprintf(skill, "Bull    : %3d #l|#n ", ch->stance[5]);
	send_to_char( skill, ch );
	sprintf(skill, "#pPurple#n: %3d\n\r", ch->spl[0]);
	send_to_char( skill, ch );
	sprintf(skill, "Slash  : %4d #l|#n ", ch->wpn[3]);
	send_to_char( skill, ch );
	sprintf(skill, "Viper   : %3d #l|#n ", ch->stance[1]);
	send_to_char( skill, ch );
	sprintf(skill, "#rRed#n   : %3d\n\r", ch->spl[1]);
	send_to_char( skill, ch );
	sprintf(skill, "Slice  : %4d #l|#n ", ch->wpn[1]);
	send_to_char( skill, ch );
	sprintf(skill, "Crane   : %3d #l|#n ", ch->stance[2]);
	send_to_char( skill, ch );
	sprintf(skill, "#lBlue#n  : %3d\n\r", ch->spl[2]);
	send_to_char( skill, ch );
	sprintf(skill, "Stab   : %4d #l|#n ", ch->wpn[2]);
	send_to_char( skill, ch );
	sprintf(skill, "Crab    : %3d #l|#n ", ch->stance[3]);
	send_to_char( skill, ch );
	sprintf(skill, "#gGreen#n : %3d\n\r", ch->spl[3]);
	send_to_char( skill, ch );
	sprintf(skill, "Whip   : %4d #l|#n ", ch->wpn[4]);
	send_to_char( skill, ch );
	sprintf(skill, "Mongoose: %3d #l|#n ", ch->stance[4]);
	send_to_char( skill, ch );
	sprintf(skill, "#yYellow#n: %3d\n\r", ch->spl[4]);
	send_to_char( skill, ch );
	sprintf(skill, "Claw   : %4d #l|#n ", ch->wpn[5]);
	send_to_char( skill, ch );
	sprintf(skill, "Tiger   : %3d #l|#n ", ch->stance[8]);
	send_to_char( skill, ch );
        sprintf(skill, "#0Black#n : %3d\n\r", ch->spl[5]);
        send_to_char( skill, ch );
	sprintf(skill, "Blast  : %4d #l|#n ", ch->wpn[6]);
	send_to_char( skill, ch );
	sprintf(skill, "Mantis  : %3d #l|#n\n\r", ch->stance[6]);
	send_to_char( skill, ch );
	sprintf(skill, "Pound  : %4d #l|#n ", ch->wpn[7]);
	send_to_char( skill, ch );
	sprintf(skill, "Dragon  : %3d #l|#n\n\r", ch->stance[7]);
	send_to_char( skill, ch );
	sprintf(skill, "Crush  : %4d #l|#n ", ch->wpn[8]);
	send_to_char( skill, ch );
	sprintf(skill, "Swallow : %3d #l|#n\n\r", ch->stance[10]);
	send_to_char( skill, ch );
	sprintf(skill, "Grep   : %4d #l|#n ", ch->wpn[9]);
	send_to_char( skill, ch );
	sprintf(skill, "Monkey  : %3d #l|#n\n\r", ch->stance[9]);
	send_to_char( skill, ch );
	sprintf(skill, "Bite   : %4d #l|#n               #l|#n\n\r", ch->wpn[10]);
	send_to_char( skill, ch );
	sprintf(skill, "Pierce : %4d #l|#n               #l|#n\n\r", ch->wpn[11]);
	send_to_char( skill, ch );
	sprintf(skill, "Suck   : %4d #l|#n               #l|#n\n\r", ch->wpn[12]);
	send_to_char( skill, ch );
	send_to_char("#l--------------------------------------------#n\n\r", ch);
	...
	send_to_char( skill, ch );
	send_to_char("#l--------------------------------------------#n\n\r", ch);
	if (IS_WEREWOLF(ch) && !IS_ABOMINATION(ch) && !IS_BLACK_SPIRAL_DANCER(ch) && !IS_SKINDANCER(ch))
	    send_to_char("You are a werewolf.\n\r", ch);
	if (IS_DEMON(ch) && !IS_ANGEL(ch) && !IS_BAALI(ch) && !IS_BLACK_SPIRAL_DANCER(ch) && !IS_NEPHANDI(ch))
	    send_to_char("You are a demon.\n\r", ch);
        if (IS_MAGE(ch) && !IS_LICH(ch) && !IS_NEPHANDI(ch) && !IS_SKINDANCER(ch))
            send_to_char("You are a mage.\n\r", ch);
        if (IS_VAMPIRE(ch) && !IS_ABOMINATION(ch) && !IS_LICH(ch) && !IS_LICH(ch))
