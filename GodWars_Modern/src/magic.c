...
void improve_spl( CHAR_DATA *ch, int dtype, int sn )
{
    ...
    else if (dtype == 5 ) sprintf(buftype,"black");
    else return;
...
void say_spell( CHAR_DATA *ch, int sn )
{
    ...
    else if ( skill_table[sn].target == 5 )
    {
...
void do_cast( CHAR_DATA *ch, char *argument )
{
	...
	!IS_VAMPAFF(ch,VAM_DISGUISED) && !IS_FAE(ch) && !IS_KOE(ch))
    {
	if ( !is_obj(ch) )
	{
...
/*
	...
	AFF_SHADOWPLANE))
    {
        send_to_char( "You are too insubstantial!", ch);
        return;
    }
...
/* Skills shouldnt be able to be casted, this piece of code restricts that
   IE you shouldnt be able to cast peek, you should just peek - Tijer -Added by Xentoth*/
   	...
   	||    ( skill_lookup( arg1 ) == 74 )    /* hide      */
    	...
    	||    ( skill_lookup( arg1 ) == 97 )    /* punch     */
    	||    ( skill_lookup( arg1 ) == 114)    /* berserk   */
    	||    ( skill_lookup( arg1 ) == 120)    /* track     */
        ||    ( skill_lookup( arg1 ) == 115)    /* fastdraw  */
	||    ( skill_lookup( arg1 ) == 151)    /* first aid  */
	||    ( skill_lookup( arg1 ) == 152) )  /* repair    */
    {
...
//    WAIT_STATE( ch, skill_table[sn].beats );
	...
	if (IS_NPC(ch))
        ...
        else if ( IS_MAGE(ch) )
        {
            ...
            {
             (*skill_table[sn].spell_fun) ( sn, (ch->spl[skill_table[sn].target]*(0.25-(ch->spheres[MENT]*.02))), ch, vo );
             improve_spl(ch,skill_table[sn].target,sn);
            }
	    else
	    {
...
void spell_dispel_magic( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (victim->position == POS_STUNNED) update_pos(victim);
...
void spell_enchant_armor( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    ||   obj->chobj != NULL )
    {
        send_to_char("You are unable to enchant this armor.\n\r",ch);
        return;
    }
...
void spell_faerie_fog( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( !IS_NPC(ich) && IS_SET(ich->act, PLR_WIZINCOG) )
	    continue;
...
//	    continue;
    ...
    if ( IS_AFFECTED(ich, AFF_NINJAHIDE) )
        REMOVE_BIT(ich->affected_by, AFF_NINJAHIDE);
	
    if (IS_VAMPAFF(ich, VAM_OBFUSCATE))
	REMOVE_BIT(ich->vampaff, VAM_OBFUSCATE );
...
void spell_locate_object( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( !can_see_obj( ch, obj ) || !is_name( target_name, obj->name ) )
	    continue;
	if ( IS_SET(obj->quest, QUEST_ARTIFACT) && !IS_IMMORTAL(ch)) continue;
	...
	if (i >= listcap)
	{
	sprintf( buf, "there are too many listingsi\n\r");
	continue;
	}
...
void spell_brew( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if      ( skill_table[sn].target == 0 )
        ...
        {obj->value[0] = ch->spl[5]/4;sprintf(col,"black");}
    else
    {
...
void spell_engrave( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if      ( skill_table[sn].target == 0 )
        ...
        {obj->value[0] = (ch->spl[5]+1)/4;sprintf(col,"black");}
    else
    {
	send_to_char( "Oh dear...big bug...please inform KaVir.\n\r", ch );
	return;
...
void spell_preserve( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	IS_SET(obj->extra_flags, ITEM_VANISH) || IS_SET(obj->quest, QUEST_ARTIFACT))
    {
...
void spell_jail( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (IS_NPC(ch) || !IS_DEMON(ch)) return;
