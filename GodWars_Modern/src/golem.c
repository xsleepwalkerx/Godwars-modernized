...
/* -----------------------------------------------------------------------
 * Golem compatibility stubs for Godwars-Modernized
 * ----------------------------------------------------------------------- */

/* nodamcap / xpdiv: in MR these are globals; define local versions here */
bool nodamcap = FALSE;
static double xpdiv_local = 1.0;
#define xpdiv xpdiv_local

/* Simple helpers that replace MR's fancy string-centering utilities */
static char *bash_color(char *str)
{
    /* Strip ANSI/MR colour codes – return same string (display only) */
    return str;
}
static int len_bash_color(char *str)
{
    return (int)strlen(str);
}
static char *centerjob(const char *border, const char *text, int width)
{
    static char buf[256];
    (void)border; (void)width;
    snprintf(buf, sizeof(buf), "%s\n\r", text);
    return buf;
}
static char *name_funny(const char *text, int a, int b)
{
    (void)a; (void)b;
    return (char *)text;
}

/* Stub for find_kingdom – Godwars-Modernized has no kingdom system */
static void *find_kingdom(int k)
{
    (void)k;
    return NULL;
}

/* granite_struct – allocate / free GRANITE_DATA nodes */
typedef enum { sCreate, sFree } structure_dowhat;
GRANITE_DATA *granite_struct(GRANITE_DATA *hold, structure_dowhat what)
{
    if (what == sCreate)
    {
        GRANITE_DATA *d;
        d = (GRANITE_DATA *)alloc_perm(sizeof(GRANITE_DATA));
        d->vnum = 0;
        d->name = str_dup("");
        d->next = NULL;
        return d;
    ...
    else /* sFree */
    {
        if (hold)
        {
            ...
            free_mem(hold, sizeof(GRANITE_DATA));
        }
        return NULL;
    }
}

/* do_rocktalk stub – golem class channel (full implementation TBD) */
void do_rocktalk(CHAR_DATA *ch, char *argument)
{
    ...
    sprintf(buf, "[Rocktalk] %s: %s\n\r", ch->name, argument);
    for (vch = char_list; vch; vch = vch->next)
    {
        if (!IS_NPC(vch) && (IS_GOLEM(vch) || IS_IMMORTAL(vch)))
...
#define IS_MOVE(ch, move) (ch)->golem_moves & (move)
#define SET_MOVE(ch, move) (ch)->golem_moves |= (move)
...
emerald_convert(ch);}
 else {ch->golem_moves = 0;SET_MOVE(ch,EMERALD_MOVE1);}
...
void do_backslash(CHAR_DATA *ch, char *argument)
{
 ...
 {ch->golem_moves = 0;SET_MOVE(ch,OBSIDIAN_MOVE1);}
...
if (IS_MOVE(ch,OBSIDIAN_MOVE1))
 ...
 {ch->golem_moves = 0; SET_MOVE(ch, CRYSTAL_MOVE1);}
...
if (count > 50)
{
 ...
 {
    stc("Please choose out of:\r\nsolid granite create/list/project/delete\n\r", ch);
    return;
 }
 }
...
sprintf(buf,"&MWelcome to your portal chambers, %s %s. This room is covered\r\n&Min granite and has mystical entrances to anywhere.\n\r",
golem_ranks[ch->generation], ch->name);
...
/* cost = EXP(ch,cost); -- EXP scaling not available in this codebase */
...
void do_hitch(CHAR_DATA *ch, char *argument)
{
 ...
 if (IS_NPC(vict) && find_kingdom(vict->clan))
 {
    stc("Not on a kingdom guard you fool.", ch);
    hurt_person(ch,ch,ch->hit+10, FALSE);
    return;
 }
...
void do_modify(CHAR_DATA *ch, char *argument)
{
 ...
 {
    stc("Choose out of the commands 'ditto' and 'normal' (ditto to make\r\ntheir texture(clan) the same as yours, normal to make their texture\r\nrogue.\n\r", ch);
    return;
 }
...
bi("visor") ? 11411 : bi("emerald") ? 11412 : bi("obsidian") ? 11413 : 0;
 ...
 if (!vnum)
 {
    stc("Please choose out of: a ring, collar, breastplate, helmet,\r\nleggings, boots, gauntlets, sleeves, cape, belt, bracer, visor,\r\nemerald(sword) or obsidian(sword).\n\r", ch);
    return;
 }
