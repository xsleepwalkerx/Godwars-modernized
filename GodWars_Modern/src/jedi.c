...
#ifndef DPOWER_DROWSHIELD
#define DPOWER_DROWSHIELD   0   /* Drow shield power — not implemented in this codebase */
#endif
...
void do_jtouch( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
		...
		else if ((touchdamage > 0) && (touchdamage < 101))
		{
			...
			sprintf(buf,"$n lashes out with the force at $N [%d]\n\r",touchdamage);
			act(buf,ch,NULL,victim,TO_NOTVICT);
			hurt_person(ch, victim, touchdamage, FALSE);
...
void do_jsense( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_NPC(victim))
		...
		sprintf(buf, "%s's Stats : ", victim->name);
    send_to_char(buf,ch);
...
void do_jbutterfly( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
		...
		else if (ch->pcdata->stats[JEDI_COMBO_CURR] == 7)
		{
			...
			act(buf,ch,NULL,victim,TO_NOTVICT);
			hurt_person(ch, victim, (dam/2), FALSE);
//			victim->hit -= (dam/2);
			victim->move -= dam;
