...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
	...
	&&   (IS_SET(obj->quest, QUEST_ARTIFACT)) )
        {
            sprintf(buf, "%s has 2 artifacts.", ch->name);
            log_string(buf);
    	}
...
*/
	...
	if ( !can_drop_obj( ch, obj ) )
	{
        ...
        /* Trying to fix the prob of people stealing and sacing eq using bags - Xentoth */

        if ( IS_SET(obj->quest, QUEST_ARTIFACT) || ( obj->questowner != NULL && 
	strlen(obj->questowner) > 1 && str_cmp(ch->name,obj->questowner)))
        {
	    send_to_char( "It won't go in.\n\r", ch );
	    return;
        }
	...
	{
	    obj_next = obj->next_content;
/* COMMENTED BY MISTIF ---- FIX FOR PUT ALL BAG CRASH BUG
	    if (hundred_objects(ch, container)) return;
*/
	    if ( ( arg1[3] == '\0' || is_name( &arg1[4], obj->name ) )
	    &&   can_see_obj( ch, obj )
...
void do_give( CHAR_DATA *ch, char *argument )
{
    ...
    /* MobProg: give trigger on mob receiving object */
    if ( IS_NPC(victim) )
        mprog_give_trigger( victim, ch, obj );
    /* ObjProg: get trigger from receiver's perspective */
    oprog_get_trigger( victim, obj );
    do_autosave(ch,"");
    do_autosave(victim,"");
    return;
}
...
*/
		...
		&& obj->item_type != ITEM_PTOKEN)
	{
        ...
        if (obj->item_type == ITEM_PTOKEN && ch->practice >= 100)
	{
	    send_to_char("Burn some primal first.\n\r",ch);
	    return;
        }

	if ( !IS_NPC(ch) && ch->pcdata->condition[COND_FULL] > 50 &&
	    obj->item_type != ITEM_TRASH && obj->item_type != ITEM_QUEST &&
	    obj->item_type != ITEM_PILL && obj->item_type != ITEM_PTOKEN)
	{
...
*/
	...
	break;
    case ITEM_PTOKEN:
	if ( !IS_NPC(ch) ) ch->practice += obj->value[0];
	break;
...
void do_wear( CHAR_DATA *ch, char *argument )
{
	...
	!IS_VAMPAFF(ch,VAM_DISGUISED) && !IS_SET(ch->act, PLR_WOLFMAN) && !IS_FAE(ch) && !IS_KOE(ch))
    {
	...
	for ( obj = ch->carrying; obj != NULL; obj = obj_next )
	{
        ...
        /* Demon armor pain in the ass bug -Xentoth */ 
        if (!IS_NPC(ch) && ch->pcdata->atm > 75)
        {
             ch->pcdata->atm = 75;
        }
	return;
...
/* Bloodagony bug fix -- Aristoi */
    if ( obj->wear_loc == WEAR_WIELD ) REMOVE_BIT(ch->itemaffect, ITEMA_RIGHT_AGG);
            else if ( obj->wear_loc == WEAR_HOLD ) REMOVE_BIT(ch->itemaffect, ITEMA_LEFT_AGG);
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !CAN_WEAR(obj, ITEM_TAKE) || obj->item_type == ITEM_QUEST ||
	 ...
	 obj->item_type == ITEM_QUESTCARD || IS_SET(obj->quest, QUEST_ARTIFACT) ||
         IS_SET(obj->extra_flags, ITEM_VANISH) || obj->item_type == ITEM_PTOKEN ||
    ...
    if (expgain > 500)
	expgain = 500;
    if ((IS_ANGEL(ch) || IS_DEMON(ch) || IS_NEPHANDI(ch) || 
         IS_BLACK_SPIRAL_DANCER(ch) || IS_BAALI(ch)) && 
    IS_SET(obj->spectype, SITEM_DEMONIC) && obj->item_type != ITEM_PAGE)
    {
...
/*  Claimed EQ Sac fix.  **Aristoi & Mistif***/
...
void do_steal( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( ch->level + number_range(5,30) < victim->level          )
    ||   ( !IS_NPC(ch) && !IS_NPC(victim) && ch->level < 3         )
    ||   ( !IS_NPC(ch) && !IS_NPC(victim) && victim->level < 3     )
    ||   ( victim->position == POS_FIGHTING                        )
    ||   ( !IS_NPC(victim) && IS_IMMUNE(victim,IMM_STEAL) && !IS_NINJA(ch))
...
CHAR_DATA *find_keeper( CHAR_DATA *ch )
{
    ...
    {
	strcpy( buf, "Sorry, The shop is closed, please come back later." );
	do_say( keeper, buf );
	return NULL;
    }
    ...
    {
	strcpy( buf, "Sorry, The shop is closed. Come back tomorrow." );
	do_say( keeper, buf );
	return NULL;
    }
...
/* New artifact additions -Aristoi */
	if ( IS_SET(ch->in_room->room_flags, ROOM_VNUM_ARTIFACTS) )
	{
	   if (IS_SET(ch->itemaffect, ITEMA_ARTIFACT))
	   {
		send_to_char("Why would you want 2 artifacts?\n\r", ch);
		return;
	   }
...
void do_sell( CHAR_DATA *ch, char *argument )
{
    ...
    if ((obj->pIndexData->vnum == 29662)
    ||  (obj->pIndexData->vnum == 29663)
    ||  (obj->pIndexData->vnum == 29664)
    ||  (obj->pIndexData->vnum == 29665)
    ||  (obj->pIndexData->vnum == 30037)
    ||  (IS_SET(obj->quest, QUEST_RELIC)))
    {	
	act("$n says 'Oh look,  We've got a prankster!", keeper, obj, ch, TO_VICT);
	act("$n says 'And for trying to cheat me,  you shall be punished!.", keeper, obj, ch, TO_VICT);
	...
	if (ch->max_mana > 2500)
        ...
        if (ch->max_move > 2500)
           ch->max_move -= 1500;
	sprintf(buf, "#G%s #Rjust tried to cheat a shopkeeper,  and is stunned at MS!#n", ch->name);
	do_info(ch, buf);
	return;
    }
    if ( obj->item_type == ITEM_PILL )
    {
	act( "$n tells you 'I do not want your homemade pills!", keeper, obj, ch, TO_VICT );
        return;
    }
    if ( obj->item_type == ITEM_POTION )
    {
        act( "$n tells you 'I do not want your homemade potions!", keeper, obj, ch, TO_VICT );
        return;
    }
    if ( obj->item_type == ITEM_WAND )
    {
        act( "$n tells you 'I do not want your homemade wands!", keeper, obj, ch, TO_VICT );
        return;
    }
...
void quest_object( CHAR_DATA *ch, OBJ_DATA *obj )
{
    ...
    if (object < 1 || object > 156) object = 0;
    ...
    while ((obj->value[2] == obj->value[1]) || (obj->value[2] == obj->value[0]));
    do
    {
    object = number_range(obj->level, obj->level + 100);
    if (object < 1 || object > 156) object = 0;
    ...
    while ((obj->value[3] == obj->value[2]) || (obj->value[3] == obj->value[1]) || (obj->value[3] == obj->value[0]));
    return;
...
void do_questcard( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "That is not a real quest item.\n\r", ch );
        return;
    }
    if (qobj->value[0] != -1)
	{pObjIndex = get_obj_index( qobj->value[0] );
