...
void do_propose( CHAR_DATA *ch, char *argument )
{
    ...
    if ( (ch->sex == SEX_MALE && victim->sex == SEX_FEMALE) ||
	 (ch->sex == SEX_FEMALE && victim->sex == SEX_MALE) ||
         (ch->sex == SEX_FEMALE && victim->sex == SEX_FEMALE) ||
         (ch->sex == SEX_MALE && victim->sex == SEX_MALE) )
    {
...
void do_accept( CHAR_DATA *ch, char *argument )
{
	...
	REMOVE_BIT(ch->extra, EXTRA_MARRIED);
...
void do_quest( CHAR_DATA *ch, char *argument )
{
    	...
    	else if ( arg2[0] == '\0' )
	{
    	...
    	else if (!str_cmp(arg2,"notepaper" )) {add = ITEM_NOTEPAPER;     value = 10;}
	else
	{
	...
	if (ch->pcdata->quest < value)
	{
        ...
        if (add == ITEM_NOTEPAPER)
        {
           obj = create_object(get_obj_index(OBJ_VNUM_NOTEPAPER), 0);
           send_to_char("You create a notepaper\n\r",ch);
           obj_to_char(obj,ch);
           return;
        }
...
/*    if ( !str_cmp( arg2, "clone" ) )
	...
	{
	    sprintf(buf,"It will cost you %d quest points to clone
%s.\n\r",
...
void oset_affect( CHAR_DATA *ch, OBJ_DATA *obj, int value, int affect, bool is_quest )
{
    ...
    if (IS_SET(obj->spectype, SITEM_DEMONIC))
    {
	if (obj->specpower == ARM_PURPLE  ) max += 100;
	if (obj->specpower == ARM_RED) max += 250;
	if (obj->specpower == ARM_BLACK   ) max += 750;
	if (obj->specpower == ARM_MIDNIGHT ) max += 1500;
    }
