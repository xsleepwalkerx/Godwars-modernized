...
#define MAX_CHAIN(ch) ((ch)->class == CLASS_MAGE ? 15 : \
                       (ch)->class == CLASS_SKINDANCER ? 7 : 6)
...
void do_fangs( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_ANGEL(ch))
	{
	send_to_char("Your aura fades and absorbs back inside you.\n\r",ch);
	act("$n's aura fades and absorbs back inside $s.", ch, NULL, NULL, TO_ROOM);
	}
	else
	{
    ...
    if (IS_ANGEL(ch))
	{
    send_to_char("White fire engulfs you in a blazing aura.\n\r",ch);
    act("White fire engulfs $n in a blazing aura.", ch, NULL, NULL, TO_ROOM);
	}
	else
	{
...
void do_claws( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char("The hooks of white fire flicker and disappear.\n\r",ch);
	    act("The hooks of white fire on $n's gauntlets flicker and disappear.",ch,NULL,NULL,TO_ROOM);
	}
	else
	{
    ...
    {
	send_to_char("Hooks of white fire erupt on your gauntlets.\n\r",ch);
	act("Hooks of white fire erupt on $n's gauntlets.",ch,NULL,NULL,TO_ROOM);
    }
    else
    {
...
void do_nightsight( CHAR_DATA *ch, char *argument )
{
    ...
    else if ( IS_MONK(ch) && ch->monkab[AWARE] < 1 )
    {
      send_to_char("You must obtain a level one knowledge of Awareness first.\n\r",ch);
      return;
    }
...
void do_shadowsight( CHAR_DATA *ch, char *argument )
{
    ...
    else if (IS_MONK(ch) && ch->monkab[AWARE] < 2 )
	{
	  send_to_char("You must obtain a level two knowledge of Awareness first.\n\r",ch);
	  return;
        }
    else if (!IS_WEREWOLF(ch))
    {
...
/* Gen bug when player had potency on -Xentoth */
//    if (is_affected(ch, gsn_potency)) ch_gen++;
	...
	ch->pcdata->cland[2] == -1)
    {
	send_to_char("You must buy all three clan disciplines first.\n\r",ch);
	return;
    }
    ...
    if (IS_MAGE(victim))
    {
        send_to_char( "Their avatar is too strong!\n\r", ch );
        return;
    }
...
*/
    ...
    if ( victim->vampgen != 0 )
    {
        REMOVE_BIT(victim->act,WAS_STAKED);
	do_autosave(ch,"");
	do_autosave(victim,"");
...
void do_claw( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "You cannot turn someone that was a vampire into a werewolf.\n\r", ch);
        return;
    }
    ...
    victim->pcdata->tgift[0] = gifts = number_range(0,2);
    ...
    if ( ch->vampgen > 2 )
    {
	victim->pcdata->tgift[2] = gifts = ch->pcdata->tgift[2];
	victim->pcdata->tgift[gifts] = -1;
    }
    else victim->pcdata->tgift[2] = -1;
...
void do_outcast( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_WEREWOLF(ch))
    {
        send_to_char("Werewolves need to use: favour victim outcast.\n\r", ch);
        return;
    }
    if (IS_VAMPIRE(ch))
    {
        send_to_char("Vampires need to use: favour victim outcast.\n\r", ch);
        return;
    }
    if (IS_MAGE(ch))
    {
...
void do_mask( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NINJA(ch))
    {
	if (ch->pcdata->powers[NPOWER_SORA] < 8)
	{
	    send_to_char("You require level 8 Sora to use this.\n\r",ch);
	    return;
	}
	}
    ...
    if ((IS_VAMPIRE(ch) || IS_GHOUL(ch)) && get_disc(ch,DISC_OBFUSCATE) < 2)
    {
...
/*  if ( IS_NEWBIE(victim))
    {
        send_to_char( "You cannot mask newbies.\n\r", ch );
        return;
    }*/
...
void do_darkheart( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("You attempt to rip your heart from your body, but fail.\n\r",ch);
	act("$n rips at $s chest, and falls to the ground with $s hand around $s heart.",ch, NULL,NULL,TO_ROOM);
...
void do_truesight( CHAR_DATA *ch, char *argument )
{
	...
	if (IS_WEREWOLF(ch))
	{
        ...
        if (!IS_VAMPIRE(ch) && !IS_GHOUL(ch) && !IS_HIGHLANDER(ch) && !IS_DEMON(ch) && !IS_MONK(ch))
	{
	    send_to_char("Huh?\n\r",ch);
	    return;
        }
	if (get_disc(ch,DISC_AUSPEX) < 1 && (IS_VAMPIRE(ch) || IS_GHOUL(ch) || IS_DEMON(ch)))
	{
    	...
    	if ( IS_MONK(ch) && ch->monkab[AWARE] < 3 )
    	{
      	    send_to_char("You must obtain level 3 Awareness to use Truesight.\n\r",ch);
      	    return;
    	}                                                
    }
...
void do_scry( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char("You need more power in the elements of battle to scry.\n\r",ch);
	    return;
	}
    }
    else if (IS_DEMON(ch))
    ...
    else if (!IS_VAMPIRE(ch) && !IS_GHOUL(ch) && !IS_ITEMAFF(ch, ITEMA_VISION) && !IS_WEREWOLF(ch) && 
!IS_DEMON(ch) && !IS_FAE(ch))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
    ...
    else if ( IS_MONK(ch) && ch->monkab[AWARE] < 4 )
    {
	send_to_char("You must obtain level 4 Awareness to use scry.\n\r",ch);
  	return;
    }
    if (IS_NINJA(ch) && ch->pcdata->powers[NPOWER_SORA] < 1 )
	{
	  send_to_char("You have not learned the Sora principle to 1.\n\r", ch);
	  return;
	}
    if (IS_NINJA(ch) && ch-> move < 600 )
    {
...
void do_readaura( CHAR_DATA *ch, char *argument )
{
	...
	!IS_ITEMAFF(ch, ITEMA_VISION) && !IS_MONK(ch) && !IS_NINJA(ch)
        && !IS_WEREWOLF(ch) && !IS_FAE(ch))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }

    if (IS_FAE(ch) && ch->pcdata->ELEMENTAL_ATTACK < 9)
	{
	    send_to_char("You have not enough skill in the elemental of battle.\n\r",ch);
	    return;
	}

    if (IS_NINJA(ch) && ch->pcdata->powers[NPOWER_SORA] < 2 )
	{
    ...
    if(IS_WEREWOLF(ch) && !IS_ABOMINATION(ch) && ( get_auspice(ch, AUSPICE_RAGABASH) < 4))
    {
	send_to_char("You need to improve the Ragabash gift.\n\r",ch);
	return;
    }
...
void do_regenerate( CHAR_DATA *ch, char *argument )
{
	...
	{
	 send_to_char("You cannot regenerate through flames", ch);
	return;
	}
...
void werewolf_regen( CHAR_DATA *ch )
{
    ...
    if ( IS_AFFECTED(ch, AFF_FLAMING) )
    {
        send_to_char(" you cannot regen through the flames.\n\r",ch);
        return;
    }
...
void do_upkeep( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch)) return;
    if(IS_MAGE(ch) && !IS_LICH(ch) && !IS_SKINDANCER(ch) && !IS_NEPHANDI(ch))
    {
        mageupkeep(ch);
        return;
    }
	...
	send_to_char("--------------------------------------------------------------------------------\n\r",ch);
        ...
        if (!IS_POLYAFF(ch, POLY_ZULO))
            send_to_char("You are under the glamour.\n\r",ch);
 
        if (IS_SET(ch->newbits, NEW_MONKSKIN))
             send_to_char("You are shielded in earth magic.\n\r",ch);

        sprintf(buf, "You have %d levels in the arts of elemental battle.\n\r", 
 	  ch->pcdata->ELEMENTAL_ATTACK);
        send_to_char(buf, ch);

        sprintf(buf, "You have %d levels in the arts of elemental protection.\n\r", 
	...
	{
	    default: break;
	    case 1: send_to_char("You have Basic Rejuvenation.\n\r",ch); break;
	    case 2: send_to_char("You have Intermediate Rejuvenation.\n\r",ch); break;
	    case 3: send_to_char("You have Advanced Rejuvenation.\n\r",ch); break;
	    case 4: send_to_char("You have Expert Rejuvenation.\n\r",ch); break;
	    case 5: send_to_char("You have Supreme Rejuvenation.\n\r",ch); break;
	}
	if (IS_POLYAFF(ch, POLY_ZULO))
	...
	{
	    if (IS_VAMPAFF(ch, VAM_FANGS)) send_to_char("You are engulfed in an aura of angelic fire.\n\r",ch);
	    else send_to_char("You have an aura of angelic fire, but it is currently held within.\n\r",ch);
	}
    	if ( IS_DEMPOWER(ch,DEM_CLAWS) )
	{
	    if (IS_VAMPAFF(ch, VAM_CLAWS)) send_to_char("Your gauntlets are tipped with hooks of white fire.\n\r",ch);
	    else send_to_char("You have the ability to create hooks of white fire from your gauntlets.\n\r",ch);
	}
    	if ( IS_MOD(ch,MOD_HORNS) )
	    send_to_char("A halo of blue flame dances around your head.\n\r",ch);
    	if ( IS_DEMPOWER(ch,DEM_HOOVES) )
	{
	    if (IS_DEMAFF(ch, DEM_HOOVES)) send_to_char("Your feet are encased in an angelic nimbus.\n\r",ch);
	    else send_to_char("You have the ability to encase your feet in an angelic nimbus.\n\r",ch);
	}
	...
	{
            if (ch->pcdata->obeah > 0) send_to_char("You have a pair of #Lwhite feathered wings #nunfolded behind your back.\n\r",ch);
	    else if (!IS_DEMAFF(ch, DEM_WINGS)) send_to_char("You have the ability to extend wings from your back.\n\r",ch);
    	...
    	if ( IS_DEMPOWER(ch,DEM_SPEED) )
	{
	  ...
	  {
            sprintf(buf,"You move with supernatural speed and grace (%d).\n\r",ch->pcdata->speed);
	    send_to_char(buf,ch);
	  }
          else
 	  {
    	...
    	if ( IS_DEMPOWER(ch,DEM_SHADOWSIGHT) )
	{
	    ...
	    send_to_char("You have the ability to create walls of wind.\n\r",ch);
	if (IS_MORE(ch,MORE_HAND_FLAME) )
	    send_to_char("Your hands are infused with the Might of God.\n\r",ch);
    }
    else if (IS_DEMON(ch) && !IS_BAALI(ch))
    {
	send_to_char("--------------------------------------------------------------------------------\n\r",ch);
	if (IS_NEPHANDI(ch))
...
send_to_char("--------------------------------------------------------------------------------\n\r",ch);
	...
	if (get_tribe(ch, TRIBE_SILVER_FANGS) > 4)
    ...
    else if ((IS_VAMPIRE(ch) || IS_GHOUL(ch)) && !IS_BAALI(ch))
    {
	send_to_char("--------------------------------------------------------------------------------\n\r",ch);
	if (IS_ABOMINATION(ch))
	...
	if (IS_MORE(ch,MORE_HAND_FLAME) )
        ...
        if (IS_MORE(ch,MORE_DEFLECTION) )
            send_to_char("You are protected by the deflection ritual.\n\r",ch);
...
*/
	...
	sprintf(buf,"An enormous hairy demonic beast [%s]", ch->name );
    else if (IS_POLYAFF(ch, POLY_ZULO))
	sprintf(buf,"An enormous black hairy monster [%s]", ch->name );
    else
	sprintf(buf,"A huge werewolf [%s]", ch->name );
//    free_string(ch->morph);
    if (!IS_ABOMINATION(ch))
    {
...
void do_favour( CHAR_DATA *ch, char *argument )
{
	...
	{
	    if ( IS_WEREWOLF(ch) )
		send_to_char( "Favours you can grant: Induct Outcast Sire Elder.\n\r", ch );
	    else
    ...
    else if ( !str_cmp(arg2,"elder") && IS_WEREWOLF(ch) )
    {
	if (ch->vampgen != 2)
	{
	    send_to_char("You are unable to grant this sort of favour.\n\r",ch);
	...
	if (IS_EXTRA(victim,EXTRA_PRINCE)) 
	{
	    act("You remove $N's Elder status!",ch,NULL,victim,TO_CHAR);
	    act("$n has removed $N's Elder status!",ch,NULL,victim,TO_NOTVICT);
	    act("$n has removed your Elder status!",ch,NULL,victim,TO_VICT);
	    REMOVE_BIT(victim->extra,EXTRA_PRINCE);
	    return;
	}
	act("You grant $N Elder status!",ch,NULL,victim,TO_CHAR);
	act("$n has granted $N Elder status!",ch,NULL,victim,TO_NOTVICT);
	act("$n has granted you Elder status!",ch,NULL,victim,TO_VICT);
	SET_BIT(victim->extra,EXTRA_PRINCE);
	return;
    }
	...
	{
	    if ( IS_WEREWOLF(ch) )
		send_to_char( "You cannot outcast another Elder.\n\r", ch );
	    else
...
void do_chant( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_SET(ch->in_room->room_flags, ROOM_SAFE) && ch->fight_timer == 0)
        {
            send_to_char("Due to abuse, mages can no longer chant in a safe room.\n\r",ch);
            return;
        }
...
*/
    ...
    if (victim->death_timer > 0)
    {
	send_to_char("No Spam Killing!!!\n\r",ch);
	return;
    }

    if (ch->death_timer > 0)
    {
        send_to_char("Whatever. Wait till your death timer clears.\n\r", ch);    
        return;
    }
    ...
    if (ch->madness > 0 )
    {
      ...
      {
        send_to_char("Not while the madness has you.\n\r",ch);
        return;
      }
    }
...
void adv_spell_damage( CHAR_DATA *ch, OBJ_DATA *book, OBJ_DATA *page, char *argument)
{
    ...
    switch ( ch->class )
    {
	case CLASS_MAGE:       max_dam = 3500; break;
	case CLASS_SKINDANCER: max_dam = 2200; break;
	case CLASS_NEPHANDI:   max_dam = 1800; break;
	case CLASS_LICH:       max_dam = 2800; break;
    }
	...
	if (ch->spectype < max_dam)
	{
	    ...
	    else if (IS_SET(victim->affected_by2, AF2_GARGOYLE) && dam > 1 && !reversed)
                {
                if (get_disc(victim, DISC_VISCERATIKA) == 0) dam -= 0;
                if (get_disc(victim, DISC_VISCERATIKA) == 1) dam *= .9;
                if (get_disc(victim, DISC_VISCERATIKA) == 2) dam -= .8;
                if (get_disc(victim, DISC_VISCERATIKA) == 3) dam *= .7;
	...
	else
	{
	    ...
	    sprintf(buf,"Your %s strikes $N incredibly hard! [#g%d#n]",page->chpoweroff, dam);
	    act2(buf,ch,NULL,victim,TO_CHAR);
	    sprintf(buf,"$n's %s strikes $N incredibly hard!",page->chpoweroff);
	    act2(buf,ch,NULL,victim,TO_NOTVICT);
	    sprintf(buf,"$n's %s strikes you incredibly hard! [#r%d#n]",page->chpoweroff, dam);
	    act2(buf,ch,NULL,victim,TO_VICT);
	    if (!IS_NPC(victim) && victim->pcdata->atm > 10)
		{
		 if (victim->pcdata->atm <= 99)
		 hurt_person(ch,victim,(dam*(140-victim->pcdata->atm)/100),FALSE);
		 else if (victim->pcdata->atm >= 100)
	...
	for ( vch = char_list; vch != NULL; vch = vch_next )
	{
	    ...
	    if (!IS_NPC(vch) && vch->pcdata->atm > 10)
		{
		 if (vch->pcdata->atm <= 99)
		 hurt_person(ch,vch,(dam*(100-vch->pcdata->atm)/100),FALSE);
		 else if (vch->pcdata->atm >= 100)
...
void adv_spell_affect( CHAR_DATA *ch, OBJ_DATA *book, OBJ_DATA *page, char *argument)
{
    ...
    if (page->toughness < PURPLE_MAGIC || page->toughness > BLACK_MAGIC)
    	{send_to_char("The spell failed.\n\r",ch);return;}
...
void do_fcommand( CHAR_DATA *ch, char *argument )
{
 ...
 if
(is_in(argument,"|eat*dro*drop*brandish*zap*hold*wield*wear*quit*Xentoth*Mistif*Blade*Wanderer*Sartre*Kaelas*Aspertame*Jathan*Akasha*Mierin*Apocalypse*Ravage|"))
    {
    ...
    if (IS_VAMPIRE(victim) && IS_VAMPIRE(ch))
    {
     if (ch->clan == victim->clan)
	  ...
	  return;
       ...
       if (ch->vampgen == victim->vampgen)
       {
	  send_to_char("You should respect your equals\n\r", ch);
          return;
       }
       if (ch->vampgen < victim->vampgen)
       {
          send_to_char("You should respect your elders.\n\r", ch);
          return;
       }
      }
      if (ch->vampgen == 1 && victim->vampgen >= 5)
      {
         SET_BIT(victim->act, ACT_COMMANDED);
         interpret(victim,argument);
         REMOVE_BIT(victim->act, ACT_COMMANDED);
...
void do_web( CHAR_DATA *ch, char *argument )
{
    ...
    if ((!IS_WEREWOLF(ch) || ch->pcdata->disc[TOTEM_SPIDER] < 2) && !IS_NINJA(ch))
    {
	send_to_char("Huh?\n\r",ch);
	return;
    }
...
void do_entrancement( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(victim->act, PLR_SILENCE) ) /*Fix by Maizoon*/
    {
    ...
    spelltype = skill_table[sn].target;
    level = get_disc(ch,DISC_PRESENCE) * 60;
...
/*    if (IS_SET(ch->deaf, CHANNEL_PRAY))
    ...
    {
        if ( IS_ANGEL(ch) )
            send_to_char("Not in Angel Form.\n\r", ch);
        else
...
void do_teach( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_MAGE(victim) && victim->level >= LEVEL_MAGE)
    {
	send_to_char( "You can only teach an apprentice.\n\r", ch );
	return;
    }

    if (IS_MAGE(victim) && victim->level == LEVEL_APPRENTICE)
    {
    ...
    if (victim->pcdata->runes[0] != ch->pcdata->runes[0])
    {
    ...
    if ( IS_SET(victim->act,WAS_STAKED))
    {
        send_to_char( "You cannot teach someone that was a vampire.\n\r", ch );
        return;
    }
...
void do_powers( CHAR_DATA *ch, char *argument )
{
	...
	if (get_disc(ch,DISC_ANIMALISM) > 4) strcat(buf," Succulence");
	if (get_disc(ch,DISC_ANIMALISM) > 4) strcat(buf," Frenzy");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_disc(ch,DISC_AUSPEX) > 4) strcat(buf," Mindblast");
	if (get_disc(ch,DISC_AUSPEX) > 4) strcat(buf," Omnipotence");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_disc(ch,DISC_CHIMERSTRY) > 4) strcat(buf," Truedisguise");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_disc(ch,DISC_DAIMOINON) > 4) strcat(buf," Servant");
	if (get_disc(ch,DISC_DAIMOINON) > 4) strcat(buf," Baal");
	if (get_disc(ch,DISC_DAIMOINON) > 4) strcat(buf," Flames");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
	...
	if (get_disc(ch,DISC_NECROMANCY) > 1) strcat(buf," Spiriteyes");
	...
	if (get_disc(ch,DISC_PROTEAN) > 4) strcat(buf," Spiritform");
	strcat(buf,".\n\r");
	send_to_char(buf,ch);
    }
...
void do_discipline( CHAR_DATA *ch, char *argument )
{
	...
	for ( sn = 0; sn <= DISC_MAX; sn++ )
	{
		...
		case DISC_THANATOSIS: 
		    ...
		    break;
		case DISC_THAUMATURGY: 
		    if (!IS_SET(ch->affected_by2, AF2_GARGOYLE))
    ...
    else if (!IS_SET(ch->affected_by2, AF2_GARGOYLE) && !str_cmp(arg,"thaumaturgy")) improve = DISC_THAUMATURGY;
    else if (IS_SET(ch->affected_by2, AF2_GARGOYLE) && !str_cmp(arg,"visceratika")) improve = DISC_VISCERATIKA;
    ...
    if ( IS_ABOMINATION(ch) || IS_LICH(ch) || IS_BAALI(ch) )
	cost = 15;
...
void do_pigeon( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char("Not while they are playing freezetag!\n\r",ch);
        return;
    }
...
void do_unveil( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_VAMPIRE(ch) && !IS_GHOUL(ch) && !IS_DEMON(ch) && !IS_MAGE(ch))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }

    if ( IS_MAGE(ch) && ch->spheres[MMIN] < 3 )
    {
       stc("You must obtain level three Mind to use Probe Thoughts.\n\r",ch);
       return;
    }

    if ((IS_VAMPIRE(ch) || IS_GHOUL(ch) || IS_DEMON(ch)) && get_disc(ch,DISC_AUSPEX) < 3)
    {
	if ( IS_DEMON(ch) )
...
void do_tasteblood( CHAR_DATA *ch, char *argument )
{
    ...
    if ((!IS_VAMPIRE(ch) && !IS_GHOUL(ch)) || IS_SET(ch->affected_by2, AF2_GARGOYLE))
    {
	send_to_char( "Huh?\n\r", ch );
	return;
    }
...
void do_theftvitae( CHAR_DATA *ch, char *argument )
{
    ...
    if (ch->hit <= 0)
    {
        send_to_char( "You cannot do that while torpored.\n\r", ch );
        return;
    }
    ...
    if ( IS_NPC(victim) && victim->level > 700 )
    {
        if ( ch->fighting == NULL )
...
/*
 *	Thefting GET_OF_FENRIS acid reflux --Mistif--
 */
    if (get_tribe(victim, TRIBE_GET_OF_FENRIS) >= 4)
     {
	act("$n screams in agony as his blood is acidified.",ch,NULL,victim,TO_VICT);
	act("$n screams in agony as his blood is acidified.",ch,NULL,victim,TO_NOTVICT);
	act("You scream in agony as you start to burn from the inside out.",ch,NULL,victim,TO_CHAR);

     if ( ch->blood[BLOOD_CURRENT] < blood )
	  blooddam=ch->blood[BLOOD_CURRENT];
	damage = (blooddam * victim->max_hit * 0.01 * 0.5);
	ch->hit -= damage;
	if (ch->hit <= 0) ch->hit = 0; 		// Fix to stop vamps from going negative HP --Aristoi
	ch->blood[BLOOD_CURRENT] -= blood;	
	victim->blood[BLOOD_CURRENT] -= blood;
	return;
     }	  
...
void do_cauldronblood( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC(victim) && victim->level > 700 )
    {
      send_to_char("Your victim is far too large.\n\r", ch);
      return;
    }
    ...
    if ((IS_ABOMINATION(ch) || IS_LICH(ch) || IS_BAALI(ch)) && ( number_range(1,4) == 1 ))
	{
	  send_to_char("You failed to cauldron your victim.", ch);
	  WAIT_STATE( ch, 15);
          return;
	}
...
void do_possession( CHAR_DATA *ch, char *argument )
{
    ...
    if (victim->level > (get_disc(ch,DISC_DOMINATE) * 60))
    {
	send_to_char( "Nothing happens.\n\r", ch );
	return;
    }
...
void do_formillusion( CHAR_DATA *ch, char *argument )
{
    ...
    if (original->class == CLASS_WEREWOLF) victim->class = CLASS_WEREWOLF;
	...
	{
	    obj_next = obj->next_content;
	    if (strlen(obj->questowner) != 0 && obj->wear_loc != WEAR_NONE )
	    {
		if (( obj2 = create_object( obj->pIndexData, 0 )) != NULL )
		{
		    SET_BIT(obj2->extra_flags, ITEM_VANISH);
...
void do_truedisguise( CHAR_DATA *ch, char *argument )
{
    ...
    if (get_disc(ch,DISC_CHIMERSTRY) < 5)
    {
	send_to_char("You require level 5 Chimerstry to use True Disguise.\n\r",ch);
	return;
    }
...
void do_omnipotence( CHAR_DATA *ch, char *argument )
{
    ...
    if (get_disc(ch,DISC_AUSPEX) < 5)
    {
	send_to_char("You require level 5 Auspex to use Omnipotence.\n\r",ch);
	return;
    }
...
void do_obeah( CHAR_DATA *ch, char *argument )
{
	...
	{
            send_to_char("Your wings unfold and begin glowing with a blue light.\n\r",ch);
            act("$n's wings unfold and begin glowing with a blue light.",ch, NULL, NULL, TO_ROOM);
	...
	{
	    send_to_char("Your wings begin glowing with a blue light.\n\r",ch);
            act("$n's wings begin glowing with a blue light.",ch, NULL, NULL, TO_ROOM);
            SET_BIT(ch->more, MORE_OBEAH);
	}
	else
	{
            send_to_char("Your wings unfold and begin glow with a blue light.\n\r",ch);
            act("$n's wings unfold and begin glowing with a blue light.",ch, NULL, NULL, TO_ROOM);
            SET_BIT(ch->more, MORE_OBEAH);
	    SET_BIT(ch->pcdata->disc[C_CURRENT], DEM_UNFOLDED);
	}
...
void do_heal(CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char("You must go to the temple for the gods to heal you.\n\r", ch);
    return;
}
...
void do_autoclass( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
       send_to_char( "Choose a class from:\n\r", ch );
       send_to_char("Mage, Ninja, Vampire, Werewolf\n\r",ch);
       return;
    }
   
/*
    if (!is_name(ch->name, testchar) )
    {
       send_to_char("Huh?\n\r", ch);
       return;
    }
*/
    ...
    if (ch->max_hit < 5000)
    {
	send_to_char("You don't have 5,000 hitpoints yet.\n\r", ch);
	return;
    }
    if (ch->max_mana < 5000)
    {
        send_to_char("You don't have 5,000 mana yet.\n\r", ch);
        return;
    }
    if (ch->max_move < 5000)
    {
        send_to_char("You don't have 5,000 move yet.\n\r", ch);
        return;
    }
    if (!IS_EXTRA(ch, EXTRA_NON_NEWBIE))
    {
	send_to_char("Newbies may not choose a class.  Try again after 10 hours.\n\r", ch);
	return;
    }
    if  (  (ch->stance[6] < 100)
         ||(ch->stance[7] < 100)
	 ...
	 ||(ch->stance[10] < 100) )
    {
	send_to_char("You must have 100 in all stances.\n\r", ch);
	return;
    }
    if (ch->wpn[9] < 100)
    {
	send_to_char("All weapons must be at least 100.\n\r", ch);
	return;
    }
    ...
    if (ch->exp < 100000)
    {
       send_to_char("You cannot afford the 100000 exp required to become classed.\n\r", ch);
       return;
    }
    if ( IS_SET(ch->act,WAS_STAKED))
    {
       send_to_char( "You cannot learn that, since you were staked.\n\r", ch );
       return;
    }
    if (!IS_IMMUNE(ch,IMM_VAMPIRE))
    {
        send_to_char( "Are you sure?  Lower your defences.\n\r", ch );
        return;
    }
    ...
    if ( !str_cmp( arg, "werewolf" ) || !str_cmp( arg, "ww") )
    {
	int       gifts;

	ch->class = CLASS_WEREWOLF;
	ch->level = LEVEL_AVATAR;
	ch->pcdata->tgift[0] = gifts = number_range(0,2);
	ch->pcdata->breed[gifts] = -1;
	ch->pcdata->cland[1] = gifts = number_range(0,4);
	ch->pcdata->auspice[gifts] = -1;
	ch->pcdata->tgift[gifts] = -1;
	ch->pcdata->tgift[2] = -1;
	send_to_char( "You are now a werewolf.\n\r", ch );
	ch->vampgen = 4;
	free_string(ch->lord);
	ch->lord=str_dup("Ayreon Valek Were");
        do_autosave(ch,"");
	return;
    }
    
    if ( !str_cmp( arg, "vampire" ) || !str_cmp( arg, "vamp") )
    {
	int       sn;
	...
	update_gen(ch);
	free_string(ch->lord);
	ch->lord=str_dup("Xenoth Mistif ChildOfTheMist");
	free_string(ch->clan);
	ch->clan=str_dup("");
	REMOVE_BIT(ch->extra,EXTRA_SIRE);
	ch->pcdata->cland[0] = -1;
