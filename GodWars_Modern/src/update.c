...
/* claw and bite bug bandaid  --Aristoi 
   if ( (ch->position == POS_FIGHTING) && (IS_VAMPAFF(ch,VAM_FANGS)) )
   {
	REMOVE_BIT(ch->vampaff, VAM_FANGS);
	SET_BIT(ch->vampaff, VAM_FANGS);
   }
   if ( (ch->position == POS_FIGHTING) && (IS_VAMPAFF(ch, VAM_CLAWS)) )
   {
	REMOVE_BIT(ch->vampaff, VAM_CLAWS);
	SET_BIT(ch->vampaff, VAM_CLAWS);
   }
 End claw and bite bug bandaid --Aristoi */
...
/*
 *Commented by Mistif: This is thought to be a bug.  It was coded to do exactly
 *what it does.  I am commenting this out for future "if I want it backs"
 *
 *			    ch->race++;
 *			    vch->race--;
 */
...
/* Fix for Skindancer not coming out of rage --Aristoi  */
	    if (IS_SKINDANCER(ch))
	    {
                if (ch->position == POS_STANDING && ch->pcdata->wolf > 0) 
		{
		   ch->pcdata->wolf -= number_range(1,10);
			if (ch->pcdata->wolf < 0) ch->pcdata->wolf = 0;
		   return;
		}
                if (ch->position == POS_FIGHTING )
                {
                    if (ch->pcdata->wolf < 300 && !IS_ITEMAFF(ch, ITEMA_RAGER))
                    {
		...
		if ( ch->quint[BODY] < (ch->spheres[MPRI]) )
                ...
                if (ch->pcdata->wolf > 0) werewolf_regen(ch);
...
/*  Mage hybrids couldn't get quint with this in place -- Aristoi
&& !IS_DEMON(ch) && !IS_WEREWOLF(ch) && !IS_VAMPIRE(ch) */   )
            {
            if ( ch->quint[BODY] < (ch->spheres[MPRI]) )
                 ch->quint[BODY]++;
            if ( ch->quint[AVATAR] < (ch->spheres[MPRI]) )
...
/*            else if (IS_MAGE(ch) && !IS_DEMON(ch) && !IS_WEREWOLF(ch) &&
			!IS_VAMPIRE(ch))
            {
                 if (ch->pcdata->power[AVATAR] < 50)
                     ch->pcdata->power[AVATAR]++;
            }
*/
	    else if (IS_FAE(ch))
	    {
		...
		werewolf_regen(ch);
            ...
            else if ((IS_KOE(ch)) || (IS_NINJA(ch) && (!str_cmp(ch->clan, "Nightstalkers"))))
            {
                 if ( ch->position == POS_FIGHTING)
                 ...
                 if ( ch->pcdata->power[NINJAKI] < 300 )
                   {
                      ch->pcdata->power[NINJAKI]++;
                 ...
                 if (IS_KOE(ch) && ch->pcdata->powers[NINJAKI] < 600)
                   {                                                 
...
/*                  if (!IS_KOE(ch) && ch->pcdata->power[NINJAKI] > 300 )
                       ch->pcdata->power[NINJAKI] = 300;*/             
                ...
                if( number_range(1,3) == 1 || ch->position == POS_MEDITATING )
                {
                   if (ch->loc_hp[6] > 0)
                        {
                            int sn = skill_lookup( "clot" );
                ...
                else werewolf_regen(ch);
            }   

            else if (IS_NINJA(ch) || (IS_KOE(ch)))
            {
                 if ( ch->position == POS_FIGHTING)
                    REMOVE_BIT(ch->affected_by, AFF_NINJAHIDE);
		 ...
		 if ( ch->pcdata->power[NINJAKI] < 300 )
                 ...
                 if ( ch->position == POS_FIGHTING && ch->pcdata->wolf > 0
                  ...
                  else if (ch->pcdata->wolf >= 0)
		  {
                    ch->pcdata->wolf--;
                  }
		if( number_range(1,5) == 1 || ch->position == POS_MEDITATING )
                {
                   if (ch->loc_hp[6] > 0)
                        {
                            int sn = skill_lookup( "clot" );
		...
		if (IS_ANGEL(ch))
		{
			   ...
			   act( "A white aura forms around $n and puts out the flames.", ch, NULL, NULL, TO_ROOM );
                        }
			ch->agg = 0;
			ch->blood[BLOOD_CURRENT] = ch->blood[BLOOD_POOL];
			werewolf_regen(ch);
	...
	/* MobProg: delay trigger */
	if ( IS_NPC(ch) && ch->mprog_delay > 0 )
	{
	    ch->mprog_delay--;
	    if ( ch->mprog_delay == 0 )
	    {
		mprog_delay_trigger( ch );
		if ( ch->in_room == NULL ) continue;
	    }
	}

	/* MobProg: random trigger */
	if ( IS_NPC(ch) && ch->in_room != NULL &&
	     IS_SET(ch->pIndexData->progtypes, TRIG_RANDOM) )
 ...
 */
    ...
    if ((time_info.hour == 2 || 18 ) && (IS_SET(world_affects, WORLD_MIST)) && (number_range(1,20) == 4))
    {
            ...
            for ( d = descriptor_list; d != NULL; d = d->next )
            {
	            ...
	            &&  IS_OUTSIDE(ch))
  		    send_to_char("#7The Mist disperses#n.\n\r",ch);
            }
    }

    if ((time_info.hour == 8 || 12 || 23) && (!IS_SET(world_affects, WORLD_MIST)) && (number_range(1,190) == 31))
    {
	   SET_BIT(world_affects, WORLD_MIST);
           for ( d = descriptor_list; d != NULL; d = d->next )
           {
               if ( d->connected == CON_PLAYING
	            ...
	            &&  ch->in_room->sector_type != SECT_INSIDE
	            &&  IS_OUTSIDE(ch)) 
		    send_to_char("#CA thick #7Mist #Crises from the ground#n.\n\r",ch);
           }
    }

/*    if ((time_info.hour == 1 || 5 || 10 || 15 || 19 ) && (number_range(1,10) ==2))
    {
        if (IS_SET(world_affects, WORLD_MIST))
        {
            ...
            for ( d = descriptor_list; d != NULL; d = d->next )
            {
	            ...
	            &&  ch->in_room->sector_type != SECT_INSIDE
	            &&  IS_OUTSIDE(ch))
				send_to_char("The Mist disperses.\n\r",ch);
            }
         }
         else
         {
...
*/	
    ...
    case 12:
        ...
        {
        REMOVE_BIT(world_affects, WORLD_MIST);
        strcat(buf, "#7The Mist disperses#n.\n\r");
        }
        break;
	...
	for ( d = descriptor_list; d != NULL; d = d->next )
	{
		...
		if ( IS_VAMPIRE(ch) && (!IS_ABOMINATION(ch) || !IS_LICH(ch) || !IS_BAALI(ch)))
		{
...
*/
	if ( !IS_NPC(ch) && ch->level < LEVEL_IMMORTAL && !is_obj)
	{
	   ...
	   /* if (!IS_SET(ch->in_room->room_flags, ROOM_SAFE) 
            ||   IS_IMMORTAL(ch)
	    ||   IS_SET(ch->tag_flags,GAME_ON)
	    ||   IS_SET(ch->act,IN_CRIB)
	    ||   ch->in_room->vnum == ROOM_VNUM_LIMBO ) 
            {
                ch->pcdata->safetimer = 0;
            }
            if ( ++ch->pcdata->safetimer >= 5 && ch->hit >= ch->max_hit )
            {
                ch->pcdata->safetimer = 0;
                ch->was_in_room = ch->in_room;
		ch->position = POS_STANDING;
                send_to_char( "You have been thrust out of the safe room!\n\r", ch );
                act( "$n has been thrust from the safe room!", ch, NULL, NULL, TO_ROOM );
                do_autosave( ch, "" );
                char_from_room( ch );
                char_to_room( ch, get_room_index( 3014 ) );
            }*/
	    if ( ++ch->pcdata->imtimer >= 5 

            && IS_ITEMAFF(ch, ITEMA_PEACE) )
	    {
		act("$n no longer seems so protected from harm", ch, NULL, NULL, TO_ROOM ); 
...
*/
	...
	if ( IS_VAMPIRE(ch) && (!IS_AFFECTED(ch,AFF_SHADOWPLANE)) &&
	    (!IS_NPC(ch) && !IS_IMMUNE(ch,IMM_SUNLIGHT)) && 
        (!IS_VAMPAFF(ch,VAM_EARTHMELDED))  && (!IS_SET(world_affects, WORLD_MIST)) 
		&& ch->in_room != NULL &&
	    ...
	    (!room_is_dark(ch->in_room)) && (weather_info.sunlight != SUN_DARK) )
	{
	   ...
	   /** This one's to keep Zarkas quiet ;) **/
	    if (IS_POLYAFF(ch, POLY_SERPENT))
	    	ch->hit = ch->hit - number_range(20,55);
	    else
	    	ch->hit = ch->hit - number_range(35,150);
		    if (!IS_AFFECTED(ch, AFF_FLAMING))
		      {
		          if ( number_percent() > 70 )
			    {
			    act( "$n's screams in agony as $s flesh bursts into flames!", ch, NULL, NULL, TO_ROOM );
			    send_to_char( "You scream in agony as your flesh bursts into flames!\n\r", ch );
	...
	{
	    if ((IS_IMMORTAL(ch)) || (IS_SET(ch->more, MORE_MARAUDER) && ch->paradox[1] > 0))
	    {
	    ch->paradox[1] = 0;
	    ch->paradox[2] = 0;
	    ch->paradox[3] = 0;
...
//	cptimer_check();
	...
	if (ch->pcdata->pain > 0)
        {
            ...
            {
               REMOVE_BIT(ch->more, MORE_NOPAIN);
	       send_to_char("You no longer feel the angels strength.\n\r",ch);
	       return;
            }
        } 
	...
	if (ch->save_timer >= 500)
	{
	ch->save_timer = 0;
	do_autosave(ch,"");
	}

	/* Timed Double by Maizoon */
		...
		if (dxp_info.time > 1)
		{
                ...
                if (dxp_info.time <= 1)
		 {
		   dxp_info.time = 0;
		   dxp_info.dxp = 0;
		   sprintf(buf,"Double Exp has been deactivated\n\r");
		   do_info(ch,buf);
		 }
		}
		...
		if (ch->pcdata->obeah > 0 && !IS_MORE(ch, MORE_NEUTRAL))
		{
			...
			{
			    send_to_char("The blue glow fades from your wings.\n\r",ch);
			    act("The blue glow in $n's wings fades.",ch,NULL,NULL,TO_ROOM);
	...
	if ( pulse_mobile   <= 0 )
	{
	    ...
	    /* RoomProg: random trigger — scan all loaded rooms */
	    {
		int hash;
		for ( hash = 0; hash < MAX_KEY_HASH; hash++ )
		{
		    ...
		    {
			if ( room->people != NULL &&
			     IS_SET(room->progtypes, TRIG_ROOM_RAND) )
			    rprog_random_trigger( room );
		    }
		}
	    }
	}
...
void paradox( CHAR_DATA *ch )
{
    ...
    send_to_char("You feel the air charge with electricity.\n\r",  ch);
    send_to_char("The paradox has come for your soul!\n\r",  ch);
    sprintf( buf, "#r%s#n has been electrified by a paradox.\n\r", ch->name);
    do_info(ch, buf);
    ch->paradox[1] = 0;
    ch->paradox[2] = 0;
