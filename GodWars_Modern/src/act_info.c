...
int get_disc( CHAR_DATA *ch, int disc )
{
    ...
    ||    IS_LICH(ch))
//    ||    IS_BAALI(ch))
    &&   (disc == ch->pcdata->cland[0]
    ||    disc == ch->pcdata->cland[1]
    ||    disc == ch->pcdata->cland[2])))
    	max = 5;
...
int char_hitroll( CHAR_DATA *ch )
{
	...
	if (IS_NINJA(ch) && ch->pcdata->powers[NPOWER_CHIKYU] >= 6
	&& ch->pcdata->powers[HARA_KIRI] > 0)
	hr += 100;
	if (IS_NINJA(ch) && ch->pcdata->wolf > 0)
	    hr += ch->pcdata->wolf;
...
int char_damroll( CHAR_DATA *ch )
{
	...
	else if (IS_NINJA(ch) && ch->pcdata->wolf > 0)
	    dr += ch->pcdata->wolf;
...
/* Freeze tag */
    if ( IS_SET(victim->tag_flags,TAG_RED) ) strcat( buf,"#R[RED]#n "  );
    if ( IS_SET(victim->tag_flags,TAG_BLUE)) strcat( buf,"#L[BLUE]#n" );
    if ( IS_SET(victim->tag_flags,TAG_FROZEN)) strcat( buf, "#7[FROZEN]#n " );
    if ( IS_EXTRA(victim, EXTRA_AFK)) strcat( buf,"#L(#cAFK#L)#n " );
    if ( IS_AFFECTED(victim, AFF_NINJAHIDE)) strcat( buf,"#r(#0Kak#r)#n " );
/* end freeze tag */
	...
	if ( !IS_NPC(ch) && IS_WEREWOLF(ch) && 
	   (IS_ITEMAFF(victim, ITEMA_LEFT_SILVER) ||
	    ...
	    && IS_SET(victim->affected_by2, AF2_GARGOYLE)) strcat( buf, "(Stony) " );  
	if (!IS_NPC(victim) && IS_FAE(victim) && IS_POLYAFF(victim, POLY_ZULO))
	    strcat( buf, "(Feral) " );
        if (!IS_NPC(victim) && IS_KOE(victim)) strcat( buf, "(Kue-Jin) " );
    }
	...
	switch ( victim->class )
	{
	    ...
	    case CLASS_NINJA:                  strcat( buf, "(Ninja) " ); break;
            case CLASS_MONK:                   strcat( buf, "(Monk) " ); break;
    ...
    {
        if ( IS_NPC(victim))
          sprintf( buf4, "\n\r...%s is surrounding by earth.", victim->short_descr);
        else if ( !IS_NPC(victim) && IS_AFFECTED(victim,AFF_POLYMORPH))
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    else if (IS_FAE(victim) && IS_VAMPAFF(victim, VAM_NIGHTSIGHT)) act ("$N's eyes hold the secrets of forest glades.",ch,NULL,victim,TO_CHAR);
	...
	{
	  if (IS_ANGEL(victim))
	act("$N is engulfed in an aura of angelic fire.",ch,NULL,victim,TO_CHAR);
	else
	...
	{
	    if ( IS_ANGEL(victim) )
		act("$N's gauntlets have hooks of angelic fire",ch,NULL,victim,TO_CHAR);
	    else
	...
	else
	{
	    ...
	    else if (IS_POLYAFF(victim, POLY_ZULO) && !IS_ANGEL(victim) && !IS_FAE(victim))
		act("$N has long black talons extending from $S fingers.",ch,NULL,victim,TO_CHAR);
	    else if ( IS_ANGEL(victim) )
		act("$N's gauntlets have hooks of angelic fire",ch,NULL,victim,TO_CHAR);
            else if (IS_FAE(victim) )
                act("$N's fingers have glittering talons on the tips.",ch,NULL,victim,TO_CHAR);
	    else
		act("$N has razor sharp talons extending from $S fingers.",ch,NULL,victim,TO_CHAR);
	}
    }
    if (!IS_NPC(victim) && IS_SET(victim->affected_by2, AF2_GARGOYLE) && get_disc(victim, DISC_VISCERATIKA > 0))
    {
	act("$N's skin is stony with rock like protrusions.",ch,NULL,victim,TO_CHAR);
    ...
    {
	if ((victim->pcdata->obeah > 0) && IS_VAMPIRE(victim))
	    act("$N has a brightly glowing eye in the centre of $S forehead.",ch,NULL,victim,TO_CHAR);
	else if ((victim->pcdata->obeah > 0) && IS_ANGEL(victim))
	    act("$N's wings are glowing light blue.",ch,NULL,victim,TO_CHAR);
	else
	...
	{
	    if (IS_ANGEL(victim))
	    act("$N has a halo of blue flame dancing around $s head.",ch,NULL,victim,TO_CHAR);
	    else if(IS_FAE(victim))
		act("$N has a pair of glittering transparent horns.",ch,NULL,victim,TO_CHAR);
            else 
	...
	else if (IS_MOD(victim, MOD_WINGS))
	{
		...
		if ( IS_ANGEL(victim) )
		    ...
		    act("$N has a pair of large glittering transparent wings out behind $S back.",ch,NULL,victim, TO_CHAR);
		else
		    act("$N has a pair of batlike wings spread out behind $S back.",ch,NULL,victim,TO_CHAR);
	    }
	    else
		...
		if ( IS_ANGEL(victim) )
		    ...
		    act("$N has a pair of glittering wings folded behind $S back.",ch,NULL,victim,TO_CHAR);
		else
		    act("$N has a pair of batlike wings folded behind $S back.",ch,NULL,victim,TO_CHAR);
	    }
	}
...
void do_look( CHAR_DATA *ch, char *argument )
{
	...
	if (!IS_VAMPIRE(ch) || get_disc(ch,DISC_OBTENEBRATION) == 0)
	{
		...
		{
                    if (!(IS_NINJA(ch) && (!str_cmp(ch->clan, "Nightstalkers"))))
                    {
    ...
    if (!IS_IMMORTAL(ch) && IS_EXTRA(ch, BLINDFOLDED))
    {
        send_to_char( "It is pitch black.....\n\r", ch);
        return;
    }

    if (!IS_IMMORTAL(ch) && ch->in_room != NULL  && IS_SET(world_affects, WORLD_MIST) 
       && (IS_OUTSIDE(ch)))
    {
        switch( number_range(1,10))
		{
		default:
		   send_to_char( "The Mist flows and boils around you.\n\r", ch);
                   show_char_to_char( ch->in_room->people, ch );
		   break;
		case 1:
		   send_to_char( "You can hardly see through the thick white Mists.\n\r", ch);
            	   show_char_to_char( ch->in_room->people, ch );
		   break;
		case 2:
		   send_to_char( "You are surrounded by Mists.\n\r", ch);
            	   show_char_to_char( ch->in_room->people, ch );
		   break;
		case 3:
		case 4:
		   send_to_char( "The Mists pulsate... they feel alive.\n\r", ch);
            	   show_char_to_char( ch->in_room->people, ch );
		   break;
		case 5:
		case 6:
		case 7:
		   send_to_char( "You stumble through the Mists.\n\r", ch);
            	   show_char_to_char( ch->in_room->people, ch );
		   break;
		case 8:
		case 9:
		   send_to_char( "You are enveloped in a swirling Mist.\n\r", ch);
            	   show_char_to_char( ch->in_room->people, ch );
		   break;
		}
        return;    
...
//	    send_to_char( ch->in_room->description, ch );
	...
	case ITEM_PORTAL:
		...
		return;
            ...
            if ( obj->item_type == ITEM_WGATE && !IS_MAGE(ch) )
            {
...
void do_exits( CHAR_DATA *ch, char *argument )
{
	...
	&&   !IS_SET(pexit->exit_info, EX_CLOSED) )
	{
	    ...
	    {
		if (IS_SET(world_affects, WORLD_MIST) && (IS_OUTSIDE(ch)))
                {
                    sprintf( buf + strlen(buf), "#y%-5s #r-#n The Mist is too thick to tell.\n\r",
                    capitalize( dir_name[door] ) );
                }
                else if (IS_SET(world_affects, WORLD_FOG) && (IS_OUTSIDE(ch)))
                {
                    sprintf( buf + strlen(buf), "#y%-5s #r-#n Too foggy to tell.\n\r",
                    capitalize( dir_name[door] ) );
                }
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( buf, ch );
    send_to_char("#l-----------------------------------------------------------------------#n\n\r",ch);
    sprintf( buf, "Character Created: %s\n\r", ch->createtime );
    send_to_char( buf, ch );
    send_to_char("#l-----------------------------------------------------------------------#n\n\r",ch);
    ...
    sprintf( buf,
        "Gold: %d       Carrying: %d/%d       Weight: %d/%d\n\r", 
    ch->gold, ch->carry_number, can_carry_n (ch), ch->carry_weight, can_carry_w (ch) );
    send_to_char( buf, ch);
	...
	if ( IS_NINJA(ch) )
	{
	    sprintf(buf,"You currently have %d out of 300 Ki stored.\n\r",ch->pcdata->power[NINJAKI] );
            send_to_char(buf,ch);
        }
        if ( IS_KOE(ch) )
        {
            sprintf(buf,"You currently have %d out of 600 Chi stored.\n\r", ch->pcdata->powers[NINJAKI] );
            send_to_char(buf,ch);
        }
        if ( IS_KOE(ch) && ch->pcdata->wolf > 0) 
        {
	    sprintf(buf,"Your inner demon is in control: Affects Hitroll and Damroll by +%d.\n\r",ch->pcdata->wolf);
            send_to_char( buf, ch );
	}
	...
	send_to_char(buf,ch);
        ...
        if (ch->fight_timer > 0)
        {
        sprintf(buf,"You have %d seconds left on your fight timer.\n\r", ch->fight_timer);
        send_to_char(buf,ch);
        }
        if (ch->death_timer > 0)
        {
        sprintf(buf,"You have %d seconds left on your death timer.\n\r",ch->death_timer);
        send_to_char(buf,ch);
        }
    ...
    {
        sprintf( buf, "You have %d seconds of supernatural speed remaining\n\r",ch->pcdata->speed );
        send_to_char(buf,ch);
    }
...
void do_time( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf( buf,
	"It is %d o'clock %s, Day of %s, %d%s the Month of %s.\n\rEthereal Mist started up at %s\rThe system time is %s\r",
...
void do_weather( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(world_affects, WORLD_MIST) )
        send_to_char( "A thick mist surrounds you.\n\r", ch );
...
//	else if(!str_cmp(wch->name, "Chaos")) class = "#PGoddess#n      ";
	...
	case MAX_LEVEL -  5:/* class = "#cBuilder#n      "; break;*/
             if(!str_cmp(wch->name, "Gortoris")) class = "#GP#gunk #yA#orse#n    ";
	     else class = "#cBuilder#n      "; break;
	case MAX_LEVEL -  6:
	case MAX_LEVEL -  7:
	case MAX_LEVEL -  8:
	case MAX_LEVEL -  9:
                     if (wch->race <= 0 ) class = "#rAvatar#n       ";
                else if (wch->race <= 4 ) class = "#2Immortal#n     ";
                else if (wch->race <= 9 ) class = "#3Godling#n      ";
                else if (wch->race <= 14) class = "#4Demigod#n      ";
                else if (wch->race <= 19) class = "#5Lesser God#n   ";
                else if (wch->race <= 24) class = "#6Greater God#n  ";
                else class = "#7Supreme God#n  ";

            if (IS_VAMPIRE(wch) && !IS_SET(wch->affected_by2, AF2_GARGOYLE))
            {
                     if (wch->race <= 0 ) class = "#rNeonate#n      ";
                else if (wch->race <= 4 ) class = "#2Childe#n       ";
                else if (wch->race <= 9 ) class = "#3Ancilla#n      ";
                else if (wch->race <= 14) class = "#4Elder#n        ";
                else if (wch->race <= 19) class = "#5Methuselah#n   ";
                else if (wch->race <= 24) class = "#6Ancient Wyrm#n ";
                else class = "#7Caine#n        ";
            }
	   
	    if (IS_VAMPIRE(wch) && IS_SET(wch->affected_by2, AF2_GARGOYLE))
	    {
		     if (wch->race <= 0 ) class = "#rWarden#n       ";
		else if (wch->race <= 4 ) class = "#2Watchkeeper#n  ";
		else if (wch->race <= 9 ) class = "#3Guardian#n     ";
		else if (wch->race <= 14) class = "#4Gatekeeper#n   ";
		else if (wch->race <= 19) class = "#5Sentinel#n     ";
		else if (wch->race <= 24) class = "#6Vigilante#n    ";
		else class = "#7Cerberus#n     ";
	    }

            if (IS_WEREWOLF(wch))
            {
                     if (wch->race <= 0 ) class = "#rPup#n          ";
                else if (wch->race <= 4 ) class = "#2Lesser Garou#n ";
                else if (wch->race <= 9 ) class = "#3Greater Garou#n";
                else if (wch->race <= 14) class = "#4Tribal Shaman#n";
                else if (wch->race <= 19) class = "#5Tribal Chief#n ";
                else if (wch->race <= 24) class = "#6Fenris Wolf#n  ";
                else class = "#7Gaia#n         ";
            }

            if (IS_MAGE(wch) && !IS_SET(wch->more, MORE_MARAUDER ))
            {
                     if (wch->race <= 0 ) class = "#rAvatar#n       ";
                else if (wch->race <= 4 ) class = "#2Apprentice#n   ";
                else if (wch->race <= 9 ) class = "#3Conjurer#n     ";
                else if (wch->race <= 14) class = "#4Mage#n         ";
                else if (wch->race <= 19) class = "#5ArchMage#n     ";
                else if (wch->race <= 24) class = "#6Wizard#n       ";
                else class = "#7Supreme Mage#n ";
            }

	    if (IS_MAGE(wch) && IS_SET(wch->more, MORE_MARAUDER ))
	    {
		     if (wch->race <= 0 ) class = "#sAwakened#n     ";
		else if (wch->race <= 4 ) class = "#sDeviant#n      ";
		else if (wch->race <= 9 ) class = "#sEnlightened#n  ";
		else if (wch->race <= 14) class = "#sMarauder#n     ";
		else if (wch->race <= 19) class = "#sGrand Mage#n   ";
		else if (wch->race <= 24) class = "#sChaos Mage#n   ";
		else class = "#sMad Hatter#n   ";
	    }

            if (IS_DEMON(wch))
            {
                     if (wch->race <= 0 ) class = "#rFarsu#n        ";
                else if (wch->race <= 4 ) class = "#2Lesser Demon#n ";
                else if (wch->race <= 9 ) class = "#3Greater Demon#n";
                else if (wch->race <= 14) class = "#4Pit Fiend#n    ";
                else if (wch->race <= 19) class = "#5ArchDemon#n    ";
                else if (wch->race <= 24) class = "#6Devil#n        ";
                else class = "#7ArchDevil#n    ";
            }

            if (IS_ANGEL(wch))
            {
                     if (wch->race <= 0 ) class = "#rCherub#n       ";
                else if (wch->race <= 4 ) class = "#2Lesser Angel#n ";
                else if (wch->race <= 9 ) class = "#3Greater Angel#n";
                else if (wch->race <= 14) class = "#4Holy Warrior#n ";
                else if (wch->race <= 19) class = "#5Holy Knight#n  ";
                else if (wch->race <= 24) class = "#6ArchAngel#n    ";
                else class = "#7Dominion#n     ";
            ...
            {
                     if (wch->race <= 0 ) class = "#rAvatar#n       ";
                else if (wch->race <= 4 ) class = "#2Swordsman#n    ";
                else if (wch->race <= 9 ) class = "#3Bladesmaster#n ";
                else if (wch->race <= 14) class = "#4Ancient#n      ";
                else if (wch->race <= 19) class = "#5Legendary#n    ";
                else if (wch->race <= 24) class = "#6Mythical#n     ";
                else class = "#7Divine Master#n";
            }
            if (IS_NINJA(wch))
            {
                     if (wch->race <= 0 ) class = "#rRokue#n        ";
                else if (wch->race <= 4 ) class = "#2Gokue#n        ";
       	        else if (wch->race <= 9 ) class = "#3Shekue#n       ";
 	        else if (wch->race <= 14) class = "#4Sankue#n       ";
     	        else if (wch->race <= 19) class = "#5Neekue#n       ";
	        else if (wch->race <= 24) class = "#6Iechkue#n      ";
                else class = "#7Master#n       ";
            }
	    if (IS_LICH(wch))
            {
                     if (wch->race <= 0 ) class = "#rLich#n         ";
                else if (wch->race <= 4 ) class = "#2Dark Justicar#n";
                else if (wch->race <= 9 ) class = "#3Arcane#n       ";
                else if (wch->race <= 14) class = "#4Greater Lich#n ";
                else if (wch->race <= 19) class = "#5Unholy Magus#n ";
                else if (wch->race <= 24) class = "#6Necromancer#n  ";
                else class = "#7Archon#n       ";
            }
	    if (IS_NEPHANDI(wch))
            {
                     if (wch->race <= 0 ) class = "#rNephandi#n     ";
                else if (wch->race <= 4 ) class = "#2Demonic Mage#n ";
                else if (wch->race <= 9 ) class = "#3Archaic Wyrm#n ";
                else if (wch->race <= 14) class = "#4Greater Magus#n";
                else if (wch->race <= 19) class = "#5Fiendish Mage#n";
                else if (wch->race <= 24) class = "#6Spawn of Hell#n";
                else class = "#7Satan's Magus#n";
            }
	    if (IS_BLACK_SPIRAL_DANCER(wch))
            {
                     if (wch->race <= 0 ) class = "#rBSD#n          ";
                else if (wch->race <= 4 ) class = "#2Gaia's Fallen#n";
                else if (wch->race <= 9 ) class = "#3Fallen Garou#n ";
                else if (wch->race <= 14) class = "#4Demonic Beast#n";
                else if (wch->race <= 19) class = "#5Fallen One#n   ";
                else if (wch->race <= 24) class = "#6Horde Leader#n ";
                else class = "#7Gaia's Hated#n ";
            }
	    if (IS_ABOMINATION(wch))
            {
                     if (wch->race <= 0 ) class = "#rAbomination#n  ";
                else if (wch->race <= 4 ) class = "#2Gaia's Evil#n  ";
                else if (wch->race <= 9 ) class = "#3Undead Keeper#n";
                else if (wch->race <= 14) class = "#4Deaths Minion#n";
                else if (wch->race <= 19) class = "#5Wyrm Outcast#n ";
                else if (wch->race <= 24) class = "#6The Damned#n   ";
                else class = "#7Caine's Hated#n";
            }
	    if (IS_SKINDANCER(wch))
            {
                     if (wch->race <= 0 ) class = "#rSkindancer#n   ";
                else if (wch->race <= 4 ) class = "#2Arcane Wolf#n  ";
                else if (wch->race <= 9 ) class = "#3Garou Mage#n   ";
                else if (wch->race <= 14) class = "#4Gaia's Might#n ";
                else if (wch->race <= 19) class = "#5Magi Knight#n  ";
                else if (wch->race <= 24) class = "#6Fenris Mage#n  ";
                else class = "#7Arcane Fenris#n";
            }
	    if (IS_BAALI(wch))
            {
                     if (wch->race <= 0 ) class = "#rBaali#n        ";
                else if (wch->race <= 4 ) class = "#2Demon Kind#n   ";
                else if (wch->race <= 9 ) class = "#3Damned Soul#n  ";
                else if (wch->race <= 14) class = "#4Hell's Avatar#n";
                else if (wch->race <= 19) class = "#5BlackHeart#n   ";
                else if (wch->race <= 24) class = "#6Wyrm Knight#n  ";
                else class = "#7Baal's Avatar#n";
            }
            if (IS_KOE(wch))                                                                                                     
            {
                     if (wch->race <= 0 ) class = "#rKue-Jin#n      "; 
                else if (wch->race <= 4 ) class = "#2Humble Wyrm#n  "; 
                else if (wch->race <= 9 ) class = "#3Swordsman#n    "; 
                else if (wch->race <= 14) class = "#4Chi Master#n   "; 
                else if (wch->race <= 19) class = "#5Sensei#n       "; 
                else if (wch->race <= 24) class = "#6Chi Warrior#n  "; 
                else class = "#7Akuma's Might#n";                      
            }                                                          
	    if (IS_FAE(wch))
	    {
                     if (wch->race <= 0 ) class = "#rFaeborn#n      ";
                else if (wch->race <= 4 ) class = "#2Journeyman#n   ";
                else if (wch->race <= 9 ) class = "#3Wanderer#n     ";
                else if (wch->race <= 14) class = "#4Life's Child#n ";
                else if (wch->race <= 19) class = "#5Lightbringer#n ";
                else if (wch->race <= 24) class = "#6Clan of Gaian#n";
                                     else class = "#7The Hunter#n   ";
            }            

        break;
	case MAX_LEVEL - 10:
	case MAX_LEVEL - 11:
	case MAX_LEVEL - 12: class = "#oMortal#n       "; break;
	}
	/*
	 * Format it up.
	 */
	if ( IS_ABOMINATION(wch))
	{ strcpy( openb, "<(" ); strcpy( closeb, ")>#n" ); }
	else if ( IS_LICH(wch))
	{ strcpy( openb, "<{" ); strcpy( closeb, "}>#n" ); }
...
*/
	{
	    if (IS_FAE(wch))
	    {
		if (wch->sex == SEX_MALE) sprintf( kav, ". %sFae Lord%s", openb, closeb );
		else if (wch->sex == SEX_FEMALE) sprintf( kav, ". %sFae Lady%s", openb, closeb);
		else sprintf( kav, ". %sFae Noble%s",openb,closeb);
	    }
	    if (IS_ABOMINATION(wch) )
	    {
		if ( wch->vampgen <= 3 )
		...
		else if (!str_cmp(wch->lord, wch->clan) && !IS_SET(wch->act, PLR_DEMON))
		    sprintf( kav, ", %sFallen Angel%s", openb, closeb );
	    ...
	    {
		if ( IS_ACT(wch, PLR_CLASSELDER) )
                    sprintf( kav, ". %sDemonic Elder%s", openb, closeb );
		else if (!str_cmp(wch->clan, wch->lord) && !IS_SET(wch->act, PLR_DEMON))
		    sprintf( kav, ", %sTiefling%s", openb, closeb );
		else if ( IS_SET(wch->act, PLR_DEMON) )
	    ...
	    else if (IS_NINJA(wch) && wch->vampgen == 1 )
	    	sprintf( kav, ". %sMaster Ninja%s", openb, closeb );
	    else if (IS_MONK(wch) && wch->vampgen == 1 )
	    	sprintf( kav, ". %sGod's Abbot%s", openb, closeb );

	    else if (IS_VAMPIRE(wch) && wch->vampgen == 1 )
		{
	 	if(!str_cmp(wch->name, "Xentoth")) 
		sprintf( kav, ". %sThe Ancient%s", openb, closeb );
	    	else
	    ...
	    else if ( IS_VAMPIRE(wch) && !IS_SET(wch->affected_by2, AF2_GARGOYLE) )
	    {
		bool use_age = FALSE;
		char clanname[80];
		...
		if ( strlen(wch->clan) > 1 )
		{
			...
			sprintf( kav, ". %sElder of %s%s", openb,wch->clan, closeb );
		    else
			sprintf( kav, ". %sClaith of %s%s", openb,wch->clan, closeb );
		}
            ...
            {
		if (wch->vampgen == 2)
                sprintf(kav, ". %sRevolutionist%s", openb, closeb );
            else if (wch->vampgen == 3)
                sprintf(kav, ". %sCreature of Stone%s", openb, closeb );
            }
            else if ( IS_MAGE(wch) )
	    {
		char *mage_col;
		switch ( wch->pcdata->runes[0] )
		{
		    ...
		    case YELLOW_MAGIC:	mage_col = "Lightning";	break;
                    case BLACK_MAGIC:   mage_col = "Death";     break;
		}
		switch ( wch->level )
		{
		    default:
			...
			break;
                ...
                sprintf( kav, ". %sRogue Mage%s", openb, closeb );

		else if (IS_MORE(wch, MORE_MARAUDER))
		sprintf( kav, ". #7%sM#nar#0au#nde#7r%s#n", openb, closeb );
	    }
	    else if ( IS_KOE(wch) )
            {                                                             
                     if (wch->vampgen <= 1)                                   
                sprintf(kav, ". %sMaster Kue-Jin%s", openb, closeb);     
		else if (wch->vampgen == 2)                                   
     		sprintf(kav, ". %sCathayan%s", openb, closeb);           
		else if (wch->vampgen >= 3)                                   
     		sprintf(kav, ". %sKindred of the East%s", openb, closeb);
	    }
            else if ( IS_HIGHLANDER(wch) )
	    {
		    ...
		    && wch->clan != NULL && strlen(wch->clan) > 1 )
	    {
	    	if (wch->vampgen == 2)
	    	    ...
	    	    sprintf( kav, ". %s%sMonk%s", openb, wch->clan, closeb );
	    }
	    else if ( IS_MONK(wch)
		&& wch->clan != NULL && strlen(wch->clan) < 1 )
...
*/
	...
	if (wch->level > 6)
	{
        sprintf(immo + strlen(immo), "%-13s", class);
            if (IS_SET(wch->tag_flags, TAG_RED))
            sprintf(immo +strlen(immo), " #R[RED] #n   ");
            else if (IS_SET(wch->tag_flags, TAG_BLUE))
            sprintf(immo +strlen(immo), "#L[BLUE] #n   ");
	    else if (IS_SET(wch->affected_by, AFF_NINJAHIDE))
            sprintf(immo +strlen(immo), " #L(#cKAK#L)#n    ");
            else if (IS_EXTRA(wch, EXTRA_AFK))
            sprintf(immo +strlen(immo), " #L(#cAFK#L)#n    ");
  	    else if (IS_AFFECTED(wch, AFF_NINJAHIDE))
            sprintf(immo +strlen(immo), " #r(#0Kak#r)#n    ");
	    else if (IS_VAMPAFF(wch,VAM_EARTHMELDED))
	    sprintf(immo +strlen(immo), "  #L(#o^#L)#n     ");
	    else if (IS_SET(wch->act, PLR_WIZINVIS))
	    sprintf(immo +strlen(immo), "  #L(#g*#L)#n     ");
	    else if (IS_SET(wch->act, PLR_WIZINCOG))
	    sprintf(immo +strlen(immo), "  #L(#G*#L)#n     ");
	    else
	    sprintf(immo +strlen(immo), "          ");
        sprintf(immo + strlen(immo), " #0%s#n%s%s\n\r", 
	    wch->name,
	    wch->pcdata->title,
	    kav);
        imm++;
        ...
        {
        sprintf( avat + strlen(avat), "%-13s", class);
            if (IS_SET(wch->tag_flags, TAG_RED))
            sprintf(avat +strlen(avat), " #R[RED] #n   ");
            else if (IS_SET(wch->tag_flags, TAG_BLUE))
            sprintf(avat +strlen(avat), "#L[BLUE] #n   ");
            else if (IS_EXTRA(wch, EXTRA_AFK))
            sprintf(avat +strlen(avat), " #L(#cAFK#L)#n    ");
	    else if (IS_AFFECTED(wch, AFF_NINJAHIDE))
            sprintf(avat +strlen(avat), " #r(#0Kak#r)#n    ");
	    else if ((wch->pkill <= 0) && (get_hours(wch) <= 10))
            sprintf(avat +strlen(avat), " #P(#CNEWBIE#P)#n ");
            else if (wch->pcdata->bounty > 0)
            sprintf(avat +strlen(avat), " #0(#RB#ro#Ru#rn#Rt#ry#0)#n ");
            else if (IS_VAMPAFF(wch,VAM_EARTHMELDED))
            sprintf(avat +strlen(avat), "  #L(#o^#L)#n     ");
            else
            sprintf(avat +strlen(avat), "          ");
        sprintf(avat + strlen(avat), " #C%s#n%s%s\n\r",
            wch->name,
            wch->pcdata->title,
            kav);
        ava++;
        ...
        {
        sprintf( mort + strlen(mort), "%-13s", class);
            if (IS_SET(wch->tag_flags, TAG_RED))
            sprintf(mort +strlen(mort), " #R[RED] #n   ");
            else if (IS_SET(wch->tag_flags, TAG_BLUE))
            sprintf(mort +strlen(mort), "#L[BLUE] #n   ");
            else if (IS_EXTRA(wch, EXTRA_AFK))
	    sprintf(mort +strlen(mort), " #L(#cAFK#L)#n    ");
            else
            sprintf(mort +strlen(mort), "          ");
        sprintf(mort + strlen(mort), " %s%s%s\n\r",
            wch->name,
            wch->pcdata->title,
            kav);
         mor++;
...
/*
	if ( wch->level > 6 )
	{
	sprintf( immo + strlen(immo), "%-16s %-7s #0%s#n%s%s\n\r",
	    class,
            IS_SET(wch->tag_flags,TAG_RED) ? "#R[RED] #n    " :
            (IS_SET(wch->tag_flags,TAG_BLUE) ? "#L[BLUE] #n   " : ""),
	    wch->name,
	    wch->pcdata->title,
	    kav );
	imm++;
	...
	{
	sprintf( avat + strlen(avat), "%-16s %-7s #C%s#n%s%s\n\r",
	    class,
	    IS_SET(wch->tag_flags,TAG_RED) ? "#R[RED] #n    " :
            (IS_SET(wch->tag_flags,TAG_BLUE) ? "#L[BLUE] #n   " : ""),
            wch->name,
	...
	{
	sprintf( mort + strlen(mort), "%-13s %-7s %s%s%s\n\r",
	    class,
            IS_SET(wch->tag_flags,TAG_RED) ? "#R[RED] #n    " :
            (IS_SET(wch->tag_flags,TAG_BLUE) ? "#L[BLUE] #n   " : ""),
	    wch->name,
...
*/
    ...
    send_to_char( "#l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#n\n\r", ch );
    send_to_char( "                            #CE #7T #CH #7E #CR #7E #CA #7L  #CM #7I #CS #7T #CS#n\n\r", ch );
    send_to_char( "#l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#n\n\r", ch );

    if ( imm > 0 )
    {
     if ( imm > 1 )
      sprintf(buf, "#l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#c %d Immortals #l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#n\n\r",imm);
     else
      sprintf(buf, "#l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#c 1 Immortal #l=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#n\n\r");
     send_to_char( buf, ch );
    ...
    {
      if ( ava > 1 )
       sprintf(buf, "#l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#r %d Avatars #l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#n\n\r",ava);
      else
       sprintf(buf, "#l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#r One Avatar #l=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#n\n\r");
      send_to_char( buf, ch );
      send_to_char( avat, ch );
    }

    if ( mor > 0 )
    {
      if ( mor > 1 )
       sprintf(buf, "#l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#g %d Mortals #l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#n\n\r",mor);
      else
       sprintf(buf, "#l-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-#g One Mortal #l=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=#n\n\r");
      send_to_char( buf, ch );
...
if (nMatch == 1 && total == 0)
       sprintf( buf2, "#4There are a total of #u#21#n #4visible players connected.#n \n\r");
        ...
        sprintf(buf2, "#4There are a total of #u#21#n #4visible player out of #2%d #4total players connected.#n\n\r", total);
    else
       sprintf( buf2, "#4There are a total of #u#2%d#n#4 visible players.#n\n\r", nMatch );
...
*/
    ...
    {
	if ((nTotal - nMatch) < 2)
	    sprintf( buf2, "#cThere is one invisible player connected.#n\n\r");
	else
...
void do_where( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_SET(world_affects, WORLD_FOG) && (IS_OUTSIDE(ch)))
    {
    ...
    if (IS_SET(world_affects, WORLD_MIST) && (IS_OUTSIDE(ch)))
    {
        send_to_char("The Mist is too thick to locate anyone.\n\r",ch);
        return;
    }
...
void do_title( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(ch->act,IN_CRIB))
    {
        send_to_char("Not while cribbed.\n\r", ch);
        return;
    }
...
void do_description( CHAR_DATA *ch, char *argument )
{
	...
	if ( argument[0] == '+' )
	{
	    ...
	    if ( strlen2(buf)  >= MAX_STRING_LENGTH - 2 )
        {
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	if ((IS_DEMON(ch) && !IS_ANGEL(ch)) || IS_IMMORTAL(ch))
	{
	    send_to_char( !IS_SET(ch->deaf, CHANNEL_DEMON)
	    ? " +INCANT"
	    : " -incant",
	    ch );
	}

        if (IS_ANGEL(ch) || IS_IMMORTAL(ch))
        {
            send_to_char( !IS_SET(ch->deaf, CHANNEL_ANGEL)
            ? " +PRAY"
            : " -pray",
            ch );
        }
	...
	else
	{
	    ...
	    return;
	     ...
	     if ( !str_cmp( arg+1, "mage"     ) ) bit = CHANNEL_MAGETALK;
...
/*	else if ((IS_SET(ch->act, PLR_CHAMPION) || 
	    ...
	    !str_cmp( arg+1, "pray") ) bit = CHANNEL_DEMON;
*/
	else if ( !str_cmp( arg+1, "pray"     ) ) bit = CHANNEL_ANGEL;
	else if ( !str_cmp( arg+1, "incant"     ) ) bit = CHANNEL_DEMON;
...
void do_config( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char(  IS_SET(ch->act, PLR_MOBSTUN)
	    ? "[+STUN     ] You will stun players and mobiles.\n\r"
	    : "[-stun	   ] You will not stun players and mobiles.\n\r"
	    , ch );
	...
	else
	{
        ...
        else if ( !str_cmp( arg+1, "stun"     ) ) bit = PLR_MOBSTUN;
	else
	{
...
void obj_score( CHAR_DATA *ch, OBJ_DATA *obj )
{
	...
	break;
    case ITEM_PTOKEN:
	sprintf( buf, "Your primal point value is %d.\n\r", obj->value[0] );
	send_to_char( buf, ch );
	break;
...
void do_affects( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char("  \n\r",ch);
    send_to_char("You have the following shields:             \n\r",ch);
    if (IS_ITEMAFF(ch, ITEMA_FIRESHIELD))
        {
            send_to_char("You have a fireshield.\n\r", ch);
        }
    if (IS_ITEMAFF(ch, ITEMA_ICESHIELD))
        {
            send_to_char("You have an iceshield.\n\r", ch);
        }
    if (IS_ITEMAFF(ch, ITEMA_SHOCKSHIELD))
        {
            send_to_char("You have a shockshield.\n\r", ch);
        }
    if (IS_ITEMAFF(ch, ITEMA_ACIDSHIELD))
        {
           send_to_char("You have an acidshield.\n\r", ch);
        }
    if (!((IS_ITEMAFF(ch, ITEMA_ACIDSHIELD)) || (IS_ITEMAFF(ch, ITEMA_SHOCKSHIELD)) || (IS_ITEMAFF(ch,
ITEMA_ICESHIELD)) || (IS_ITEMAFF(ch, ITEMA_FIRESHIELD))))
        {
            send_to_char("You have no shields.\n\r", ch);  
        }
    return;
}
   ...
   for (letter = ch->carrying; letter; letter = letter->next_content)
   {
      if ((letter->item_type == ITEM_NOTEPAPER)
        && ((letter->wear_loc == WEAR_HOLD) ||
            (letter->wear_loc == WEAR_WIELD)))
	break;
   }

   if (letter == NULL)
   {
     send_to_char("You aren't holding a note\n\r",ch);
     return;
   }

   if (letter->value[1] != 0)
   {
     send_to_char("It's already been scribed.\n\r",ch);
     return;
   }

   if (arg[0] == '\0' )
   {
     send_to_char("What would you like to scribe?\n\r",ch);
     return;
   }

   smash_tilde(argument);
   for ( ed = letter->extra_descr; ed != NULL; ed = ed->next )
/*
...
*/
   ...
   send_to_char("You scribe your note.\n\r",ch);
   return;
}

void do_peruse(CHAR_DATA *ch, char *argument)
{
   OBJ_DATA *letter;
   ...
   send_to_char("#7The note reads:#n\n\r",ch);
...
void do_seal(CHAR_DATA *ch, char *argument)
{
   ...
   if (letter->value[2] != 0)
   {
      send_to_char("It's already sealed.\n\r",ch);
      return;
   }
...
void do_unseal(CHAR_DATA *ch, char *argument)
{
   ...
   if (letter == NULL)
   {
    ...
    if (letter->value[2] == 0)
    {
        send_to_char("It's already been unsealed.\n\r",ch);
	return;
    }
