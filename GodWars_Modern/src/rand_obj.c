...
*  Ported to GodWars Modernized — adapted affect system and exit structure *
***************************************************************************/
...
/*
 * Compatibility defines for constants that exist in Distorted Illusions /
 * ROT 1.4 but not in GodWars.  We map them to safe no-op values so the
 * tables compile unchanged.
 */
#ifndef NO_FLAG
#define NO_FLAG             0
#endif
/* Map APPLY_SAVES to the most common save type we carry */
#define APPLY_SAVES         APPLY_SAVING_SPELL
/* Regen apply — not in GodWars; maps to APPLY_NONE so the entry is skipped */
#define APPLY_REGEN         APPLY_NONE
/* ROM morph form apply — not used; maps to APPLY_NONE */
#ifndef APPLY_MORPH_FORM
#define APPLY_MORPH_FORM    APPLY_NONE
#endif
/* ROM affect bitvectors not in GodWars — map to 0 = NO_FLAG */
#define AFF_REGENERATION    0
/* MORPH_* constants referenced in min/max fields — values unused since
   their apply type resolves to APPLY_NONE */
#define MORPH_RED           0
#define MORPH_GOLD          0

/*
 * ROM/ROT resist, vulnerability, and extra affect bitvectors used in the
 * prefix/suffix tables.  These fields are stored in table structs but are
 * ignored by process_mods() — GodWars uses a different immunity model.
 * Define them all as 0 so the table initialiser compiles unchanged.
 */
/* Resistance flags (ROM TO_RESIST) */
#define RES_FIRE            0
#define RES_LIGHTNING       0
#define RES_POISON          0
#define RES_COLD            0
#define RES_ACID            0
#define RES_NEGATIVE        0
#define RES_HOLY            0
#define RES_MENTAL          0
#define RES_SOUND           0
#define RES_LIGHT           0
#define RES_DISEASE         0
#define RES_CHARM           0
/* Vulnerability flags (ROM TO_VULN) */
#define VULN_FIRE           0
#define VULN_LIGHTNING      0
#define VULN_POISON         0
#define VULN_COLD           0
#define VULN_ACID           0
#define VULN_NEGATIVE       0
#define VULN_HOLY           0
#define VULN_SOUND          0
#define VULN_MENTAL         0
#define VULN_LIGHT          0
#define VULN_DISEASE        0
#define VULN_CHARM          0
/* Extra AFF_ bitvectors (ROM, not in GodWars) */
#define AFF_CALM            0
#define AFF_PLAGUE          0
#define AFF_WEAKEN          0
#define AFF_SLOW            0
#define AFF_DETECT_MOTION   0
#define AFF_DETECT_GOOD     0
#define AFF_FARSIGHT        0
#define AFF_DARK_VISION     0
#define AFF_BERSERK         0
/* Extra APPLY_ types (ROM, not in GodWars) */
#define APPLY_MANA_REGEN    APPLY_NONE
...
*/
	...
	{ "Skyntil",	        99,             0,
        NO_FLAG,   		NO_FLAG,        NO_FLAG,
        APPLY_REGEN,    	5,              30,
        APPLY_NONE,	MORPH_RED,      MORPH_GOLD,
        APPLY_NONE,         	0,		0  },
...
/* =========================================================================
 * Local helper — allocate an AFFECT_DATA from the pool (GodWars style).
 * ========================================================================= */
static AFFECT_DATA *new_rand_affect( void )
{
    AFFECT_DATA *Af;
    if ( affect_free == NULL )
...
/* =========================================================================
 * process_mods — apply the numeric APPLY_* modifiers from a prefix or
 * suffix table entry onto the object's affect list.
 *
 * Note: The original ROM version also applied TO_AFFECTS / TO_RESIST /
 * TO_VULN bitvectors which are not part of GodWars's simpler affect_data.
 * Those fields are present in the tables but are silently ignored here.
 * ========================================================================= */
void process_mods( OBJ_DATA *rand_obj, int mod_number, bool prefix )
{
    ...
    if ( prefix )
    {
        mod1 = prefix_table[mod_number].mod_one;
        min1 = prefix_table[mod_number].min_one;
        max1 = prefix_table[mod_number].max_one;
        mod2 = prefix_table[mod_number].mod_two;
        min2 = prefix_table[mod_number].min_two;
        max2 = prefix_table[mod_number].max_two;
        mod3 = prefix_table[mod_number].mod_three;
        min3 = prefix_table[mod_number].min_three;
        max3 = prefix_table[mod_number].max_three;
    }
    else
    {
        mod1 = suffix_table[mod_number].mod_one;
        min1 = suffix_table[mod_number].min_one;
        max1 = suffix_table[mod_number].max_one;
        mod2 = suffix_table[mod_number].mod_two;
        min2 = suffix_table[mod_number].min_two;
        max2 = suffix_table[mod_number].max_two;
        mod3 = suffix_table[mod_number].mod_three;
        min3 = suffix_table[mod_number].min_three;
        max3 = suffix_table[mod_number].max_three;
    }

    if ( mod1 != APPLY_NONE )
    {
        value       = number_range( min1, max1 );
        Af          = new_rand_affect();
        Af->location = mod1;
        Af->modifier = value;
...
/* =========================================================================
 * make_cracked — degrade a weapon or armor item slightly.
 * GodWars items use value[0..3]; value[3] is the flags slot for weapons.
 * ========================================================================= */
OBJ_DATA *make_cracked( OBJ_DATA *obj )
{
    ...
    {
        obj->value[0] -= number_range( 0, 8 );  /* primary AC */
...
/* =========================================================================
 * make_exceptional — enhance a weapon or armor item and add random applies.
 * GodWars does not use ROM-style WEAPON_* bitvectors so the weapon branch
 * just adds a stat bonus via an affect instead.
 * ========================================================================= */
OBJ_DATA *make_exceptional( OBJ_DATA *obj )
{
    ...
    if ( obj->item_type == ITEM_WEAPON )
    {
        /* Give a small hitroll or damroll bonus instead of ROM weapon flags */
        apply_type = ( dice(1,2) == 1 ) ? APPLY_HITROLL : APPLY_DAMROLL;
        value      = number_range( 1, 3 );
        Af         = new_rand_affect();
        Af->location = apply_type;
        Af->modifier = value;
    ...
    {
        obj->value[0] += number_range( 0, 10 );
        obj->value[1] += number_range( 0, 10 );
        obj->value[2] += number_range( 0, 10 );
        obj->value[3] += number_range( 0, 10 );

        /* Random extra flag for armors */
        switch ( dice( 1, 4 ) )
        {
        case 1: SET_BIT( obj->extra_flags, ITEM_MAGIC  ); break;
        case 2: SET_BIT( obj->extra_flags, ITEM_HUM    ); break;
        case 3: SET_BIT( obj->extra_flags, ITEM_BLESS  ); break;
        case 4: SET_BIT( obj->extra_flags, ITEM_GLOW   ); break;
        }
    }

    /* Random bonus apply — 50% chance */
    if ( number_percent() > 50 )
    {
        rand_number = number_range( 0, MAX_EXCEP_APPLY - 1 );
    ...
    /* Second random bonus apply — 25% chance */
...
/* =========================================================================
 * do_randobj2 — immortal command to manually load a random object.
 * Syntax: randobj2 <type> [prefix] [suffix]
 *   type: unique | normal | cracked | exceptional | special | cspecial |
 *         especial | list
 * ========================================================================= */
void do_randobj2( CHAR_DATA *ch, char *argument )
{
    OBJ_DATA *obj;
    ...
    obj = rand_obj2( ch, ch->level, argument );

    if ( obj != NULL )
    {
        obj_to_char( obj, ch );
        act( "$p appears in your hands.", ch, obj, NULL, TO_CHAR );
        act( "$n conjures $p.",           ch, obj, NULL, TO_ROOM );
    }
}
