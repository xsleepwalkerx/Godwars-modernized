...
#ifndef isascii
#define isascii(c) (((unsigned)(c)) <= 0x7f) /* Merge repair: fallback for strict C builds. */
#endif
...
//int	getpeername	args( ( int s, struct sockaddr *name, int *namelen ) );
//int	getsockname	args( ( int s, struct sockaddr *name, int *namelen ) );
/* gettimeofday declared by system headers on Linux — no redeclaration needed */
int	listen		args( ( int s, int backlog ) );
...
bool		    newbielock;		/* No newbies allowed		*/
...
#if defined(macintosh) || defined(MSDOS)
    boot_db( );
    log_string( "Ethereal Mist is ready to rock." );
    game_loop_mac_msdos( );
#endif
...
#define FNDELAY O_NDELAY
    ...
    *dnew		= d_zero;
    ...
    /* Initiate telnet protocol negotiation (TTYPE, NAWS, MSDP, MSSP, MXP) */
    protocol_negotiate( dnew );
...
*/
	...
	if (ch->fighting != NULL)
	{
	sprintf(buf,"#r%s#n #ghas lost their link.\n\r",ch->name);
        do_info(ch,buf);
	}
...
bool read_from_descriptor( DESCRIPTOR_DATA *d )
{
    ...
    /* Strip telnet IAC sequences and update protocol state */
    {
        char filtered[sizeof(d->inbuf)];
        int  flen = protocol_input( d, d->inbuf, iStart,
                                    filtered, sizeof(filtered) );
        if ( flen < iStart )
        {
            memcpy( d->inbuf, filtered, flen );
            d->inbuf[flen] = '\0';
        }
    }
...
void read_from_buffer( DESCRIPTOR_DATA *d )
{
	...
	else
	{
		...
		log_string( log_buf );
wiznet(log_buf,d->character,NULL,WIZ_SPAM,0,get_trust(d->character));
		write_to_descriptor( d->descriptor,
		    "\n\r*** PUT A LID ON IT!!! ***\n\r", 0 );
...
//Color code modified by Duo
//Color code created by Xkilla
void write_to_buffer( DESCRIPTOR_DATA *d, const char *txt, int length)
{
//    char tb[MAX_STRING_LENGTH], ccode;
/* Suggested buffer overflow fix from Dystopia code -Xentoth */
    char tb[30000], ccode;
    int i,j;
	  ...
	  if(txt[i]=='#')ansi = TRUE;
            ...
            switch (txt[i])
            {
                ...
                case 'i': tb[j++] = 27; tb[j++] = '['; tb[j++] = '7';
                          tb[j++] = 'm'; break;
                case 'b': tb[j++] = 27; tb[j++] = '['; tb[j++] = '5';
tb[j++] =
'm'; break;
                case 'u': tb[j++] = 27; tb[j++] = '['; tb[j++] = '4';
tb[j++] =
'm'; break;
                case 't': if (!ansi) continue;
                  tb[j++] = '['; tb[j++] = '4'; tb[j++] = 'm';
                    break;
                case 'T': if (!ansi) continue;
                  tb[j++] = 27; tb[j++] = '['; tb[j++] = '4'; tb[j++] =
'1';
                    break;
                case 'd': if (!ansi) continue;
                  tb[j++] = 27; tb[j++] = '['; tb[j++] = '5'; tb[j++] =
'1';
                    break;
                case 'D': if (!ansi) continue;
                  tb[j++] = 27; tb[j++] = '['; tb[j++] = '6'; tb[j++] =
'1';
                    break;
                case 'N':
                case 'n':
...
';';
     ...
     */
/* Start suspected membug memory area - Aristoi */
...
/* End suspected membug memory area - Aristoi */
    /*
     * Copy.
     */
...
//	    sprintf(kav,"Last connected from %s at %s\n\r",ch->lasthost,ch->lasttime);
	    sprintf(kav,"Last connected at %s\n\r",ch->lasttime);
	...
	else if ( IS_EXTRA2(ch, EXTRA_BORN) && char_age < 15 )
	{
	...
	else
	{
            if ( newbielock )
            {
                write_to_buffer( d, "\n\r\n\rSorry,  No new characters allowed right now.\n\r\n\r\n\r", 0 );
                close_socket( d );
                return;
            }
	    /* New player */
	...
	if ( ch == NULL || (!IS_EXTRA2(ch,EXTRA_NEWPASS) &&
	...
	else if ( ch == NULL || (IS_EXTRA2(ch,EXTRA_NEWPASS) &&
...
/*
	...
	** Avoid nasty duplication bug - KaVir */
	...
	if ( !IS_EXTRA2(ch,EXTRA_NEWPASS) && strlen(argument) > 1) {
...
*/
        ...
        if ( ch->level > 8 )
        {
            write_to_buffer( d, "Do you wish to enter Wizinvis, Incognito, or Normal ( W / I / N ): ", 0 );
            d->connected = CON_GET_INVIS;
        }
	else
	{
            do_help(ch, "motd" );
    ...
    case CON_GET_INVIS:
        ...
        default:
                send_to_char("\n\r", ch );
                send_to_char("(W)izinvis     or     (I)ncognito     or     (N)ormal\n\r", ch );
            return;
        }


    case CON_READ_MOTD:
	write_to_buffer( d,     "\n\rWelcome to the Ethereal Mists.\n\r",
	    0 );
	ch->next	= char_list;
	char_list	= ch;
	d->connected	= CON_PLAYING;
...
*/
	...
	sprintf(buf,"#r%s#n #ghas entered the #6E#7t#6h#7e#6r#7e#6a#7l #6M#7i#6s#7t#6s.", ch->name);
	do_info(ch,buf);
//        wiznet(log_buf,ch,NULL,NULL,WIZ_LOGINS,get_trust(ch));
        ch->fight_timer = 0;
...
bool check_parse_name( char *name )
{
     ...
     * Reserved words.
     */
    if ( is_name( name, "all auto immortal self someone gaia imms imm imp immo imps" ) )
	return FALSE;

    if (is_in(name,"|shit**voadkin*ayreon*fuck*cunt*pussy*god*assh*bitch*suck*dick*damn*damnit*yousuck*motherfucker*bastard*self*myself|me|"))
	return FALSE;
...
void send_to_char( const char *txt, CHAR_DATA *ch )
{
	...
	{
	    SET_BIT(wizard->extra2, EXTRA_CAN_SEE);
	    send_to_char("[ ",wizard);
	    if ( txt != NULL && wizard->desc != NULL )
		write_to_buffer( wizard->desc, txt, strlen(txt) );
	    REMOVE_BIT(wizard->extra2, EXTRA_CAN_SEE);
	}
    }
...
void bust_a_prompt( DESCRIPTOR_DATA *d )
{
   ...
   /* Send MSDP variable updates before the prompt */
   if ( ch->pcdata != NULL && IS_PLAYING(d) )
       msdp_update( ch );
   if ( ch->pcdata == NULL )
   {
      send_to_char( "\n\r\n\r", ch );
...
void do_password( CHAR_DATA *ch, char *argument )
{
    ...
    *pArg = '\0';
    ...
    free_string( ch->pcdata->pwd );
    ch->pcdata->pwd = str_dup( pwdnew );
    if (!IS_EXTRA2(ch,EXTRA_NEWPASS)) SET_BIT(ch->extra2,EXTRA_NEWPASS);
    do_autosave(ch,"");
...
void mud_logf (char * fmt, ...) /* Merge repair: renamed from logf to avoid libm conflict. */
{
