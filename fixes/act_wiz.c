...
#include <time.h>
#include <stdarg.h>
#include <crypt.h> /* Merge repair: expose crypt() prototype on modern libc. */
...
/* Show wiznet options - just like channel command */
   {
	...
	IS_SET(ch->wiznet,wiznet_table[flag].flag) ? "ON\n\r" : "OFF\n\r" );
                send_to_char(buf, ch);
            }
        }
    ...
    for ( d = descriptor_list; d != NULL; d = d->next )
{
		...
		send_to_char("--> ",d->character);
                ...
                send_to_char( "", d->character );
                send_to_char( string, d->character );
...
void do_setdisc( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA   *victim;
...
void do_cfat( CHAR_DATA *ch, char *argument )
{
    if ( ch->pcdata->custom == 0)
	{
	send_to_char("You haven't been given that ability\n\r",ch);
	return;
	}
    ...
    {
	smash_tilde( argument );
	free_string( ch->pcdata->cfat );
	ch->pcdata->cfat = str_dup( argument );
	send_to_char( "Ok.\n\r", ch );
    }
    return;
}
...
void do_info( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "#5Info #4->#n ",d->character );
...
void do_goto( CHAR_DATA *ch, char *argument )
{
    ...
    if ( room_is_private( location ) && get_trust(ch) < 11 )
    {
	send_to_char( "That room is private right now.\n\r", ch );
	return;
    }
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
 ...
 /*   if ( IS_NPC(victim) )
       ...
       {
       sprintf( buf, "Lv: %d.  Align: %d.  AC: %d.  Gold: %d.  Exp: %d.  Quest: %d.\n\r",
            victim->level,	     victim->alignment,	     
            char_ac(victim),     victim->gold,	     victim->exp,     victim->pcdata->quest );
       send_to_char( buf, ch );	
       }  
*/
    ...
    send_to_char( buf, ch );
        ...
        {
    sprintf( buf,
	"Lv: %d.  Trst: %d.  Security: %d.  Align: %d.  AC: %d.  Exp: %d.  Quest: %d. \n\r",
	victim->level,       victim->trust,  IS_NPC(victim) ? 0 : GET_PC( victim, security, 0 ), 
        victim->alignment,   char_ac(victim),         victim->exp,	victim->pcdata->quest );
    send_to_char( buf, ch );
         }
    ...
    {
	sprintf( buf,
	    "Spell: '%s' %s %d for %d hours with bits %s.\n\r",
...
/*
 * do_mpstat: Display MobProg data for a mobile's vnum.
 * Usage: mpstat <mob_name>
 */
void do_mpstat( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_NPC(victim) )
    {
        send_to_char( "MobProgs only apply to NPCs.\n\r", ch );
        return;
    }

    sprintf( buf, "MobProg data for [%d] %s:\n\r",
             victim->pIndexData->vnum, victim->short_descr );
    send_to_char( buf, ch );
    sprintf( buf, "Progtypes bitmask: %d\n\r", victim->pIndexData->progtypes );
    send_to_char( buf, ch );

    for ( prg = victim->pIndexData->progs; prg; prg = prg->next, count++ )
    {
        sprintf( buf, "\n\r[%d] Trigger: %d  Phrase: %s\n\r",
                 count, prg->trig_type,
                 prg->trig_phrase ? prg->trig_phrase : "(none)" );
        send_to_char( buf, ch );
        send_to_char( "--- Program ---\n\r", ch );
        send_to_char( prg->prog_text, ch );
        send_to_char( "\n\r--- End ---\n\r", ch );
    }

    if ( count == 0 )
        send_to_char( "No programs defined.\n\r", ch );
...
/*
 * do_mprog: Manually run a MobProg on a mob with a given actor.
 * Usage: mprog <mob_name> <actor_name>
 * Fires TRIG_RANDOM prog (first one found) for testing purposes.
 */
void do_mprog( CHAR_DATA *ch, char *argument )
{
    char arg1[MAX_INPUT_LENGTH];
    char arg2[MAX_INPUT_LENGTH];
    CHAR_DATA *mob;
    ...
    if ( arg1[0] == '\0' )
    {
        send_to_char( "Usage: mprog <mob> [actor]\n\r", ch );
        return;
    }

    if ( (mob = get_char_world(ch, arg1)) == NULL || !IS_NPC(mob) )
    {
        send_to_char( "No NPC found by that name.\n\r", ch );
        return;
    }

    actor = ( arg2[0] != '\0' ) ? get_char_world(ch, arg2) : ch;
    if ( actor == NULL ) actor = ch;

    if ( mob->pIndexData->progs == NULL )
    {
        send_to_char( "That mob has no programs.\n\r", ch );
        return;
    }

    PROG_LIST *prg = mob->pIndexData->progs;
    PROG_CTX ctx   = { mob, actor, NULL, NULL, "(mprog test)", 0 };
    send_to_char( "Running first prog...\n\r", ch );
...
/* Fall command to remove angel/demon CL by Maizoon */
void do_fall( CHAR_DATA *ch, char *argument )
{
	char arg1[MAX_INPUT_LENGTH];
	char arg2[MAX_INPUT_LENGTH];
	char buf[MAX_STRING_LENGTH];
	...
	victim = get_char_world(ch, arg1);
        ...
        if (!IS_IMMORTAL(ch) && IS_SET(ch->act, PLR_DEMON))
	{
          if (IS_ANGEL(ch))
	  {
	  if (arg1[0] == '\0' )
	  {
		send_to_char("Syntax : Fallen <char>\n\r",ch);
		return;
	  }
          if (victim == NULL)
          {
          ...
          {
        	send_to_char( "You are not there Lord.\n\r", ch );
	        return;
          }
	  ...
	  REMOVE_BIT(victim->extra, EXTRA_PRINCE);
          ...
          victim->lord = str_dup(capitalize(arg1));
          victim->clan = str_dup(capitalize(arg1));
          send_to_char("They have fallen.\n\r",ch);
          send_to_char( "You have fallen.\n\r",victim);
          do_autosave(victim,"");
          return;
	}
	else
	{
	  ...
	  REMOVE_BIT(victim->extra, EXTRA_PRINCE);
          ...
          victim->lord = str_dup(capitalize(arg1));
          victim->clan = str_dup(capitalize(arg1));
          send_to_char("They have been forsaken.\n\r",ch);
	...
	if((arg1[0] =='\0' ) || (arg2[0] =='\0'))
	  {
		send_to_char("Syntax : fall <char> <newlord>\n\r", ch);
		return;
	  }
   ...
   REMOVE_BIT(victim->act, PLR_DEMON);
   SET_BIT(victim->act, PLR_CHAMPION);
   victim->lord = str_dup(capitalize(arg2));
   send_to_char("Ok.\n\r",ch);
   sprintf(buf, "You have fallen from grace and now follow %s.\n\r",arg2);
   send_to_char(buf,victim);
   do_autosave(victim,"");
   return;
	}
...
void do_crib( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    char buf[MAX_STRING_LENGTH];
    ...
    sprintf(buf,"%s: baby %s",ch->name,argument);
    if (ch->level < NO_WATCH) do_watching(ch,buf); 
     
    argument = one_argument( argument, arg );
    if ( arg[0] == '\0' )
    {
        send_to_char( "Punish which of the many whiners?\n\r", ch );  
        return;
    }
...
*/    
	...
	if ( IS_SET(victim->act,IN_CRIB))
         {
         REMOVE_BIT(victim->act, IN_CRIB);
         }
        send_to_char( "You can play again.\n\r", victim );
        send_to_char( "You decide they have been tortured enough.\n\r", ch );
	char_from_room(victim);
	char_to_room(victim, get_room_index(3054));
	set_title(victim, "is allowed to play again");
        sprintf(buf2,"#r%s #3has been let out of #1The Crib!#n",victim->name);
        do_info(victim,buf2);
    }
    else
    {
        send_to_char( "Act like a baby, be treated like one!\n\r", victim );
        send_to_char( "You throw them into the crib!\n\r", ch );
	char_from_room(victim);
	char_to_room(victim, get_room_index(26015));
	do_look( victim, "auto" );
	SET_BIT(victim->act, PLR_FREEZE);
	SET_BIT(victim->act, IN_CRIB);
	set_title(victim, "#b Is Making special friends with #pBARNEY#n");
        sprintf(buf2,"#r%s #3has been put in #1The Crib!#n",victim->name);
        do_info(victim,buf2);
    }
...
void do_freeze( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(victim->act, PLR_FREEZE) )
    {
	set_title(victim, "#n Is allowed to play again.#n");
	REMOVE_BIT(victim->act, PLR_FREEZE);
	send_to_char( "You can play again.\n\r", victim );
    ...
    {
	set_title(victim, "#b Is frozen.#n");
	SET_BIT(victim->act, PLR_FREEZE);
...
void do_silence( CHAR_DATA *ch, char *argument )
{
    ...
    {
	set_title(victim, "#n Is allowed to talk again.#n");
    ...
    {
	set_title(victim, "#b Talks too damn much.#n");
...
void do_adult( CHAR_DATA *ch, char *argument )
{
    char arg1[MAX_INPUT_LENGTH];
    ...
    sprintf(buf,"%s: Adult %s",ch->name,argument);
    if (ch->level < NO_WATCH) do_watching(ch,buf);
    ...
    if((arg1[0] =='\0' ) || (arg2[0] =='\0'))
          {
                send_to_char("Syntax : adult <char> <flame/chat>\n\r", ch);
                return;
          }
    ...
    {
      if ( victim->pcdata->aflame == 1 )
      {
 	victim->pcdata->aflame = 0;
 	send_to_char( "You lost your privelages.\n\r", victim );
 	send_to_char( "FLAME removed.\n\r", ch );
      }
      else
      {
	victim->pcdata->aflame = 1;
	send_to_char( "You have come of age!\n\r", victim );
	send_to_char( "FLAME set.\n\r", ch );
      }
    }
    if ( !str_cmp( arg2, "chat" ) )
    {
      if ( victim->pcdata->achat == 1 )
      {
 	victim->pcdata->achat = 0;
 	send_to_char( "You lost your privelages.\n\r", victim );
 	send_to_char( "CHAT removed.\n\r", ch );
      }
      else
      {
	victim->pcdata->achat = 1;
	send_to_char( "You have come of age!\n\r", victim );
	send_to_char( "CHAT set.\n\r", ch );
      }
    }
    return;
}
...
void do_custom( CHAR_DATA *ch, char *argument )
{
    ...
    sprintf(buf,"%s: CUSTOM %s",ch->name,argument);
    if (ch->level < NO_WATCH) do_watching(ch,buf);
    ...
    if((arg1[0] =='\0' ) || (arg2[0] =='\0'))
          {
                send_to_char("Syntax : custom <char> <fatality>\n\r",ch);
                return;
          }
    ...
    if ( !str_cmp( arg2, "fatality" ) )
    {
      if ( victim->pcdata->custom > 0 )
      {
 	victim->pcdata->custom = 0;
 	send_to_char( "You longer have custom fat's.\n\r", victim );
 	send_to_char( "Custom Fatality removed.\n\r", ch );
      }
      else
      {
	victim->pcdata->custom = 1;
	send_to_char( "You have been given custom fat's\n\r", victim );
	send_to_char( "Custom Fatality set.\n\r", ch );
      }
    }
    return;
}
...
void do_newbielock( CHAR_DATA *ch, char *argument )
{
    ...
    if ( newbielock )
        send_to_char( "Game newbie locked.\n\r", ch );
    else
        send_to_char( "Newbie lock removed.\n\r", ch );
...
/*  Crayon command by Maizoon */
    if ( !str_cmp( arg2, "crayon" ) && IS_MAGE(victim))
    {
	if (!str_cmp(arg3, "purple"))
	{
	    victim->pcdata->runes[0] = PURPLE_MAGIC;
            ...
            send_to_char("Ok.\n\r",ch);
            send_to_char("You are now an Air mage.\n\r",victim);
            return;
        }
	else if (!str_cmp(arg3, "red"))
	{
	    victim->pcdata->runes[0] = RED_MAGIC;
            REMOVE_BIT(victim->more, MORE_OUTCAST);
            send_to_char("Ok.\n\r",ch);
            send_to_char("You are now a Fire mage.\n\r",victim);
            return;
        }
	else if (!str_cmp(arg3, "green"))
	{
	    victim->pcdata->runes[0] = GREEN_MAGIC;
            REMOVE_BIT(victim->more, MORE_OUTCAST);
            send_to_char("Ok.\n\r",ch);
            send_to_char("You are now an Earth mage.\n\r",victim);
            return;
        }
	else if (!str_cmp(arg3, "blue"))
	{
	    victim->pcdata->runes[0] = BLUE_MAGIC;
            REMOVE_BIT(victim->more, MORE_OUTCAST);
            send_to_char("Ok.\n\r",ch);
            send_to_char("You are now a Water mage.\n\r",victim);
            return;
        }
	else if (!str_cmp(arg3, "yellow"))
	{
	    victim->pcdata->runes[0] = YELLOW_MAGIC;
            REMOVE_BIT(victim->more, MORE_OUTCAST);
            send_to_char("Ok.\n\r",ch);
            send_to_char("You are now a Lightning mage.\n\r",victim);
            return;
        }
	else if (!str_cmp(arg3, "black"))
	{
	    victim->pcdata->runes[0] = BLACK_MAGIC;
            REMOVE_BIT(victim->more, MORE_OUTCAST);
            send_to_char("Ok.\n\r",ch);
            send_to_char("You are now a Dark mage.\n\r",victim);
            return;
        }
	else
	    send_to_char("Please choose purple red blue green yellow or black.\n\r",ch);
	return;
    }
...
void do_oset( CHAR_DATA *ch, char *argument )
{
    ...
    if (!IS_QUESTMAKER(ch) && (obj->questmaker == NULL || str_cmp(ch->name,obj->questmaker)))
    {
	...
	if (obj->item_type == ITEM_WEAPON && !IS_JUDGE(ch)) {
		...
		send_to_char("You cannot change a primal tokens value with oset.\n\r", ch);
		return;}
    ...
    for ( d = descriptor_list; d; d = d->next )
    {
        if ( d->character && can_see( ch, d->character ) )
        {
           ...
           switch( d->connected )
           {
              ...
              case CON_GET_INVIS:            st = "   Imm Login   "; break;
...
void do_invis( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(ch->act, PLR_WIZINCOG) )
    {
	act( "$n uncloaks and fades out of existance.", ch, NULL, NULL, TO_ROOM );
	send_to_char( "You uncloak and fade out of existance.\n\r", ch );
	SET_BIT(ch->act, PLR_WIZINVIS);
	REMOVE_BIT(ch->act, PLR_WIZINCOG);
      return;
    }
...
void do_incog( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_SET(ch->act, PLR_WIZINVIS) )
    {
	act( "$n fades in and cloaks $mself.", ch, NULL, NULL,TO_ROOM );
	send_to_char( "You fade in and cloak yourself.\n\r", ch );
	SET_BIT(ch->act, PLR_WIZINCOG);
	REMOVE_BIT(ch->act, PLR_WIZINVIS);
      return;
    }
...
void do_safe( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "The Mist protects you from the sun.\n\r", ch );
        return;
    }
...
void do_artifact( CHAR_DATA *ch, char *argument )
{
	...
	if ( in_obj->carried_by != NULL )
	{
            if ( IS_NPC(in_obj->carried_by)) continue;

	    if (IS_IMMORTAL(ch))
	    {
		sprintf( buf, "%s created by %s and carried by %s. %d Time left.\n\r",
                obj->short_descr, obj->questmaker,
                PERS(in_obj->carried_by, ch), obj->timer );
	    } 
	    else
	    {
...
/* Primal tokens (rip of do_token) by Maizoon */
void do_ptoken( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC(ch) || (ch->practice < 1 && !IS_JUDGE(ch)) )
    {
	send_to_char("You are unable to make a primal token.\n\r",ch);
	return;
    }
    else if ( arg1[0] == '\0' || !is_number( arg1 ) )
    {
	send_to_char("Please specify a value for the primal token.\n\r",ch);
	return;
    }
    else
    {
	...
	if ( value < 1 || value > 100 )
        {
	    send_to_char( "Primal token should have a value between 1 and 100.\n\r", ch );
	    return;
        }
	else if ( value > ch->practice && !IS_JUDGE(ch) )
        {
	    sprintf(buf,"You only have %d primal left to put into tokens.\n\r",ch->practice);
	    send_to_char( buf, ch );
	    return;
        }
    }
...
send_to_char( "You don't have enough exp.\n\r", ch );
    ...
    sprintf(buf,"a %d point primal token",value);
    free_string( obj->short_descr );
    obj->short_descr = str_dup( buf );
    sprintf(buf,"A %d point primal token lies on the floor.",value);
    free_string( obj->description );
    obj->description = str_dup( buf );
    if (victim != NULL && victim != ch)
    {
...
void do_copyover (CHAR_DATA *ch, char * argument)
{
    ...
    if((str_cmp(ch->name, "Ktulu")) && ch->trust < 11)
    {
    send_to_char( "Sorry, Coder and High Judge use only.\n\r", ch);
    return;
    }
        if (!fp)
        {
...
void do_worldaff( CHAR_DATA *ch, char *argument)
{
    char      arg [MAX_INPUT_LENGTH];
    ...
    if IS_NPC(ch) return;
 if (IS_SET(world_affects, WORLD_FOG))  send_to_char("WORLD_FOG.\n\r", ch);
 if (IS_SET(world_affects, WORLD_RAIN)) send_to_char("WORLD_RAIN.\n\r", ch);
 if (IS_SET(world_affects, WORLD_ECLIPSE)) send_to_char("WORLD_ECLIPSE.\n\r", ch);
 if (IS_SET(world_affects, WORLD_NICE)) send_to_char("WORLD_NICE.\n\r", ch);
 if (IS_SET(world_affects, WORLD_MIST)) send_to_char("WORLD_MIST.\n\r", ch);

 if (weather_info.sunlight == SUN_LIGHT) send_to_char("SUN_LIGHT.\n\r", ch);
 if (weather_info.sunlight == SUN_RISE) send_to_char("SUN_RISE.\n\r", ch);
 if (weather_info.sunlight == SUN_SET) send_to_char("SUN_SET.\n\r", ch);
 if (weather_info.sunlight == SUN_DARK) send_to_char("SUN_DARK.\n\r", ch);

 if (weather_info.sky == SKY_CLOUDLESS) send_to_char("SKY_COUDLESS.\n\r", ch);
 if (weather_info.sky == SKY_CLOUDY) send_to_char("SKY_COUDY.\n\r", ch);
 if (weather_info.sky == SKY_RAINING) send_to_char("SKY_RAINING.\n\r", ch);
 if (weather_info.sky == SKY_LIGHTNING) send_to_char("SKY_LIGHTNING.\n\r", ch);
 if (dxp_info.dxp == 0) send_to_char("There are NO xp mods.\n\r", ch);
 if (dxp_info.dxp == 1) send_to_char("Double xp is ON.\n\r", ch);
 if (dxp_info.dxp == 2) send_to_char("Half xp is ON.\n\r", ch);


}

void do_lordname( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char      arg [MAX_INPUT_LENGTH];
    argument = one_argument( argument, arg );
    ...
    if ( arg[0] == '\0' )
    {
    send_to_char( "Whose lord do you wish to change or name?\n\r", ch );
        return;
    }
...
void do_deleteme( CHAR_DATA *ch, char *argument)
{
    char arg1[MAX_INPUT_LENGTH];
   ...
   if (IS_IMMORTAL(ch))
   {
	send_to_char("Ask an implementor to delete you!\n\r", ch);
	return;
   }
   if (arg1[0] == '\0' )
   {
	send_to_char("Usage: delete <your password>\n\r", ch);
  	return;
   }
         ...
         strcmp( crypt( arg1, ch->pcdata->pwd ), ch->pcdata->pwd ) )
    {
        ...
        sprintf( log_buf, "#RDelete#n:  #R%s#n failed deletion", ch->name);
        log_string( log_buf );
...
void do_anull( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char      arg [MAX_INPUT_LENGTH];
    argument = one_argument( argument, arg );
    ...
    if ( arg[0] == '\0' )
    {
    send_to_char( "Whose marriage/engagement do you want to anull?\n\r", ch );   
        return;
    }
    ...
    send_to_char("Ok.\n\r", ch);
    send_to_char("Your marriage is anulled.\n\r", victim);
    return;
}


void do_nonewbie( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char      arg [MAX_INPUT_LENGTH];
    argument = one_argument( argument, arg );
    ...
    if ( arg[0] == '\0' )
    {
    send_to_char( "Whose newbie status do you wish to change?\n\r", ch );
        return;
    }
...
void do_wizdox( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char      arg [MAX_INPUT_LENGTH];
    argument = one_argument( argument, arg );
    ...
    if ( arg[0] == '\0' )
    {
        send_to_char( "Who do you wish to dox??\n\r", ch );
        return;
    }
    ...
    if ( IS_NPC(victim) ) return;
        victim->paradox[0] += 3;
        victim->paradox[1] += 3;
        victim->paradox[2] += PARADOX_TICK;
}

void do_classelder( CHAR_DATA *ch, char *argument )
{
    CHAR_DATA *victim;
    char arg [MAX_INPUT_LENGTH];
    char arg2 [MAX_INPUT_LENGTH];
    ...
    if (arg[0] == '\0')
    {
	send_to_char( "Syntax: classelder <target> <on/off>\n\r", ch );
        return;
    }
    ...
    if (arg2[0] == '\0' && arg[0] != '\0')
    {
	if (IS_ACT(victim, PLR_CLASSELDER))
	{
	send_to_char("They currently privileged with this honor\n\r", ch);
	return;
	}
	else
	{
	send_to_char("They have not been privileged with this honor\n\r", ch);
	return;
	}
    }
	...
	if (str_cmp(arg2, "on")==0)
	{
		if (IS_ACT(victim, PLR_CLASSELDER))
        	{
        		send_to_char("They are already privileged with this honor\n\r",ch);
        		return;
        	}
	 	else
		{
			SET_BIT(victim->act, PLR_CLASSELDER);
			send_to_char("They are now a class elder\n\r",ch);
			return;
		}
	}
	else if (str_cmp(arg2, "off")==0)
	{
		if (!IS_ACT(victim, PLR_CLASSELDER))
                {
                        send_to_char("They are not currently privileged with this honor\n\r", ch);
                        return;
                }
                else
                {
                        REMOVE_BIT(victim->act, PLR_CLASSELDER);
                        send_to_char("They are no longer a class elder\n\r",ch);
                        return;
                }
	}
	else
    ...
    send_to_char("Sorry, this command is for Demons only at this time.\n\r", ch);
    return;
}

void do_fightclub( CHAR_DATA *ch, char *argument )
{
    char arg1 [MAX_INPUT_LENGTH];
    char arg2 [MAX_INPUT_LENGTH];
    char arg3 [MAX_INPUT_LENGTH];
    ...
    sprintf(buf, "%s: Fightclub %s", ch->name, argument);
    if (ch->level < NO_WATCH) do_watching(ch,buf);
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' || arg3[0] == '\0' )
    {
    send_to_char("Syntax: fightclub <victim> win/lose <value>\n\r", ch);
    return;   
    }
   ...
   if ( !str_cmp( arg2, "lose" ) )
    {
        ...
        if (victim->pcdata->quest - value < 1 || value > 1500)
        {
        send_to_char("Range is between 1 and 1500.\n\r", ch);
        return;
        }
        ...
        {
          send_to_char("Ok.\n\r",ch);
          sprintf(buf, "You lose you bet and give up %s quest points.\n\r",arg3);
          send_to_char(buf,victim);
          return;
        }
}
  if ( !str_cmp( arg2, "win" ) )
    {   
        ...
        {
        send_to_char("Fightclub range is 1 to 1500.\n\r", ch);
        return;
        }
        ...
        {
        send_to_char("Ok.\n\r",ch);
        sprintf(buf, "You win your bet and gain %s quest points.\n\r",arg3);
        send_to_char(buf, victim);
        return;
        }
}
