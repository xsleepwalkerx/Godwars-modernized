...
{ "RP",    "RP discussion",            0,     2,     "all", DEF_INCLUDE,18, NULL, FALSE },
{ "Ideas",	  	"Suggestion for improvement",	 0,     2,     "all", DEF_NORMAL, 30, NULL, FALSE }, 
{ "Announce", 	"Announcements from Immortals",  0,     7, "all", DEF_NORMAL, 30, NULL, FALSE },
{ "Bugs",	  	"Typos, bugs, errors",		     0,     1,     "imm", DEF_NORMAL, 25, NULL, FALSE },
{ "Personal", 	"Personal messages",			 0,     1,     "all", DEF_EXCLUDE,20, NULL, FALSE },
{ "Quests",  	"Quest info",            0,     8,     "all", DEF_INCLUDE,14, NULL, FALSE },
{ "Coder",      "Code Changes", 7,      7,     "imm",  DEF_NORMAL,60,  NULL, FALSE }

};

/* The prompt that the character is given after finishing a note with ~ or END */
const char * szFinishPrompt = "(" BOLD "C" NO_COLOR ")ontinue, (" BOLD "V" NO_COLOR ")iew, (" BOLD "P" NO_COLOR ")ost, (" BOLD "Q" NO_COLOR ")uietly post, or (" BOLD "F" NO_COLOR ")orget it?";
...
static void do_nwrite (CHAR_DATA *ch, char *argument)
{
	...
	{
		{
		    time_t note_expire = (time_t) ch->pcdata->in_progress->expire; /* Merge repair: ctime expects time_t*. */
...
void handle_con_note_subject (DESCRIPTOR_DATA *d, char * argument)
{
	...
	/* advance to next stage */
	{
		...
		else
		{
			...
			{
			    time_t note_expire = (time_t) ch->pcdata->in_progress->expire; /* Merge repair: ctime expects time_t*. */
			    sprintf (buf, "This note will expire %s\r", ctime(&note_expire));
			}
			write_to_buffer (d,buf,0);
			write_to_buffer (d, "\n\rEnter text. Type " BOLD "~" NO_COLOR " or " BOLD "END" NO_COLOR " on an empty line to end note.\n\r"
			                    "=======================================================\n\r",0);
			...
			case 'f':
				...
				break;
                        ...
                        case 'p': /* post note */
                                ...
                                act (BOLD GREEN "$n finishes $s note." NO_COLOR , ch, NULL, NULL, TO_ROOM);
				REMOVE_BIT(ch->extra,EXTRA_AFK);
        /* Announce new notes (player idea) Made it so invis imms don't announce notes --Aristoi */
                                /* if ( !IS_SET(ch->act, PLR_WIZINVIS) ) */
                                if (!IS_IMMORTAL(ch))
				{
                                    sprintf(buf,"#r%s #Ghas posted a new note.", ch->name);
                                    do_info(ch,buf);
                                }
                                break;
