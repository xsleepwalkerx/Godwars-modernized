...
/* Affect_free doesn't seem to store free'd affects, GW mailing post fix -Xentoth */
    affect_free = paf;
...
void equip_char( CHAR_DATA *ch, OBJ_DATA *obj, int iWear )
{
    ...
    if ((IS_NPC(ch) || !IS_VAMPIRE(ch)) && ((obj->pIndexData->vnum == 29606) ||
	   (obj->pIndexData->vnum == 29607) || (obj->pIndexData->vnum == 29608) ||
	   (obj->pIndexData->vnum == 29609) || (obj->pIndexData->vnum == 29610)))
    {
    ...
    if ((ch->vampgen > 2) && (obj->pIndexData->vnum == 29610))
    {
	act( "Your blood is not pure enough to wield this weapon, you are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	obj_to_room( obj, ch->in_room );
	do_autosave( ch, "" );
    ...
    if ((IS_NPC(ch) || !IS_MAGE(ch)) && ((obj->pIndexData->vnum == 29611) ||
	   (obj->pIndexData->vnum == 29612) || (obj->pIndexData->vnum == 29613) ||
	   (obj->pIndexData->vnum == 29614) || (obj->pIndexData->vnum == 29615)  ||
	   (obj->pIndexData->vnum == 29616)))
    {
	act( "You are zapped  by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
    ...
    {
	/* Merge repair: fix invalid multiline string literal carried over from older source. */
	...
	if ((IS_NPC(ch) || !IS_NINJA(ch)) && ((obj->pIndexData->vnum == 29617) ||
	   (obj->pIndexData->vnum == 29618) || (obj->pIndexData->vnum == 29619) ||
	   (obj->pIndexData->vnum == 29620) || (obj->pIndexData->vnum == 29621)))
    {
	act( "You are zapped  by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	...
	if ((ch->vampgen > 2) && (obj->pIndexData->vnum == 29621))
    {
	act( "Your knowledge of the martial arts is not great enough to wield this weapon, you are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	obj_to_room( obj, ch->in_room );
	do_autosave( ch, "" );
	...
	if ((IS_NPC(ch) || !IS_WEREWOLF(ch)) && ((obj->pIndexData->vnum == 29622) ||
	   (obj->pIndexData->vnum == 29623) || (obj->pIndexData->vnum == 29624) ||
	   (obj->pIndexData->vnum == 29625) || (obj->pIndexData->vnum == 29626) || 
	   (obj->pIndexData->vnum == 29627)))
    {
	act( "You are zapped  by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
    ...
    if ((ch->vampgen > 2) && (obj->pIndexData->vnum == 29627))
    {
	act( "You strong enough to wield this weapon, you are zapped by $p and drop it.", ch, obj, NULL, TO_CHAR );
	act( "$n is zapped by $p and drops it.", ch, obj, NULL, TO_ROOM );
	obj_from_char( obj );
	obj_to_room( obj, ch->in_room );
	do_autosave( ch, "" );
...
*/
	...
	if (IS_SET(obj->spectype, SITEM_DEMONIC) && !IS_NPC(ch) && IS_DEMON(ch))
	{
	    ...
	    {
		ch->pcdata->demonic++;
		if (obj->specpower == ARM_RED) ch->pcdata->atm += 3;
		if (obj->specpower == ARM_BLACK   ) ch->pcdata->atm += 4;
		if (obj->specpower == ARM_MIDNIGHT ) ch->pcdata->atm += 5;
	    }
	}
...
void extract_obj( OBJ_DATA *obj )
{
	...
	if ( ( pexit = obj->in_room->exit[direction] ) != NULL )
	{
	    ...
	    REMOVE_BIT(pexit->exit_info, EX_WALL_WIND);
...
/* Crib code --> Mistif */
    ...
    /* Freeze Tag addin -Xentoth */
    if (IS_SET(ch->tag_flags,TAG_PLAYING) && IS_SET(victim->tag_flags,TAG_PLAYING))
        return TRUE;
    if (!IS_NPC(victim) && IS_VAMPAFF(victim, VAM_EARTHMELDED) && !IS_IMMORTAL(ch))
        return FALSE;
    if (IS_MORE(victim, MORE_SPIRIT) && !IS_VAMPAFF(ch, VAM_SPIRITEYES) && !IS_IMMORTAL(ch)) 
	return FALSE;
    if (IS_AFFECTED(victim, AFF_NINJAHIDE) && IS_IMMORTAL(ch))
       return TRUE;
    if (IS_AFFECTED(victim, AFF_NINJAHIDE) && !IS_NINJA( ch ))
       return FALSE;
    if (IS_AFFECTED(victim, AFF_NINJAHIDE) && !IS_KOE( ch ))
       return FALSE;
/*	Uncomment and then comment the line above when hybrids are added in -Xentoth
    if (IS_AFFECTED(victim, AFF_NINJAHIDE) && !IS_NINJA( ch ) && !IS_LICH(ch) &&
!IS_BLACK_SPIRAL_DANCER(ch) && !IS_ABOMINATION(ch))
       return FALSE;
*/
    if (IS_EXTRA( ch, BLINDFOLDED))
        return FALSE;
/*    if (IS_SET(world_affects, WORLD_MIST) && (IS_OUTSIDE(ch)) && !IS_IMMORTAL(ch))
        return FALSE;
*/
    if (!IS_NPC(ch) && IS_EXTRA2(ch, EXTRA_CAN_SEE))
    {
	if (ch->pcdata->familiar == NULL)
	    REMOVE_BIT(ch->extra2, EXTRA_CAN_SEE);
    ...
    if ( !IS_NPC(victim) && IS_IMMORTAL(victim)
    && IS_SET(victim->act, PLR_WIZINCOG)
    && get_trust( ch ) < get_trust(victim)
    && ch->in_room != victim->in_room)
        return FALSE;
/*
    if ( !IS_NPC(victim) && (IS_AFFECTED(victim, AFF_NINJAHIDE)
       || (victim, AFF_
*/
    if ( IS_VAMPAFF(victim, VAM_OBFUSCATE) && !IS_IMMORTAL(ch) &&
	!IS_ITEMAFF(ch, ITEMA_VISION))
    {
	if (IS_FAE(ch) && IS_POLYAFF(ch, POLY_ZULO)) return TRUE;
        ...
        if (!IS_NPC(ch))
	{
		...
		return TRUE;
            else if (IS_NINJA(ch) && (!str_cmp(ch->clan, "Nightstalkers")))
                return TRUE;
	    else 
		return FALSE;
	}
    ...
    if (!IS_NINJA(ch) && IS_AFFECTED(victim, AFF_NINJAHIDE))
        return FALSE;
